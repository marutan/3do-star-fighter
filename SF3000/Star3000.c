 ...
 *																								   *
 *									  (c)
 *Krisalis software 1995
 **
 *																								   *
 *							  Written by Fednet -
 *T.D.Parry & A.P.Hutchings						   *
 *																								   *
 ***************************************************************************************************/
...
performance_data player_resetperformance = {
    // Players performance reset data, *=not applicable for player
    0,
    0,
    2, // Engine, Control, Shields
    {
    ...
    0,
    0,
    0 // Laser Rate*, Missile Rate*, Launch Rate*
};

performance_data player_megaperformance = {
    // Players mega performance data, *=not applicable for player
    7,
    7,
    7, // Engine, Control, Shields
    {
...

{
  ...
  configure_waiting =
      nvram_load("StarFighter.Config", "SFC:", (char *) &configuration,
                 sizeof(game_configuration));

  if (configure_waiting == 0 ||
      config_version != configuration.version) { // No file or out of date one ?
  ...
  // cheat_feature1 = 1;
  // menus [CONFIGURE_MENU].items[3].function_value = 1;
  // // Yes - from now on, cheat !

  pyramid_initialisegame(); // Clear all pyramid levels
  arm_celinitialisecreation(cel_creation,
                            CEL_MAXCREATION); // Initialise creation cels
  ...
  ****************************************************************************************************/
  ...
  do {
    ...
    // THIS CODE BELOW ENTERS THE SELECT LANGUAGE MENU IF THE GAME COULD NOT
    // LOAD A NVRAM FILE FROM NVRAM (FIRST TIME RUN ?) - IT HAS BEEN COMMENTED
    // OUT FOR THE AMERICAN VERSION WHICH GOES STRAIGHT INTO THE MAIN MENU. (NO
    // LANGUAGE SELECT)

    // IF LANGUAGE SELECT IS PUT BACK IN, DON'T FORGET TO ALTER IN MENU
    // (STRUCTURE) AND CHEAT ACTIVATOR BECAUSE OF EXTRA LINE

    menu_enter(MAIN_MENU, 1); // Enter main menu

    // if (configure_waiting == 1)
    // // YES - go straight into main menu 	menu_enter (MAIN_MENU, 1); else
    //	menu_enter (LANGUAGE_MENU, 1);

    screen_setswap(1); // Set screen swap
    parameters.menu_display =
        DISPLAY_MENU; // Set hiscore toggle to menu display
    ...
    ****************************************************************************************************/
    ...
    do {
      ...
      pyramid_initialiseselect(
          parameters.level); // Initialise pyramid mission selection
      ...
      do {
        ...
        arm_addgamecel(&cel_quad, BASECEL_PYRAMID_LEVEL + parameters.level,
                       1024, 1024);
      ...
      // ARE WE STARTING A MISSION , IF SO, LOAD IT, DISPLAY DETAILS ETC. RUN
      // MISSION
      ...
      if (selection == PYRAMID_START) // Starting a mission ?
      {
        ...
        {
          pyramid_initialiseselect(
              parameters.level); // Started & Quit brief ? - Initialise pyramid
                                 // mission selection
...
/****************************************************************************************************
/									SWITCH
MENU'S ON TIMER / KEYPRESS ?
/
****************************************************************************************************/

void
menuswitch_update(long keypad)
  ...
  switch (parameters.menu_display) {
    case DISPLAY_MENU:
      ...
      if (keypad != 0) {
        ...
        return;
      } else if (--parameters.display_timer <= 0) {
...
void
menuswitch_next(long nextdisplaytype)
...

{
  ...
  char hiscore_string[4] = {MTXT__TRAINING, MTXT__EASY, MTXT__MEDIUM,
                            MTXT__HARD};
  ...
  if (nextdisplaytype == DISPLAY_HISCORES) {
    ...
    if (++parameters.demo_counter >= 4) // Do demo or credits ?
    {
      ...
      sprintf(message_decode(menus[HIGHSCORE_MENU].title, 1), "%s : %s",
              message_decode(MTXT__HIGHSCORE, 0),
              message_decode(hiscore_string[parameters.display_hiscoretable],
                             0));        // Do menu title
      ...
      for (entry = 0; entry < 5; entry++) // Put in high score table
      {
        hs_ypos = 64 + (32 * entry);
        cel_addalphastring(
            configuration.high_scores[parameters.display_hiscoretable][entry]
                .name,
            35, hs_ypos, 1024);
        sprintf(
            hs_score, "%ld",
      ...
      if (++parameters.display_hiscoretable >=
          4) { // Make so next shows next level, if >4, reset to zero
...
/****************************************************************************************************
/								  DISPLAY
MISSION DETAILS PRIOR TO START /
****************************************************************************************************/

long
game_missionstart(long show_credits)

// Purpose : Loads in mission & Prints up all mission details prior to start,
// then calls game_play Accepts : Nothing Returns : Nothing

{
  ...
  if (show_credits == 0) {
    load_mission(
        parameters.level,
        1 + return_mission(&pyramids[parameters.level])); // Load in mission &
                                                          // Initialise briefing
    screen_setswap(1);                                    // Set screen swap
  }

  control_showmission(show_credits,
                      message_initialisemissionfiles(
                          configuration.language,
                          (show_credits == 0) ? parameters.level : -1,
  ...
  do {
    ...
    if (show_credits ==
        0) // If doing mission brief, display insignia across menu title
    {
      cel_quad.y_pos0 = -105;
      cel_quad.shade = 7;
      cel_quad.x_pos0 = -120 - 8;
      arm_addgamecel(&cel_quad, BASECEL_PYRAMID_LEVEL + parameters.level, 1024,
                     1024);
      cel_quad.x_pos0 = 120 - 8;
      arm_addgamecel(&cel_quad, BASECEL_PYRAMID_LEVEL + parameters.level, 1024,
                     1024);
      screen_update(S_CEL | S_MENU | S_BANK |
                    S_WAITFRAME); // Render screen + title + cels
    } else {
...

  {
    if (configuration.music_on == 1 && configuration.sound_on == 1) {
      music_play(MUSIC_MOVETRACK, 0,
                 -1); // Continue playlist for return to main menu
    }
  }
...

{
  ...
  long shield_extent = 0, // Shield display variables
      ...
      crystal_loop, crystal_size, cel_graphicslimit,

       shipheight, shipheighticon, scanlocposition,

       remember_score, waitdelaytype;
  ...
  if (configuration.video_on == 1) {
    sprintf(intro_clip, "Into%ld",
            1 + (return_mission(&pyramids[parameters.level])) % 3);
  ...
  ****************************************************************************************************/
  ...
  do {
    ...
    // READ KEYPAD (DEBOUNCE 1 OF THE KEYPAD VARIABLES, KEEP THE OTHER
    // IMMEDIATE)
    ...
    if (menu_status.current_menu == NO_MENU) {
      ...
      if (keypad &
          keypad_iomap[configuration
                           .flight_controls[FLIGHT_VIEW_MAP]]) { // Check for
                                                                 // map display
      ...
      if (keypad &
          keypad_iomap[configuration.flight_controls
                           [FLIGHT_CHANGE_CAM]]) { // Check for camera change
        control_changecamera((status.current_camera == 1) ? 2 : 1);
      }

      if (keypad &
          keypad_iomap[configuration.flight_controls
                           [FLIGHT_SELECT_WEAPON]]) { // Check for weapon change
        control_selectweapon();
      }

      if (keypad &
          keypad_iomap[configuration.flight_controls[FLIGHT_MENU]]) // Entering
                                                                    // game menu
                                                                    // ?
      {
    ...
    // DECIDE WHAT CAMERA TO USE, THEN RENDER 3D WORLD TO CEL LIST USING THAT
    // CAMERA

    if (status.status == STATUS_CRASHED ||
        status.status == STATUS_GAMEOVERWAIT) { // If player is crashing,
                                                // display using camera 0
      camera_touse = 0;
    } else {
      if (camera[status.current_camera].instance ==
          -1) { // Otherwise, if player is using cam2 & no item, use cam1
        camera_touse = 1;
      } else {
    ...
    {
      if (status.track_status == TRACKING_STATUS_FREE &&
          status.clock2 == 0) { // Update tracking objects ?
        control_retrack();
      }
    }
    ...
    if (menu_status.current_menu == NO_MENU) {
      ...
      if (players_ship->shields > 0) {
        ...
        cel_quad.shade =
            (shield_value < 32 || players_ship->damage != DAMAGE_NOTHING)
                ? (status.clock2 & 8) + 6
                : 11;
        ...
        cel_quad.x_pos0 = -135;
        cel_quad.y_pos0 = -103;
        shield_extent = players_ship->performance->shields
                        << 5;                   // Shields Rating (32 to 255)
        shield_value = (players_ship->shields); // Current Value (0-255)
        arm_addgamecel(&cel_quad, BASECEL_SHIELD + 0, 1024,
                       1024); // Put in shields bar
        arm_setcel_hw(&cel_quad, shield_value & 255,
                      -1); // Set it's length to shields value

        // Add shields (BLUE BIT)

        cel_quad.x_pos0 = shield_value - 135;
        arm_addgamecel(&cel_quad, BASECEL_SHIELD + 1,
                       (shield_extent - shield_value) << 7, 1024);
        ...
        if (cel_quad.shade == 6) {
          ...
          cel_quad.y_pos0 = -105;
          cel_quad.shade = 11;
          arm_addgamecel(&cel_quad, BASECEL_SHIELD + 3, shield_extent << 7,
                         1024);
        }
        ...
        if (status.track_type != TRACKING_NOTHING) {
          if (status.track_type == TRACKING_SHIP) // Tracking a ship ?
          {
            ...
            target.x_aim = (status.track_xpos)
                           << 24; // Tracking a ground object ?
        ...
        arm_addgamecel(&cel_quad,
                       BASECEL_GRIDREF + (((players_ship->x_pos) >> 29) & 7),
                       1024, 1024);

        cel_quad.x_pos0 += 9;
        arm_addgamecel(&cel_quad,
                       BASECEL_GRIDREF + 8 +
                           (((players_ship->y_pos) >> 29) & 7),
                       1024, 1024);
        ...
        if (status.track_type != TRACKING_NOTHING) {
          ...
          cel_quad.x_pos0 += 16;
          arm_addgamecel(&cel_quad, BASECEL_GRIDLOCK + status.track_status,
                         1024, 1024);

          cel_quad.x_pos0 += 24;
          arm_addgamecel(&cel_quad, tracking_graphic, 1024,
                         1024); // Display tracking type

          cel_quad.x_pos0 += 24;
          arm_addgamecel(&cel_quad, BASECEL_GRIDREF + tracking_distance, 1024,
                         1024); // Display distance
          ...
          {
            shipheight =
                (players_ship->z_pos - status.ship_tracking->z_pos) >> 25;
            shipheighticon = 2;
            if (shipheight != 0) {
              shipheighticon = (shipheight > 0) ? 1 : 0;
            }
            arm_addgamecel(&cel_quad, BASECEL_GRIDSIGN + shipheighticon, 1024,
                           1024);
            cel_quad.x_pos0 += 27;
          }
          ...
          tracking_distance =
              ((target.x_rot + (64 << 10) - players_ship->x_rot) &
               ((1024 << 10) - 1)) >>
              17;
          arm_addgamecel(&cel_quad, BASECEL_DIRECTION + tracking_distance, 1024,
                         1024);
        }
      }
      ...
      if (!(status.weapon != WEAPON_LASER &&
            players_ship->performance->weapons[status.weapon] <= 0 &&
            (status.clock1 & 2))) {
        cel_quad.x_pos0 = 124;
        cel_quad.y_pos0 = -106;
      ...
      for (crystal_loop = 0; crystal_loop <= 3; crystal_loop++) {
        if (status.pickups[crystal_loop] != 0) {
          ...
          arm_addgamecel(&cel_quad,
                         BASECEL_CRYSTAL + (status.pickups[crystal_loop] - 1),
      ...
      if (mission.mission_timer > 0) {
        cel_quad.x_pos0 = -135;
        cel_quad.y_pos0 = 83;
        arm_addgamecel(&cel_quad, BASECEL_TIMER + ((status.clock2 >> 2) & 3),
                       1024, 1024);
      }
      ...
      cel_quad.x_pos0 =
          -(((cockpit_missiles + cockpit_enemybig + cockpit_enemysmall) * 5) >>
            1);
      cel_quad.y_pos0 = 86;
      cel_quad.shade = 11;
    ...
    if (planet_info.space_mission == 1) {
      ...
      // armanim_update (animate, *sprite_map);			// NO - Update
      // Ground Animations [TAKEN OUT]
      armburn_updateexplosions(*sprite_map,
                               &cel_quad); // NO - Update Ground Explosions
  ...
  while (status.status != STATUS_GAMEOVER &&
         status.status != STATUS_MISSIONCOMPLETED);

  if (status.status ==
      STATUS_GAMEOVER) // Reset players performance if they have failed mission
  {
  ...
  if (status.status == STATUS_MISSIONCOMPLETED) {
    if (pyramid_registercompleted() ==
        2) { // Mark mission as completed - have we finished pyramid ?
...
void
game_checkforhighscore(long load_backdropfile)

// Purpose : Looks to see if a hi-score should be entered, if so, gets it and
// enters it Accepts : Pointer to backdrop to load (ignored if null) Returns :
// nothing

{
  long score_search, score_shift, score_positionfound = -1;
  ...
  do {
    if (status.score >=
        configuration.high_scores[parameters.level][score_search].score) {
  ...
  if (score_positionfound != -1) {
    ...
    for (score_shift = 4; score_shift > score_positionfound; score_shift--) {
      configuration.high_scores[parameters.level][score_shift].score =
          configuration.high_scores[parameters.level][score_shift - 1].score;
      strcpy(configuration.high_scores[parameters.level][score_shift].name,
             configuration.high_scores[parameters.level][score_shift - 1].name);
    }
    ...
    configuration.high_scores[parameters.level][score_positionfound].score =
        status.score;
    menu_getname(
        configuration.high_scores[parameters.level][score_positionfound].name,
        16, message_decode(MMNU_GETNAME + 1, 1), 1);
    ...
    parameters.menu_display =
        DISPLAY_MENU; // Set hiscore toggle to menu display
    parameters.display_hiscoretable =
        parameters.level; // Look at this hiscore next
  }
}
...
// Purpose : Initialise everything (system resources, base files etc.) at game
// start Accepts : nothing Returns : nothing, any fatal errors, program
// terminates by calling DONE

{
  ...
  if (!OpenGraphics(screen, 2)) {
    game_end(); // If failed, quit game
  }
  ...
  SetCEControl(screen->sc_BitmapItems[0], 1 << 26,
               1 << 26); // Enable cel clipping for screen 0
  SetCEControl(screen->sc_BitmapItems[1], 1 << 26,
               1 << 26); // Enable cel clipping for screen 1
  ...
  if (cel_initialise() == 0) {
    game_end(); // Init Cel Memory Areas
  }
  if (font_loadin() == 0) {
    game_end(); // Load in fonts
  }
...
// Purpose : Plays a demo mission, changing tracking camera randomly for preset
// time Accepts : nothing Returns : nothing

{

  long keypad, camera_swivel = 0, timeout_cameratype = 128,
               timeout_cameraview = 128, message_timer = 256, message_count = 0;
  ...
  status.formation_mode =
      SDB_FORMATION_AGGRESSIVE; // Set friendlies to aggressive
  ...
  do // Main demo loop
  {
    ...
    if (--message_timer < 0) {
      message_add(MESSAGE_DEMO, MTXT__DEMOTEXTSTART + message_count, GREEN_15,
                  MESSAGE_SCROLL, 96, MESSAGE_CURSOR1);
      if (++message_count >= 4) {
        message_count = 0;
        message_timer = 512;
      } else {
    ...
    if (camera[0].counter2 == 0) {
      ...
      if (--timeout_cameraview < 0) // Change instance of view ? not if ship we
                                    // are watching is in trouble !
      {
    ...
    if (!(status.status ==
          STATUS_GAMEOVER)) { // DRAW SCREEN (Only if not just about to quit)
...
void
game_pyramidcomplete(void)

// Purpose : Pyramid is finished - do something ! Move onto next level - Play
// outro if game finished ! Accepts : Now't Returns : Now't
