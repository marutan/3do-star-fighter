...
// INITIALISE VARIABLES AND GRAPHICS
	...
	nearest_found = TRACKING_SHIP;				// Set current tracking status to default (player) or locked status
...
// ----------------------
		...
		if (planet_info.space_mission != 1)
			...
			y_draw = -16;	x_draw = -16;
			cel_quad.x_pos0 = (3 + ((x_draw*6) - ((6 - (y_draw*6))>>1)));		// Bottom Left
			cel_quad.y_pos0 = (6-(y_draw*6));
			
			y_draw = -16;	x_draw = 15;
			cel_quad.x_pos1 = (6 + 3 + ((x_draw*6) - ((6 - (y_draw*6))>>1)));	// Bottom Right
			cel_quad.y_pos1 = (6-(y_draw*6));
						
			y_draw = 15;	x_draw = 15;
			cel_quad.x_pos2 = 6 + ((x_draw*6) - ((6 - (y_draw*6))>>1));			// Top Right	
		...
		if (status.track_type != TRACKING_NOTHING)
			...
			cel_quad.y_pos0 += 8;
			arm_addgamecel (&cel_quad, (status.track_type == TRACKING_SHIP) ? BASECEL_GRIDLOCK + 2 : BASECEL_GRIDLOCK + 3, 1024, 1024);
			}
		...
		for (icon_loop = 0; icon_loop < 3; icon_loop++)
			...
			cel_quad.shade = (icon_loop == (4-map_zoom)) ? 9+icon_loop : 4+icon_loop;
		...
		if (nearest_found == TRACKING_NOTHING)
			...
			if (status.track_type == nearest_found && status.track_status == TRACKING_STATUS_LOCKED)
				{
				if (nearest_found == TRACKING_SHIP)
					{
...
long	item_side;
	...
	if (map_getoffsets (&item_xpos, &item_ypos) == 1)				// Adjust co-ordinates for around players pos & zoom
		...
		if (info_type == 0)
			{
			arm_addgamecel (&cel_quad, BASECEL_DIRECTION + item_rotation, 1024, 1024);		// Display direction arrow
...
long	item_ypos = ship->y_pos;										// Get ship's ypos
	...
	if (map_getoffsets (&item_xpos, &item_ypos) == 1)					// Adjust co-ordinates for around players pos & zoom
		...
		arm_addgamecel (&cel_quad, item_type_sprite, 1024, 1024);		// Display type sprite
		}
}
...
long	distance;
	...
	for (object_loop = 0; object_loop < mission.groundcount; object_loop++)
		...
		if (mission.groundlist [object_loop].type == GROUND_DESTROY || cheat_feature4 == 1)	
			{
			...
			// Adjust & draw target icon on map (if on screen) - Also draw locked cross if locked onto & calc new temp lock
			...
			if (map_getoffsets (&item_xpos, &item_ypos) == 1)		
				...
				if ((!(map_toggle !=0)) && status.track_status == TRACKING_STATUS_LOCKED &&
				status.track_type == TRACKING_GROUND && status.track_xpos == item_gridxpos && status.track_ypos == item_gridypos)
					{
					cel_quad.x_pos0-=7;
					cel_quad.y_pos0-=7;
					arm_addgamecel (&cel_quad, BASECEL_MAP_QUESTION, 1024, 1024);		// Display lock icon
					}	

				// Work out what nearest distance check should be (ie. temp tracking ship / nothing)
				
				item_xpos = (((map_x + cross_x) & 255) <<24) - item_rememberxpos;		// Get object's xpos relative to cross
				item_ypos = (((map_y + cross_y) & 255) <<24) - item_rememberypos;		// Get object's ypos relative to cross
				...
				item_ypos += (height_map [item_gridypos] [item_gridxpos]) >> map_zoom;	// Adjust for height offset
