...
long map_x, // Map X, Y Offsets and zoom
    map_y, map_zoom, cross_x, cross_y, item_xpos, item_ypos;

ship_stack *nearest_ship = NULL;

long nearest_found, tracking_x = 0, tracking_y = 0, nearest_distance;
...

{
  ...
  long map_timer = 0, keypad, keypad_constant, icon_loop;

  long last_xgrid = -1, last_ygrid = -1;

  long x_draw, y_draw, cross_lock_fail, move_speed;

  long move_speeds[5] = {
      0,  // Zoom = 0	// NOT AVAILABLE ANYMORE DUE TO A TIGHT FISCAL OUTLOOK
  ...
  nearest_found = TRACKING_SHIP; // Set current tracking status to default
                                 // (player) or locked status
  nearest_ship = players_ship;
  tracking_x = (players_ship->x_pos >> 24) & 255;
  tracking_y = (players_ship->y_pos >> 24) & 255;

  if (status.track_status == TRACKING_STATUS_LOCKED) {
    nearest_found = status.track_type;

    switch (status.track_type) {
      case TRACKING_GROUND:
        tracking_x = (status.track_xpos & 255);
        tracking_y = (status.track_ypos & 255);
        break;

      default:
        tracking_x = (status.ship_tracking->x_pos >> 24) & 255;
        tracking_y = (status.ship_tracking->y_pos >> 24) & 255;
        break;
    }
  }
  ...
  do {
    ...
    move_speed = move_speeds[map_zoom]; // Ammount to move by
    if (nearest_found != TRACKING_NOTHING) {
      move_speed = move_speed >> 1; // Half it if we are temporarily locked
    }

    if (keypad_constant & ControlLeft) { // Moving map left ?
      cross_x = (cross_x - move_speed) & 255;
    }
    ...
    if (keypad_constant & ControlDown) { // Moving map down ?
      cross_y = (cross_y - move_speed) & 255;
    }
    ...
    if ((keypad & ControlRightShift) && map_zoom > 2) // Zoom map in ?
    {
      ...
      map_zoom -= 1;
      if (nearest_found != TRACKING_NOTHING) {
        cross_x = tracking_x & 255;
        cross_y = tracking_y & 255;
      }
    }
    ...
    // DRAW BIG MAP ON SCREEN (ONLY IF NOT SPACE) , + BORDER
    ...
    if (planet_info.space_mission != 1) {
      ...
      x_draw = -16;
      cel_quad.x_pos0 =
          (3 + ((x_draw * 6) - ((6 - (y_draw * 6)) >> 1))); // Bottom Left
      cel_quad.y_pos0 = (6 - (y_draw * 6));

      y_draw = -16;
      x_draw = 15;
      cel_quad.x_pos1 =
          (6 + 3 + ((x_draw * 6) - ((6 - (y_draw * 6)) >> 1))); // Bottom Right
      cel_quad.y_pos1 = (6 - (y_draw * 6));

      y_draw = 15;
      x_draw = 15;
      cel_quad.x_pos2 =
          6 + ((x_draw * 6) - ((6 - (y_draw * 6)) >> 1)); // Top Right
      cel_quad.y_pos2 = (6 - (y_draw * 6)) - 6;

      y_draw = 15;
      x_draw = -16;
      cel_quad.x_pos3 = (x_draw * 6) - ((6 - (y_draw * 6)) >> 1); // Top Left
      cel_quad.y_pos3 = (6 - (y_draw * 6)) - 6;

      cel_quad.shade = 0;
      arm_addmonocel(&cel_quad, 0, DARK_PAL, 0);
    }

    // DRAW BOX FOR ZOOM ICONS

    cel_rectangle(25, 62, 83, 20, BLUE_PAL, REC_BORDER | REC_BOX, 1);

    // DRAW CO-ORDINATES OF HUNTING CROSS

    cel_quad.x_pos0 = 30 - 160;
    cel_quad.y_pos0 = 46 - 120;
    arm_addgamecel(&cel_quad, BASECEL_MAP_NUMBERS + (cross_x >> 5), 1024, 1024);

    cel_quad.x_pos0 = 38 - 160;
    arm_addgamecel(&cel_quad, BASECEL_MAP_LETTERS + (cross_y >> 5), 1024, 1024);

    if (cross_x >> 5 != last_xgrid || cross_y >> 5 != last_ygrid) {
      last_xgrid = cross_x >> 5;
      last_ygrid = cross_y >> 5;
      sound_playsample(SOUND_BEEP, 60, 50, 0);
    }

    // DISPLAY TRACKING LOCK STATUS

    if (status.track_type != TRACKING_NOTHING) {
      cel_quad.x_pos0 = 54 - 160;
      arm_addgamecel(&cel_quad, BASECEL_GRIDLOCK + status.track_status, 1024,
                     1024);

      cel_quad.y_pos0 += 8;
      arm_addgamecel(&cel_quad,
                     (status.track_type == TRACKING_SHIP)
                         ? BASECEL_GRIDLOCK + 2
                         : BASECEL_GRIDLOCK + 3,
                     1024, 1024);
    ...
    for (icon_loop = 0; icon_loop < 3; icon_loop++) {
      cel_quad.x_pos0 = (30 + (icon_loop << 4)) - 160;
      cel_quad.y_pos0 = (25) - 120;
      cel_quad.shade =
          (icon_loop == (4 - map_zoom)) ? 9 + icon_loop : 4 + icon_loop;
    ...
    if (nearest_found == TRACKING_NOTHING) {
      ...
      arm_addgamecel(&cel_quad, BASECEL_MAP_QUESTION + 1, 1024,
                     1024); // Draw it
    } else {
      cross_lock_fail = 0;
      ...
      if (status.track_type == nearest_found &&
        ...
        if (nearest_found == TRACKING_SHIP) {
          ...
          if (status.track_xpos == tracking_x &&
              status.track_ypos == tracking_y) {
            cross_lock_fail = 1;
          }
        }
      ...
      if (cross_lock_fail == 0) {
        ...
        if (map_getoffsets(&item_xpos, &item_ypos) == 1) {
          if (nearest_found == TRACKING_SHIP) {
            cel_quad.x_pos0 = item_xpos - 13;
            cel_quad.y_pos0 = item_ypos - 13;
          } else {
            item_ypos -= ((height_map[tracking_y][tracking_x]) >> map_zoom);
            cel_quad.x_pos0 = item_xpos - 11;
            cel_quad.y_pos0 = item_ypos - 12;
          }
          arm_addgamecel(&cel_quad, BASECEL_MAP_QUESTION, 1024,
                         1024); // Draw it (target found)
        }
      }
    }
    ...
    // DRAW GROUND OBJECTS, SHIPS, FLASHING TARGETS, ICONS ON SCREEN THEN RENDER
    // AND DISPLAY SCREEN

    if (mission.scanner_bust == 0) {
      arm_drawmaptargets(poly_map, map_zoom, &cel_quad,
                         map_x + (map_y << 8)); // Draw map objects
    }
...

{
  ...
  for (y_draw = -16; y_draw <= 15; y_draw++) {
    ...
    for (x_draw = -16; x_draw <= 15; x_draw++) {
      ...
      cel_quad.y_pos0 =
          MAPBASEY + y_screen -
          ((height_map[actual_mapypos][actual_mapxpos]) >> height_shift);

      cel_quad.x_pos1 = MAPBASEX + x_screen + 6;
      cel_quad.y_pos1 =
          MAPBASEY + y_screen -
          ((height_map[actual_mapypos]
                      [(actual_mapxpos + (1 << shift_zoom)) & 255]) >>
           height_shift);

      cel_quad.x_pos2 = MAPBASEX + x_screen + 6 + 3;
      cel_quad.y_pos2 =
          MAPBASEY + y_screen - 6 -
          ((height_map[(actual_mapypos + (1 << shift_zoom)) & 255]
                      [(actual_mapxpos + (1 << shift_zoom)) & 255]) >>
           height_shift);

      cel_quad.x_pos3 = MAPBASEX + x_screen + 3;
      cel_quad.y_pos3 = MAPBASEY + y_screen - 6 -
                        ((height_map[(actual_mapypos + (1 << shift_zoom)) & 255]
                                    [actual_mapxpos]) >>
                         height_shift);
      ...
      if (mission.scanner_bust == 0) {
        cel_quad.shade =
            8 + ((height_map[actual_mapypos][actual_mapxpos]) >> 3);
      }
  ...
  for (x_draw = -4; x_draw <= 3; x_draw++) {
    ...
    if (map_getoffsets(&actual_mapxpos, &y_screen) == 1) {
      ...
      arm_addgamecel(&cel_quad, BASECEL_MAP_NUMBERS + (x_draw & 7), 1024, 1024);
    }
  }

  // Draw letters on map side
  ...
  for (y_draw = -4; y_draw <= 3; y_draw++) {
    ...
    if (map_getoffsets(&x_screen, &actual_mapypos) == 1) {
      ...
      arm_addgamecel(&cel_quad, BASECEL_MAP_LETTERS + (y_draw & 7), 1024, 1024);
    }
  }
}
...

{
  ...
  long distance, item_xpos, item_ypos, item_type;
  ...
  if (mission.scanner_bust == 0) // Only look & display of scanner O.K
  {
    ...
    {
      item_type =
          (ship->type) >> 4; // Get actual type of ship (big ship, fighter etc.)
...

      {
        ...
        switch ((ship->type) >> 4) {
          ...
          case SMALL_SHIP:
            map_addshipitem2(ship, BASECEL_MAP_TYPE_FIGHTER +
                                       ship->special_data->control_mode);
            map_addshipitem1(ship, 1);
            break;
      ...
      if (item_type == SMALL_SHIP || item_type == BIG_SHIP ||
          item_type == CAR || item_type == PLAYERS_SHIP ||
          item_type == SATELLITE) {
        item_xpos = (((map_x + cross_x) & 255) << 24) -
                    ship->x_pos; // Get ship's xpos relative to cross
        item_ypos = (((map_y + cross_y) & 255) << 24) -
                    ship->y_pos; // Get ship's ypos relative to cross
        ...
        item_xpos -= ((item_ypos >> 1) + 3); // Adjust for angle de-la dangle
        item_ypos += 6;

        distance = (item_xpos * item_xpos) +
                   (item_ypos * item_ypos); // Get actual distance

        if (distance < 256) { // Is it within scan range ?
          if (distance < nearest_distance ||
              nearest_distance == -1) // Yes - nearer than last ?
          {
    ...
    nearest_found =
        (nearest_found_ship == NULL) ? TRACKING_NOTHING : TRACKING_SHIP;
    if (nearest_found_ship != NULL) {
      nearest_ship = nearest_found_ship;
      tracking_x = (nearest_ship->x_pos >> 24) & 255;
      tracking_y = (nearest_ship->y_pos >> 24) & 255;
    }
  }
...
void
map_addshipitem1(ship_stack *ship, long info_type)

// Purpose : Displays a single item (ship, missile etc.) on the screen, with
// direction, side (FRAME 1 INFO) Accepts : Pointer to ship & info type (0 is
// direction, 1 is kill/save status) Returns : Now't

{
  ...
  if (map_getoffsets(&item_xpos, &item_ypos) ==
      1) // Adjust co-ordinates for around players pos & zoom
  {
    ...
    if (info_type == 0) {
      arm_addgamecel(&cel_quad, BASECEL_DIRECTION + item_rotation, 1024,
                     1024);     // Display direction arrow
      if (ship != players_ship) // Do colour for side ?
      {
        item_side = ship->special_data->side; // Get side ship is on
        arm_setgamecelpalette(&cel_quad,
                              BASECEL_MAP_PALETTE + 1 +
                                  item_side); // Set palette for correct side
      }
    }
      ...
      switch (ship->special_data->killstatus) {
        ...
        case SDB_KILL_MUSTSURVIVEFLIGHT:
          arm_addgamecel(&cel_quad, BASECEL_MAP_STATUS, 1024,
                         1024); // Display save icon ?
          break;

        case SDB_KILL_MUSTDIE:
        case SDB_KILL_MUSTSTOPFLIGHT:
          arm_addgamecel(&cel_quad, BASECEL_MAP_STATUS + 1, 1024,
                         1024); // Display kill icon ?
          break;
      }
      ...
      if (status.track_status == TRACKING_STATUS_LOCKED &&
          status.track_type == TRACKING_SHIP && status.ship_tracking == ship)

      {
        cel_quad.x_pos0 -= 3;
        cel_quad.y_pos0 -= 3;
        arm_addgamecel(&cel_quad, BASECEL_MAP_QUESTION, 1024,
                       1024); // Display status icon
      }
    }
  }
}
...
void
map_addshipitem2(ship_stack *ship, long item_type_sprite)

// Purpose : Displays a single item (ship, missile etc.) on the screen
...

{
  ...
  {
    cel_quad.shade = 11;
    cel_quad.x_pos0 = item_xpos - 7;
    cel_quad.y_pos0 = item_ypos - 7;
    arm_addgamecel(&cel_quad, item_type_sprite, 1024,
                   1024); // Display type sprite
  }
}
...
void
map_drawtargets(long map_toggle)
...

{
  ...
  for (object_loop = 0; object_loop < mission.groundcount; object_loop++) {
    ...
    // Get item, check for ground destroy object. Get it's X & Y position
    ...
    if (mission.groundlist[object_loop].type == GROUND_DESTROY ||
      ...
      item_rememberxpos = item_xpos;
      item_rememberypos = item_ypos;

      // Adjust & draw target icon on map (if on screen) - Also draw locked
      // cross if locked onto & calc new temp lock

      if (map_getoffsets(&item_xpos, &item_ypos) == 1)

      {
        item_ypos -= ((height_map[item_gridypos][item_gridxpos]) >> map_zoom);
        cel_quad.x_pos0 = item_xpos - 4;
        cel_quad.y_pos0 = item_ypos - 5;

        // Don't print anything when scanner nackered or toggled off

        if (!(map_toggle != 0 || mission.scanner_bust == 1)) {
          arm_addgamecel(
              &cel_quad,
              (mission.groundlist[object_loop].type == GROUND_DESTROY)
                  ? BASECEL_MAP_TARGET
                  : BASECEL_MAP_CHEAT,
              1024, 1024); // Display direction arrow
        }

        // Draw target lock icon ?
        ...
        if ((!(map_toggle != 0)) &&
          ...
          cel_quad.y_pos0 -= 7;
          arm_addgamecel(&cel_quad, BASECEL_MAP_QUESTION, 1024,
                         1024); // Display lock icon
        }

        // Work out what nearest distance check should be (ie. temp tracking
        // ship / nothing)

        item_xpos = (((map_x + cross_x) & 255) << 24) -
                    item_rememberxpos; // Get object's xpos relative to cross
        item_ypos = (((map_y + cross_y) & 255) << 24) -
                    item_rememberypos; // Get object's ypos relative to cross
        ...
        item_ypos += (height_map[item_gridypos][item_gridxpos]) >>
                     map_zoom; // Adjust for height offset
        ...
        if (distance < 256) { // Is it within scan range ?
              ...
              nearest_distance == -1) // Yes - nearer than last
          {
            ...
            tracking_x = item_gridxpos;
            tracking_y = item_gridypos;
          }
        }
      }
    }
...
long
map_getoffsets(long *item_xpos, long *item_ypos)
...

{
  ...
  *item_ypos = *item_ypos << (4 - map_zoom);
  ...
  if (*item_xpos >= -128 && *item_ypos >= -128 && *item_xpos <= 128 &&
      *item_ypos <= 128) {
    *item_xpos -= (*item_ypos >> 1);
    *item_xpos = (MAPBASEX + (*item_xpos >> 1) + (*item_xpos >> 2)) - 3;
    *item_ypos = (MAPBASEY + (*item_ypos >> 1) + (*item_ypos >> 2)) + 6;
    return (1);
  }
...

{
  ...
  control_showmission(0, -1); // Setup mission brief

  do {
    screen_base(S_WAIT);                      // Wait to stop flicker
    cel_quad.temp_cels = 0;                   // Reset plotting list
    map_backdrop();                           // Do backdrop
    brief_status = control_showmission(2, 0); // Do brief
    screen_update(S_CEL | S_MENU | S_BANK |
                  S_WAITFRAME); // Render screen, flip banks with wait
  } while (brief_status == BRIEF_CONTINUE); // Loop until quit / finish

  menu_enter(NO_MENU, 1); // Clear menu

  if (configuration.sound_on == 1) {
    music_terminate(); // Re-Initialise music player ?
    music_initialise(0);
  }
...
void
map_showstatus(ship_stack *ship)
...

{
  ...
  map_performance player_items[12] =
      ...
      {{BASECEL_MAP_ENGINE, BASECEL_MAP_ENGINE + 1, 0, 0, 7},
       ...
       {BASECEL_WEAPON + 0, BASECEL_MAP_LASERBAR, 0, 3, 7},
       {BASECEL_WEAPON + 1, -1, -2, 4, 2},
       {BASECEL_WEAPON + 2, -1, -2, 5, 2},
       {BASECEL_WEAPON + 3, -1, -2, 6, 2},
       {BASECEL_WEAPON + 4, -1, 7, 4, 2},
       {BASECEL_WEAPON + 5, -1, 7, 5, 2},
       {BASECEL_WEAPON + 6, -1, 7, 6, 2},
       {BASECEL_WEAPON + 7, -1, -2, 7, 2},
       {BASECEL_LIVES, -1, 7, 7, 2}};
  ...
  cel_rectangle(25, 36, 319 - 25, 12,
                (ship->special_data->side == SDB_SIDE_ENEMY) ? RED_PAL
                                                             : GREEN_PAL,
  ...
  for (item_loop = 0; item_loop < item_maxloop; item_loop++) {
    ...
    data_pointer = (ship == players_ship) ? &player_items[item_loop]
                                          : &other_items[item_loop];
    ...
    if (data_pointer->bar != -1) {
      ...
      cel_quad.shade = 10;
      for (item_valueloop = 0; item_valueloop <= data_pointer->x_length;
           item_valueloop++) {
    ...
    cel_rectangle(info_x + 160 - 2, info_y + 18 + 120, info_x + 18 + 160,
                  info_y - 2 + 120, GREY_PAL, REC_BORDER | REC_BOX, 1);
    cel_rectangle(info_x + icon_xgap + 160, info_y + 18 + 120,
                  info_x + icon_xgap + ((1 + data_pointer->x_length) * 16) +
                      160,
                  info_y - 2 + 120, BLUE_PAL, REC_BORDER | REC_BOX, 1);
    ...
    if (data_pointer->bar != -1) {
      ...
      for (item_valueloop = 0; item_valueloop < icon_perfvalue;
           item_valueloop++) {
        cel_quad.shade = 11;
  ...
  if (ship == players_ship) {
    ...
    sprintf(text_string, "%ld",
            ship->performance->weapons[WEAPON_MULTI_MISSILE]);
...

  {
    ...
    sprintf(text_string, "%ld Metres",
            ((ship->z_pos - players_ship->z_pos) >> 17) / 25);
...
void
map_statustext(map_performance *data_pointer, char *string, long colour)
...

{
  ...
  font_paint(0, colour, 160 + info_x + icon_xgap + 8, 122 + info_y, MENU_FONT,
             SCR_UPDATE, string);
}
...
char *
map_return_ship_title(ship_stack *ship, long returnside)
...
#define ship_returnside                                                        \
  (returnside == 0) ? no_string                                                \
                    : ((ship->special_data->side != SDB_SIDE_ENEMY)            \
                           ? no_string                                         \
                           : message_decode(MTXT__ENEMY, 0))
  ...
  switch ((ship->type) >> 4) {
    ...
    case BIG_SHIP:
      if (ship_id != 1) {
        sprintf(ship_titlestring, "%s %s", ship_returnside,
                message_decode(MTXT__STRATOLINER, 0));
      } else {
        sprintf(ship_titlestring, "%s %s", ship_returnside,
                message_decode(MTXT__MOTHERSHIP, 0));
      }
      break;

    case SMALL_SHIP:
      sprintf(ship_titlestring, "%s %s", ship_returnside,
              message_decode(MTXT__FIGHTER, 0));
      break;

    case SATELLITE:
      sprintf(ship_titlestring, "%s %s", ship_returnside,
              message_decode(MTXT__SATELLITE, 0));
      break;

    case CAR:
      if (ship_id == 0) {
        sprintf(ship_titlestring, "%s %s", ship_returnside,
                message_decode(MTXT__VEHICLE, 0));
      } else {
        sprintf(ship_titlestring, "%s %s", ship_returnside,
                message_decode(MTXT__TANK, 0));
      }
      break;
  }
  ...
  if (ship->special_data->side != SDB_SIDE_ENEMY && returnside == 1) {
    if (ship->special_data->pilot_name != 0) {
      sprintf(ship_endstring, "%s - %s", ship_titlestring,
              pilot_names[ship->special_data->pilot_name]);
    }
  }
...
char *
map_return_ship_mode(ship_stack *ship)
  ...
  switch (ship->special_data->control_mode) {
    case SDB_MODE_ATTACK:
      if (ship->special_data->target_type == SDB_TARGETTYPE_MULTIPLE) {
        sprintf(ship_endstring, "%s %s", message_decode(MTXT__ATTACKING, 0),
                message_decode(MTXT__MULTIPLE_TARGETS, 0));
      } else {
        sprintf(ship_endstring, "%s %s", message_decode(MTXT__ATTACKING, 0),
                map_return_ship_title(
                    (ship_stack *) ship->special_data->command_address, 0));
      }
      break;
    ...
    case SDB_MODE_FORMATION:
      sprintf(ship_endstring, "%s %s", message_decode(MTXT__FORMATION_WITH, 0),
              map_return_ship_title(
                  (ship_stack *) ship->special_data->command_address, 0));
      break;
  }
...
char *
map_return_ship_killstatus(ship_stack *ship)
