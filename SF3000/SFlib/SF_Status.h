...
#define TRACKING_STATUS_FREE                                                   \
  0 // Status of tracking (free or locked onto 1 object ?)
...
typedef struct game_status {
  ...
  char flag_waitingtodock; // Are we waiting to dock (ie. mission completed
                           // otherwise ?)
...
void status_register(
    long, long,
    long); // Register a new game status + counter start value + count dec value
void status_updatestatus(
    void); // (Called If status counter>0), update current game status
void status_groundhit(long, long); // Check a ground object destroyed
void status_awardmissionbonus(long, long, long, long, long,
                              long); // Awards bonus from ground object or ship
