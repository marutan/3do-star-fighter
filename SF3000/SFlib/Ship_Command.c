...
small_ship_command_update(ship_stack *ship)
{
  ...
  switch (ship->command) {
    ...
    case COMMAND_TAXI_TURN:
      ...
      ship->x_rot =
          ((ship->x_rot + ((256 / COMMAND_TAXI_TURN_COUNTER) * 1024)) &
           ROT_LIMIT);
...
void
small_ship_damaged(ship_stack *ship)
{
...
void
small_fighter_damage_wobble(ship_stack *damaged_ship, long how_bad)
{
  ...
  add_smoke((damaged_ship->x_pos) + (((arm_random() & 255) - 128) << 13) +
                damaged_ship->x_vel,
            (damaged_ship->y_pos) + (((arm_random() & 255) - 128) << 13) +
                damaged_ship->y_vel,
            (damaged_ship->z_pos) + (((arm_random() & 255) - 128) << 13) +
                damaged_ship->z_vel,
            damaged_ship->x_vel, damaged_ship->y_vel, damaged_ship->z_vel,
            temp_long, 0);
}
