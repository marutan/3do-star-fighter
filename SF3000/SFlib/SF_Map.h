...
void map_addshipitem1(
    ship_stack *,
    long); // Display individual ship on map [DIRECTION / MISSION KILL STATUS]
void map_addshipitem2(ship_stack *,
                      long); // Display individual ship on map [TYPE]

long map_getoffsets(long *,
                    long *); // Centres points around player, scales to zoom
// void	planet_map_drawsmall (void);					// Draw
// small map on top right of screen
