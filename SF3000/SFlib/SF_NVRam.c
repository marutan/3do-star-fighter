...
long
nvram_load(char *filename, char *gameprefix, char *addressofloadfile,
           long sizeofloadfile)

// Purpose : Loads a file from NVRam
// Accepts : Pointer to filename & Pointer to prefix to add to filename, load
// address, length of file to load Returns : (1) If loaded succesfully, (0) If
// not
...
long
nvram_save(char *filename, char *gameprefix, char *addressofsavefile,
           long sizeofsavefile)

// Purpose : Saves a file to NVRam AFTER ADDING 'GAMEPREFIX' TO IT
// Accepts : Pointer to filename, Pointer to prefix to add, Pointer to start of
// data, Length of data Returns : (1) If saved succesfully, (0) If not (Not
// enough memory)

{
  ...
  {
    menu_usermessage(USER_NVRAMFULL);
  ...
  memset(&fileInfo, 0, sizeof(IOInfo));
  fileInfo.ioi_Command = CMD_WRITE;                 // Set command to WRITE file
  fileInfo.ioi_Send.iob_Buffer = addressofsavefile; // Set save address
  fileInfo.ioi_Send.iob_Len =
      (int) ((sizeofsavefile + 3) & ~3); // Set file length (word aligned)
  DoIO(ioReqItem, &fileInfo);
...
long
nvram_delete(char *filename, char *gameprefix)
...

{
  ...
  sprintf(full_filename, "/NVRAM/%s%s", gameprefix,
          filename); // Generate full NVRAM filename
...
long
nvram_directory(long menu_number, long menu_entry, long menu_maxentry,
                long set_itemok, char *gameprefix)

// Purpose : Reads directory of NVRAM  files into menu entries - Only those that
// start with 'gameprefix' Accepts : menu number, menu entry to start placing
// filenames in, entry to stop and default selectable status of entries Returns
...

{
  ...
  // Read all files from directory, only register those starting with
  // 'gameprefix'

  while (ReadDirectory(directory, &filename) >= 0 &&
         menu_entry <= menu_maxentry) {
    if (strncmp(filename.de_FileName, gameprefix, strlen(gameprefix)) == 0) {
      file_occurence += 1;
      strcpy(message_decode(menus[menu_number].items[menu_entry].string[0], 1),
             &filename.de_FileName[0] + strlen(gameprefix));
