...
void
status_register(long status_type, long status_set, long status_dec)
...
{
  // Filter out any 'not' allowed changes, like player dying when mission is
  // completed etc.

  if (status.status == STATUS_MISSIONCOMPLETEDWAIT &&
      status_type != STATUS_MISSIONCOMPLETED) {
    return;
  }
...

{
  ...
  if (status.chain_timer > 0) {
    if (--status.chain_timer == 0) {
      static_explode(
          (mission.groundlist[status.chain_next].xpos >> 1) +
              ((mission.groundlist[status.chain_next].ypos >> 1) << 7),
          256);
    }
  }
  ...
  if (status.clock2 == 0) {
    ...
    if (mission.mission_timer >
        0) // Has mission timer expired (and was previously set)
    {
      if (--mission.mission_timer <= 0) {
        message_add(MESSAGE_TOP, (MTXT__YOU_HAVE_RUN_OUT_OF_TIME), WHITE_15,
                    MESSAGE_FLASH, 32, MESSAGE_NOCURSOR);
        control_gameover();
      }
      message_addtimer();
    }
    ...
    if (status.mega_ship >
        0) { // Putting a ship back to normal after megasetup ?
  ...
  if (((arm_random() & 2047) == 2047) && planet_info.space_mission == 0) {
    message_add(MESSAGE_TOP, (MTXT__PARACHUTE_DROP_DETECTED), WHITE_15,
                MESSAGE_FLASH, 48, MESSAGE_NOCURSOR);
  ...
  switch (status.status) {
    ...
    case STATUS_ZOOMIN: // INITIAL ZOOM IN OF CAMERA ?
      ...
      } else if (status.current_camera == 0) {
    ...
    case STATUS_CONTINUE: // DO GENERAL IN-GAME CHECKS & UPDATES
      ...
      if (mission.defence_timer > 0) {
        if (--mission.defence_timer <= 0) {
          message_add(MESSAGE_BOTTOM, (MTXT__DEFENCES_ACTIVE), WHITE_15,
                      MESSAGE_FLASH, 32, MESSAGE_NOCURSOR);
        }
      }

      // IS 'WAITING TO DOCK' FLAG SET - IF SO, CHECK FOR DOCKED, IF SO, MISSION
      // IS FINISHED
      if (status.flag_waitingtodock == 1) {
        if (status.docked != NULL) {
          message_add(MESSAGE_BIG, (MTXT__DOCKING_SUCCESSFUL), WHITE_15,
                      MESSAGE_SCROLL, 64, MESSAGE_CURSOR2);
          message_add(MESSAGE_BOTTOM, (MTXT__MISSION_COMPLETED), WHITE_15,
      ...
      if (status.flag_allgrounddead == 0 && status.flag_allshipdead == 0 &&
        ...
        if (status.flag_waitingtodock == 0) {
          message_add(MESSAGE_BIG, (MTXT__MISSION_COMPLETED), WHITE_15,
                      MESSAGE_FLASH, 64, MESSAGE_NOCURSOR);
          if (mission.dock_to_finish == 1) {
            if (status.docked == NULL) {
              message_add(MESSAGE_BOTTOM, (MTXT__RETURN_TO_MOTHERSHIP),
                          WHITE_15, MESSAGE_FLASH, 64, MESSAGE_NOCURSOR);
            }
    ...
    case STATUS_CRASHED: // HAS PLAYER CRASHED ?
      if (status.status_count < 0) {
        ...
        armlink_releaseitem(players_ship,
                            &ships); // Release the ship for update !
        players_ship->shields = players_ship->performance->shields
                                << 5; // Reset Shields
        ...
        players_ship->z_pos =
            player_crashed_zpos + (2 << 24); // Move it up a bit

        ship = (ship_stack *) (ships.info)
                   .start_address; // Kill all non players A_T_A Missiles
        while ((ship->header).status == 1) {
          if (ship->type == AIR_TO_AIR &&
              (ship_stack *) ship->who_owns_me != players_ship) {
            ship->shields = 0;
          }
...
void
status_groundhit(long xpos, long ypos)

// Purpose : Called when a ground object has been destroyed, decided what to do
// / update Accepts : X & Y map positions Returns : Nothing

{

  long object_loop, object_type;
  ...
  for (object_loop = 0; object_loop < mission.groundcount; object_loop++) {
    ...
    if (object_type != GROUND_NULL) {
      ...
      if (mission.groundlist[object_loop].xpos == xpos &&
          mission.groundlist[object_loop].ypos == ypos) {
        mission.groundlist[object_loop].type = GROUND_NULL;
        status_awardmissionbonus(object_type,
                                 mission.groundlist[object_loop].var,
                                 object_loop, 0, xpos, ypos);
      }
    }
  }
}
...
void
status_awardmissionbonus(long object_type, long object_var, long object_loop,
                         long check_type, long gridx, long gridy)

// Purpose : Decodes and awards mission bonuses
// Accepts : Bonus type, Bonus value, whether it was from a ship / vehicle (1)
// OR ground object (0), xpos+ypos Returns : Nothing

{
  ...
  long random_awards[8][3] = {{WEAPON_LASER, MTXT__LASER, 1},
  ...
  switch (object_type) {
    ...
    case GROUND_SWEAPONBONUS1:
      ...
      message_add(MESSAGE_BIG, (MTXT__MEGABOMBS_AWARDED), YELLOW_15,
                  MESSAGE_SCROLL, 32, MESSAGE_CURSOR2);
      break;

    case GROUND_SWEAPONBONUS2:
      player_performance.weapons[WEAPON_BEAM_LASER] += object_var;
      message_add(MESSAGE_BIG, (MTXT__BEAM_LASERS_AWARDED), YELLOW_15,
                  MESSAGE_SCROLL, 32, MESSAGE_CURSOR2);
      break;

    case GROUND_SWEAPONBONUS3:
      player_performance.weapons[WEAPON_MULTI_MISSILE] += object_var;
      message_add(MESSAGE_BIG, (MTXT__MULTIMISSILE_AWARDED), YELLOW_15,
                  MESSAGE_SCROLL, 32, MESSAGE_CURSOR2);
      break;

    case GROUND_SWEAPONBONUS4:
      player_performance.weapons[WEAPON_MINE] += object_var;
      message_add(MESSAGE_BIG, (MTXT__MINES_AWARDED), YELLOW_15, MESSAGE_SCROLL,
                  32, MESSAGE_CURSOR2);
      break;
    ...
    case GROUND_ADDTODEFENCETIME:
      if (object_var == 255) {
        ...
        message_add(MESSAGE_BIG, (MTXT__ENEMY_DEFENCES_DOWN), CYAN_15,
                    MESSAGE_SCROLL, 32, MESSAGE_CURSOR2);
      }
      break;
  ...
  if (check_type == 0) {
    switch (object_type) {
      ...
      case GROUND_DESTROY:
        message_add(MESSAGE_TOP, (MTXT__MISSION_TARGET_DESTROYED), YELLOW_15,
                    MESSAGE_SCROLL, 32, MESSAGE_CURSOR2);
        status.flag_allgrounddead -= 1;
        break;

      case GROUND_HIDDENDESTROY:
        message_add(MESSAGE_TOP, (MTXT__SECRET_MISSION_TARGET_DESTROYED),
                    WHITE_15, MESSAGE_SCROLL, 32, MESSAGE_CURSOR2);
        status.flag_allgrounddead -= 1;
        break;
      ...
      case GROUND_BLOWUPPREVIOUS:
        if (object_loop > 0) {
          status.chain_next =
              object_loop - 1; // Set next explosion one to be next in loop
          status.chain_timer = object_var
                               << 1; // Frame timer before next destroyed
        }
        break;
      ...
      case GROUND_OBJECTJAMSCANNER:
        if (mission.scanner_bust == 1) {
          ...
          message_add(MESSAGE_TOP, (MTXT__SCANNER_FREE), WHITE_15,
                      MESSAGE_SCROLL, 32, MESSAGE_CURSOR2);
        }
        break;
      ...
      case GROUND_RANDOMAWARD:
        if ((gridpos & 1) == 1) {
          bonus_weapon(random_awards[gridpos & 7][0],
                       random_awards[gridpos & 7][1],
                       random_awards[gridpos & 7][2]);
        } else {
      ...
      case GROUND_FEDNETHIT:
        if (mission.hits_allowed > 0) {
          message_add(MESSAGE_TOP, (MTXT__STRATEGIC_TARGET_DESTROYED),
                      YELLOW_15, MESSAGE_SCROLL, 32, MESSAGE_CURSOR2);
          mission.hits_allowed -= 1;
        } else {
          message_add(MESSAGE_TOP, (MTXT__STRATEGIC_COMMAND_OVERRUN), RED_15,
                      MESSAGE_SCROLL, 32, MESSAGE_CURSOR2);
