...

{
  ...
  // This list contains 0 - 16383 poly_map grid refs - negative terminates the
  // list
  ...
  while (*found >= 0) {
    ...
    if (type <= 3 && (arm_random() & 1023) < ground_laser_rate &&
        ((*found) & 1) == laser_counter) {
      ...
      if (laser_header->counter > 0) {
        laser_data += (arm_random() & 1);
      }

      aim_laser(x_pos - laser_data->x_pos, y_pos - laser_data->y_pos,
                z_pos + laser_data->z_pos, players_ship->x_pos,
                players_ship->y_pos, players_ship->z_pos, 0, 0,
                0, // Gun base velocity fixed
                players_ship->x_vel, players_ship->y_vel, players_ship->z_vel,
    ...
    if (type >= 4 && type <= 6 && (arm_random() & 1023) < ground_sam_rate &&
        missile_counter == 0) {

      FIND_OBJECT_POSITION

      missile_header =
          (missile_details_header *) (details + type)->missile_data;
      missile_data = (missile_details *) (missile_header + 1);

      // If it's a 2 missile launch site then arm_randomomly pick one
      if (missile_header->counter > 0) {
        missile_data += (arm_random() & 1);
      }

      add_sam(x_pos - missile_data->x_pos, y_pos - missile_data->y_pos,
              z_pos + missile_data->z_pos, players_ship, (long) *found);

      // Reset the counter like
      missile_counter = 8;

      // Display warning
      message_add(MESSAGE_TOP, MTXT__WARNING_INCOMING_SAM, RED_15,
                  MESSAGE_FLASH, 24, MESSAGE_NOCURSOR);
    }

    // Launch a ship or wot
    if (type >= 7 && (arm_random() & 1023) < ground_ship_rate &&
        hangar_counter == 0 && collision_map[y_grid][(x_grid + 1) & 127] > 0) {
      ...
      if (ship_data->type == SHIP_RUNWAY_TAKEOFF) {
        ...
        pleb_ship =
            add_ship(x_pos - ship_data->x_pos, y_pos - ship_data->y_pos,
                     z_pos + ship_data->z_pos, (512 * 1024),
                     (SMALL_SHIP << 4) + ground_ship_type, &pleb_special_data,
      ...
      // Are there any ships launched from a aerial hangar or an aircraft
      // carrier
      if (ship_data->type == SHIP_SOUTH_LAUNCH) {
        ...
        pleb_ship = add_ship(
            x_pos - ship_data->x_pos, y_pos - ship_data->y_pos,
            z_pos + ship_data->z_pos, 0, (SMALL_SHIP << 4) + ground_ship_type,
            &pleb_special_data, &mission.performances[ground_ship_type + 1]);
        if (pleb_ship != NULL) {
          pleb_ship->who_owns_me = (long) *found;
      ...
      // Display warning
      message_add(MESSAGE_TOP, MTXT__ENEMY_SHIP_LAUNCHED, RED_15, MESSAGE_FLASH,
                  24, MESSAGE_NOCURSOR);
    }
