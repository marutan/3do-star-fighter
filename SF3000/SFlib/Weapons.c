...
void
air_to_ground_control(ship_stack *missile)
{
}

void
add_air_to_ground(ship_stack *ship)
{
  ...
  if (atg != NULL) {
    ...
    memset((void *) (((char *) atg) + (sizeof(link_header))), 0,
    ...
    atg->x_rot =
        (ship->x_rot + (16 * ((arm_random() & 2047) - 1024))) & ROT_LIMIT;
    atg->y_rot =
        (ship->y_rot + (16 * ((arm_random() & 2047) - 1024))) & ROT_LIMIT;
...
void
air_to_air_control(ship_stack *ata)
{
  ...
  ata->x_vel =
      -(((sine_table[(ata->x_rot) >> 10] * cosine_table[(ata->y_rot) >> 10]) >>
         12) *
        (ata->speed));

  ata->y_vel = -(
      ((cosine_table[(ata->x_rot) >> 10] * cosine_table[(ata->y_rot) >> 10]) >>
       12) *
      (ata->speed));
  ...
  coll_check = check_collision(ata->x_pos, ata->y_pos, ata->z_pos);
  ...
  // Check for air hits
  if (ata->what_hit_me != 0) {
    missile_collision(ata);
  }

  add_smoke(ata->x_pos, ata->y_pos, ata->z_pos, ata->x_vel, ata->y_vel,
            ata->z_vel, MISSILE_SMOKE, 0);
}

void
add_air_to_air(ship_stack *ship, ship_stack *target)
{
  ...
  if (ata != NULL) {
    ...
    memset((void *) (((char *) ata) + (sizeof(link_header))), 0,
           (sizeof(ship_stack)) - (sizeof(link_header)));
...
void
add_sam(long x_pos, long y_pos, long z_pos, ship_stack *target, long object_ref)
{
  ...
  if (sam != NULL) {
    ...
    memset((void *) (((char *) sam) + (sizeof(link_header))), 0,
           (sizeof(ship_stack)) - (sizeof(link_header)));
...
void
mega_bomb_control(ship_stack *mega_bomb)

{
  ...
  if (which_graphics_set != SPACE_GRAPHICS) {
    ...
    // Nose down
    mega_bomb->y_rot =
        ((mega_bomb->y_rot) - cosine_table[(mega_bomb->y_rot) >> 10]) &
        ROT_LIMIT;
  }

  coll_check =
      check_collision(mega_bomb->x_pos, mega_bomb->y_pos, mega_bomb->z_pos);
  ...
  // Check for air hits
  if (mega_bomb->what_hit_me != 0) {
    missile_collision(mega_bomb);
  }
}

void
add_mega_bomb(ship_stack *ship)

{
  ...
  if (mega_bomb != NULL) {
    ...
    memset((void *) (((char *) mega_bomb) + (sizeof(link_header))), 0,
           (sizeof(ship_stack)) - (sizeof(link_header)));
...
mine_control(ship_stack *mine)
{
  ...
  coll_check = check_collision(mine->x_pos, mine->y_pos, mine->z_pos);
  ...
  // Check for air hits
  if (mine->what_hit_me != 0) {
    missile_collision(mine);
  }
}

void
add_mine(ship_stack *ship)
{
  ...
  if (mine != NULL) {
    ...
    memset((void *) (((char *) mine) + (sizeof(link_header))), 0,
           (sizeof(ship_stack)) - (sizeof(link_header)));
    ...
    // Any missiles targetting the ship that launched this mine now target the
    // mine
    temp_ship = (ships.info).start_address;
    while ((temp_ship->header).status == 1) {
      if ((temp_ship->type >> 4) == WEAPON &&
          temp_ship->target == (void *) ship) {
...
void
wing_pod_control(ship_stack *pod)
{
  ...
  if (ship->fire_request == WEAPON_LASER && pod->ref == (1 << pod_counter)) {
    ...
    add_laser((pod->x_pos), //+(temp_data[0]<<12) ,
              ...
              pod->x_vel, pod->y_vel, pod->z_vel, pod->x_rot, pod->y_rot,
  ...
  if (ship->thrust_control != 0) {
    add_smoke((pod->x_pos), //+(temp_data[0]<<11) ,
              ...
              pod->x_vel, pod->y_vel, pod->z_vel, WING_POD_SMOKE, 0);
    ship->speed += 16;
    pod->goto_y += 32;
  }
  ...
  // Check for air hits
  if (pod->what_hit_me != 0) {
    missile_collision(pod);
  }
}

void
add_wing_pod(ship_stack *ship)
{
  ...
  if (pod_number >= 0) {
    ...
    if (pod != NULL) {
      ...
      memset((void *) (((char *) pod) + (sizeof(link_header))), 0,
             (sizeof(ship_stack)) - (sizeof(link_header)));
