		...
		for (ship_count=0; ship_count < mission.special_ships; ship_count++)
			...
			switch (ship_sdb_ptr->control_mode)
				...
				case SDB_MODE_FORMATION :															// Initialise ship in formation
					ship_sdb_ptr->command_address = (ship_sdb_ptr->init_ref == 255) ? players_ship : mission.ship_special [ship_sdb_ptr->init_ref].ship_address;	// Get address of ship it is attacking
					if (ship_sdb_ptr->side == SDB_SIDE_FRIEND && ship_sdb_ptr->init_ref == 255)		// FF Formation ?
...

{
	...
	if (ship->who_hit_me > 16384 && ship_sdb_ptr->fire_instruction != SDB_INCOMING_IGNORE)
		...
		if (ship_sdb_ptr->side == SDB_SIDE_FRIEND)				// Decide what to to, friend or enemy
...
ship_sdb	*firee_sdb_ptr = ship_firee->special_data;
	...
	if (ship_firee == players_ship)
		...
		if (++status.friendly_fire > __WAR_MFF && ((arm_random() & 3) == 0))
			...
			message_pilot (ship_sdb_ptr->pilot_name, (MTXT__REQUEST_TERMINATION_OF_LEADER), 0);
			}
		else
	...
	if (ship->type == MOTHER_SHIP)
		{
		if (ship->shields <= 64)
			message_pilot (ship_sdb_ptr->pilot_name, (MTXT__WARNING_MOTHERSHIP_SHIELDS_LOW), 0);
		else
			message_pilot (ship_sdb_ptr->pilot_name, (MTXT__WARNING_MOTHERSHIP_UNDER_ATTACK), 0);
		}
	
	// SHOULD THE SHIP ISSUE A MAYDAY ?
	
	if (ship->shields <=32)
			message_pilot (ship_sdb_ptr->pilot_name, (MTXT__MAYDAY_MAYDAY_REQUEST_ASSISTANCE), 0);
	...
	if (status.formation_mode == SDB_FORMATION_DEFENSIVE)
		...
		message_pilot (ship_sdb_ptr->pilot_name, (MTXT__UNDER_FIRE_BREAKING_FORMATION), 0);
		war_settoattack (ship, (ship_stack*) ship_firee);
		}
}
...
//				{
//				CHECK FIRST THAT THE FORMATION IS NOT IN 'DEFENSIVE' MODE
...
ship_sdb	*firee_sdb_ptr = ship_firee->special_data;
	...
	if (ship_following == players_ship)								// If leader was player (we are leaving formation)
		...
		if (status.formation_mode == SDB_FORMATION_DEFENSIVE)		// If in defensive mode, return 0 - we cannot leave
			return (0);
		...
		if (ship_sdb_ptr->formation_position != 0)
			...
			formation [ship_sdb_ptr->formation_position].occupied = NULL;
			ship_sdb_ptr->formation_position = 0;
			message_pilot (ship_sdb_ptr->pilot_name, (MTXT__GOING_IN_TO_DEFEND_FORMATION), 0);
			}
		}
...
ship_sdb	*formation_ship_sdb_ptr = formation_ship->special_data;
	...
	if (formation_position <__WAR_MF)														// Is pos to set < max allowed ?
		if (formation [formation_position].occupied == NULL)								// Yes, is formation pos EMPTY ?
			...
			formation_ship_sdb_ptr->distance_check = SDB_NO_DISTANCECHECK;					// Disable checking from now on
			
			// Display rejoin message (if print-message flag set)
			
			if (print_message == 1)
				message_pilot (formation_ship_sdb_ptr->pilot_name, (MTXT__REQUEST_TO_JOIN_FORMATION), 0);
