...
// passed performance datablock) - SUSPENDED IF DEMO MISSION
// If starting a mission docked, it sets status.docked to address of mothership,
// else sets it to NULL (For Hutch)
...
// ATTACK & CARRY ON 				IF NOT FOUND :
// IS CONTROL MODE 'NORMAL' - IF SO, REJOIN FORMATION ELSE
...
// FORMATION 					ARE WE ENEMY ? IF SO, BREAK
// FORMATION AND ATTACK PLAYER
//					}
...
// FRIENDLY) 				UPDATE IT USING updateship_attack()
// INSTEAD 				RETURN
//				}

//	RULE 3 : SET THE SHIP'S GOTO CO-ORDINATES TO THAT OF THE SHIP IT IS
// FOLLOWING (+OFFSETS) 	RULE 4 : HAS THE SHIP WE ARE FOLLOWING FIRED ?
// IF SO, THEN FIRE AS WELL - ONLY IF FOLLOWING PLAYER
...
// OVER' BECAUSE ENEMY REACHED FLIGHTPOINT 			IF
// SDB_KILL_MUSTSURVIVEFLIGHT	- SUB STATUS MUST-SURVIVE FLIGHT COUNTER, & SET
// STATUS TO DON'T CARE 		SEE IF SHIP SHOULD BE SET TO FROZEN
// (COMMAND OVERRIDE - CANNOT MOVE)
//		}
...
// LEAVE THE FORMATION & DISPLAY MESSAGE 				ATTACK
...
//	RULE 1 : IF COMMAND OVERRIDE IS SET TO BIG_SHIP_COMMAND_STATIC_FREE
//(STATIC UNTIL FIRED ON) FREE IT ! 	RULE 2 : IF THE SHIP IS BEING FIRED UPON
//BY ANOTHER ENEMY - IGNORE THE ATTACK 	RULE 3 : IF THE SHIP IS BEING FIRED UPON
// BY FRIENDLY (INCLUDING PLAYER) - ATTACK THE FRIENDLY SHIP
...
// IGNORE THE ATTACK 	RULE 2 : IF NOT RND(4), THEN IGNORE THE ATTACK AND CARRY
// ON 	RULE 3 : IF THE LEADER IS THE PLAYERS SHIP THEN
//				{
//				CHECK FIRST THAT THE FORMATION IS NOT IN
//'DEFENSIVE' MODE 				DISPLAY 'DEFENDING FORMATION'
//MESSAGE PRIOR TO ATTACKING 				CLEAR THE FORMATION POSITION
//				}
