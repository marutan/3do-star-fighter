...
ship_sdb player_special_data = {
    ...
    SDB_NO_DISTANCECHECK, // distance_check;				// Don't
                          // Care
    0,                    // pilot_name;					// Don't Care
    SDB_CLOAKING_OFF,     // cloaking_device;
    0,                    // start_direction;				// Don't Care
...
void
war_addmissionships(performance_data *players_performance)

// Purpose : Creates & initialises all enemy / friendly ships at the start of a
// mission
//			 Including the players ship (added first, linked to
//passed performance datablock) - SUSPENDED IF DEMO MISSION 			 If starting a
//mission docked, it sets status.docked to address of mothership, else sets it
//to NULL (For Hutch)
...

{
  ...
  // CLEAR ALL FORMATION POSITIONS, AND SET status.docked (Starting docked flag)
  // to NULL

  status.docked = NULL; // Set start status to undocked & clear formation
  for (ship_loop = 0; ship_loop < __WAR_MF; ship_loop++) {
    formation[ship_loop].occupied = NULL;
  }

  // RE-INITIALISE THE LINKED LIST AND ADD THE PLAYERS SHIP - NOTE : THE PLAYERS
  // SHIP PERFORMANCE IS NOT ADDED HERE :-
  ...
  player_special_data.ship_address =
      players_ship; // Make Players SDB point to players ship
  pleb_special_data.command_address =
      players_ship; // Make pleb SDB point to players ship
  ...
  if (mission.special_ships > 0) {
    for (ship_count = 0; ship_count < mission.special_ships; ship_count++) {
      ...
      ship_type = (unsigned) ((mission.ship_special[ship_count].ship_type) >>
                              4); // Type
      ship_instance = (unsigned) ((mission.ship_special[ship_count].ship_type) &
                                  15); // Instance of type
      ...
      switch (mission.ship_special[ship_count].killstatus) {
        ...
        case SDB_KILL_MUSTSTOPFLIGHT:
          status.flag_allshipdead +=
              1; // Inc number of special ships to destroy
          break;

        case SDB_KILL_MUSTSURVIVEFLIGHT:
          status.flag_defendland +=
              1; // Inc number of special ships to land alive
          break;
      }

      // DECIDE ON PERFORMANCE POINTER & SET SINGLE / MULTIPLE TARGET TYPES

      mission.ship_special[ship_count].target_type =
          SDB_TARGETTYPE_NORMAL; // Set default to single target

      switch (ship_type) {
        case SMALL_SHIP:
          performance =
              &mission
                   .performances[1 + ship_instance]; // Little ship performances
          break;

        case BIG_SHIP:
          mission.ship_special[ship_count].target_type =
              SDB_TARGETTYPE_MULTIPLE; // Set to multiple targets
          mission.ship_special[ship_count].fire_instruction =
              SDB_INCOMING_KEEPFLIGHTPATH;
          performance =
              &mission.performances[5 + ship_instance]; // Big ship performances
          break;
        ...
        case SATELLITE:
          mission.ship_special[ship_count].target_type =
              SDB_TARGETTYPE_MULTIPLE; // Set to multiple targets
          mission.ship_special[ship_count].fire_instruction =
              SDB_INCOMING_IGNORE;                // Ignore incoming
      ...
      ship = add_ship(
          (unsigned) (mission.ship_special[ship_count].x_start
                      << 24), // Xpos (Scale up to 32bit map ref)
          (unsigned) (mission.ship_special[ship_count].y_start << 24), // Ypos
          (unsigned) (mission.ship_special[ship_count].z_start << 24), // Zpos
          (unsigned) (mission.ship_special[ship_count].start_direction
                      << 17),                         // Xrot
          mission.ship_special[ship_count].ship_type, // Type
          &mission.ship_special[ship_count],          // SDB
          performance);                               // Performance

      // IF ADDED (ALWAYS SHOULD BE), INITIALISE SHIPS STATS, DEPENDING ON ITS
      ...
      if (ship != NULL) {
        ...
        if (mission.starting_docked == 1 &&
            ship->type == MOTHER_SHIP) { // Is this the mothership & we are
          status.docked = ship;          // Starting docked ? If so, register
        }

        if (performance != NULL) {
          ship_sdb_ptr->max_canlaunch =
              (char) performance
                  ->weapons[WEAPON_LAUNCH_SHIP]; // Set maxcanlaunch in SDB
        }
      }
    }
    ...
    for (ship_count = 0; ship_count < mission.special_ships; ship_count++) {
      ship_sdb_ptr =
          &mission.ship_special[ship_count]; // Get Pointer to ship's SDB
      ship = (ship_stack *)
                 ship_sdb_ptr->ship_address; // Get the ship it refers to
      ...
      switch (ship_sdb_ptr->control_mode) {
        case SDB_MODE_ATTACK: // Initialise ship in attack
          if (ship_sdb_ptr->init_ref != 255) {
            war_settoattack(
                ship,
                (ship_stack *) (mission.ship_special[ship_sdb_ptr->init_ref]
                                    .ship_address)); // Get address of ship it
                                                     // is attacking
          } else {
        ...
        case SDB_MODE_FORMATION: // Initialise ship in formation
          ship_sdb_ptr->command_address =
              (ship_sdb_ptr->init_ref == 255)
                  ? players_ship
                  : mission.ship_special[ship_sdb_ptr->init_ref]
                        .ship_address; // Get address of ship it is attacking
          if (ship_sdb_ptr->side == SDB_SIDE_FRIEND &&
              ship_sdb_ptr->init_ref == 255) { // FF Formation ?
            war_addshiptoformation(ship, 0);   // Set this ship to formation pos
          }
          break;

        case SDB_MODE_FLIGHTPATH: // Initialise ship in flightpath mode ? SET
                                  // INITIAL GO-TO POINTS
          ship_flight_ptr = &mission.flight_paths[(
              long) (ship_sdb_ptr->init_ref)]; // Get the flightpath Address
          ship_sdb_ptr->command_address =
              ship_flight_ptr;                       // Store it in com address
...
// Purpose : Updates all mission & pleb ships, Attack, Flightpath, Formation
// etc. Also does counts & cockpit display info Accepts : Nothing Returns :
// Nothing

{
  ...
  while ((ship->header).status == 1) // While active ship, search
  {
    ...
    if (ship_sdb_ptr == NULL) // Does ship NOT have a SDB ?
    {
      ...
      {
        if (ship->target ==
            players_ship) {      // Is it going for the players ship ?
...

    {
      ...
      if (ship_sdb_ptr->alive == SDB_SHIP_ALIVE) // Yes, is it alive ?
      {
        ...
        if (ship_sdb_ptr->sdb_type !=
            SDB_TYPE_PLAYER) // If NOT Player then update commands
        {
          war_updatemissionship(ship, ship_sdb_ptr);
        } else {
          if (ship->who_hit_me != -1 &&
              status.docked == NULL) { // Player been hit ? - NOT DOCKED
            status.hits_taken += 1;
          }
        }

        if (ship_sdb_ptr->target_type ==
            SDB_TARGETTYPE_MULTIPLE) { // Attack Multiple Targets ?
        ...
        if (ship_sdb_ptr->side == SDB_SIDE_FRIEND) {
          if (ship != players_ship) // UPDATE SHIELDS FOR FRIEND ?
          {
            if ((status.clock2 & 3) == 3) {
              if (ship->shields < (ship->performance->shields << 5) &&
                  ship->shields > 0) {
                ship->shields += 1;
              }
            }
            ...
            player_st =
                (status.docked == NULL) ? 7 : 1; // UPDATE SHIELDS FOR PLAYER ?
...
void
war_updatemissionship(ship_stack *ship, ship_sdb *ship_sdb_ptr)
...

{
  ...
  if (ship->who_hit_me > 16384 &&
      ship_sdb_ptr->fire_instruction != SDB_INCOMING_IGNORE) {

    if (ship_sdb_ptr->side ==
        SDB_SIDE_FRIEND) { // Decide what to to, friend or enemy
  ...
  switch (ship_sdb_ptr->control_mode) {
    ...
    case SDB_MODE_FORMATION: // Is the ship in formation flight with another
                             // ship ?
...
/************************************************************************************************************************************
 *												UPDATE
 *SHIP THAT IS IN ATTACK MODE
 **
 ************************************************************************************************************************************/

void
war_updateship_attack(ship_stack *ship, ship_sdb *ship_sdb_ptr)
...
//			 YES : (FRIENDLY)
//				{
//				DO LOCAL SEARCH FOR NEW TARGET, IF FOUND, SET TO
//ATTACK & CARRY ON 				IF NOT FOUND : 					IS CONTROL MODE 'NORMAL' - IF SO, REJOIN
//FORMATION 				ELSE 					DO WIDE SEARCH FOR NEW TARGET, IF FOUND, SET TO ATTACK & CARRY
//ON 					OTHERWISE : REJOIN FORMATION
//				}

//			YES : (ENEMY)
//				{
//				DO WIDE SEARCH FOR NEW TARGET, IF FOUND, SET TO
//ATTACK & CARRY ON 				ELSE : LEAVE SUSPENDED
//				}

//	RULE 2 : SET THE SHIP'S GOTO CO-ORDINATES TO THAT OF THE SHIP IT IS
//ATTACKING [HUTCH NOW DOES THIS]
...
//					DEC. TARGET COUNTER, IF <0, RESET AND
//RE-ALLOCATE TOP 3 TARGETS
//					}

{
  ...
  if ((((ship_stack *) (ship_sdb_ptr->command_address))->shields <=
       0) ||                                      // Shields <= 0 OR
...

  {
    ...
    {
      if (ship_setmultipletargets(ship, __WAR_SR_LOCAL, ship_sdb_ptr->side) !=
          0) { // Found local target ?
        war_settoattack(ship, (ship_stack *) ship->target); // YES - attack it
      } else // Couldn't find local ?
      {
        if (status.formation_mode ==
            SDB_FORMATION_NORMAL) // Normal mode ? If so, rejoin formtn
        {
          ...
          if (ship_setmultipletargets(ship, __WAR_SR_AREA,
                                      ship_sdb_ptr->side) !=
              0) { // Found area target ?
            war_settoattack(ship,
                            (ship_stack *) ship->target); // YES - attack it
    ...
    {
      if (ship_setmultipletargets(ship, __WAR_SR_AREA, ship_sdb_ptr->side) !=
          0) { // Found area target ?
        war_settoattack(ship, (ship_stack *) ship->target); // YES - attack it
      } else {
...
/************************************************************************************************************************************
 *										UPDATE
 *A SHIP THAT IS IN FORMATION WITH ANOTHER SHIP
 **
 ************************************************************************************************************************************/

void
war_updateship_formation(ship_stack *ship, ship_sdb *ship_sdb_ptr)
...
//					{
//					ARE WE FRIENDLY ? IF SO, ASK TO JOIN
//FORMATION 					ARE WE ENEMY ? IF SO, BREAK FORMATION AND ATTACK PLAYER
//					}

//	RULE 2 : HAS OUR FORMATION LEADER BEEN HIT BY ENEMY FIRE ? IF SO (AND IT
//ISN'T SET TO IGNORE INCOMING FIRE) THEN
//				{
//				SET TO ATTACK FIREE (DEFEND FORMATION IF
//FRIENDLY) 				UPDATE IT USING updateship_attack() INSTEAD 				RETURN
//				}

//	RULE 3 : SET THE SHIP'S GOTO CO-ORDINATES TO THAT OF THE SHIP IT IS
//FOLLOWING (+OFFSETS) 	RULE 4 : HAS THE SHIP WE ARE FOLLOWING FIRED ? IF SO,
//THEN FIRE AS WELL - ONLY IF FOLLOWING PLAYER

{

  ship_stack *ship_following =
      (ship_stack *) ship_sdb_ptr->command_address; // Get Ship we are following
  ...
  // MAKE OUR SHIP FIRE (IF IT WAS FOLLOWING THE PLAYER WHO FIRED & THE PLAYER
  // WASN'T DOCKED) or something like that anyway CAN ONLY FIRE EVERY OTHER
  // FRAME TO CUT DOWN ON LASERS IN STACK
...
/************************************************************************************************************************************
 *												UPDATE
 *SHIP THAT IS ON A FLIGHTPATH
 **
 ************************************************************************************************************************************/

void
war_updateship_flightpath(ship_stack *ship, ship_sdb *ship_sdb_ptr)
...
//			 IF THE SHIP IS CLOSE ENOUGH TO ITS CURRENT FLIGHT PATH
//POINT, MOVE IT ONTO THE NEXT FLIGHT PATH POINT

//	RULE 3 : IF THE SHIP HAS REACHED THE END OF IT'S FLIGHTPATH :
//		{
//		SET IT BACK TO 1ST POINT ON THE FLIGHTPATH
//			IF SDB_KILL_MUSTSTOPFLIGHT		- MARK 'GAME
//OVER' BECAUSE ENEMY REACHED FLIGHTPOINT 			IF SDB_KILL_MUSTSURVIVEFLIGHT	- SUB
//STATUS MUST-SURVIVE FLIGHT COUNTER, & SET STATUS TO DON'T CARE 		SEE IF SHIP
//SHOULD BE SET TO FROZEN (COMMAND OVERRIDE - CANNOT MOVE)
//		}

{
  ...
  check_limit = (((ship->type) >> 4) == CAR)
                    ? 4
                    : 4; // Set check limit for vehicles / ships
  ...
  if (dist > check_limit) {
    return; // Are we too far away in the X ?
  }
  ...
  if (dist > check_limit) {
    return; // Are we too far away in the Y ?
  }
  ...
  if (((ship->type) >> 4) != CAR) {
    ...
    if (dist > check_limit) {
      return; // Are we too far away in the Z ? ONLY CHECK FOR SHIPS
    }
  }

  // YES, GET NEXT FLIGHTPATH POINT, IF FINISHED FLIGHTPATH, DO END OF
  // FLIGHTPATH CODE

  ship_flight_ptr =
      (flight_path *) ship_sdb_ptr
          ->command_address; // Get the flightpath we are on from the SDB

  if (++(ship_sdb_ptr->flightpath_point) >=
      ship_flight_ptr
          ->flight_points) // Have we finished the flightpath ? - Do e-o-fp code
  {

    // HAS ENEMY REACHED END OF FLIGHTPATH WHEN IT SHOULDN'T HAVE ?

    if (ship_sdb_ptr->killstatus == SDB_KILL_MUSTSTOPFLIGHT) {
      message_add(MESSAGE_TOP, (MTXT__ENEMY_VESSEL_HAS_LANDED), RED_15,
                  MESSAGE_FLASH, 64, MESSAGE_NOCURSOR);
      control_gameover();
    }
    ...
    if (ship_sdb_ptr->killstatus == SDB_KILL_MUSTSURVIVEFLIGHT) {
      message_add(MESSAGE_TOP, (MTXT__FRIENDLY_VESSEL_HAS_LANDED), WHITE_15,
                  MESSAGE_FLASH, 64, MESSAGE_NOCURSOR);
...
/************************************************************************************************************************************
 *											UPDATE
 *FRIENDLY SHIP THAT HAS BEEN FIRED UPON
 **
 ************************************************************************************************************************************/

void
war_update_attacked_friend(ship_stack *ship, ship_sdb *ship_sdb_ptr)

// Purpose : Decides what to do if a friendly ship has been fired upon (Ship
// cannot be set to IGNORE ATTACK to get here) Accepts : Pointer to ship to
// update, Pointer to it's special data block Returns : Nothing

//	RULE 1 : IF COMMAND OVERRIDE IS SET TO BIG_SHIP_COMMAND_STATIC_FREE
//(STATIC UNTIL FIRED ON) FREE IT !
...
//					DISPLAY 'TERMINATING SQUADRON LEADER'
//MESSAGE
//					}
//				ELSE
//					DISPLAY 'DO NOT FIRE ON FRIENDLY SHIPS'
//MESSAGE
//				}

//	RULE 3 : IF THE SHIP IS BEING FIRED ON BY ANOTHER FRIENDLY, IGNORE THE
//ATTACK AND RETURN

//	RULE 4 : IF SHIP'S SHIELDS ARE LOW , DISPLAY 'MAYDAY MAY MESSAGE'

//	RULE 5 : IF IT'S THE MOTHERSHIP BEING FIRED UPON, DO MESSAGE + WARNING
//FOR LOW SHIELDS

//	RULE 6 : IF THE SHIP IS NOT IN FORMATION WITH PLAYER, RETURN

//	RULE 7 : IF FORMATION MODE IS 'DEFENSIVE' - DISPLAY 'UNDER FIRE' MESSAGE
//- DO NOT LEAVE FORMATION
//
//			ELSE
//				{
//				IF THE SHIP IS IN FORMATION WITH THE PLAYER,
//LEAVE THE FORMATION & DISPLAY MESSAGE 				ATTACK THE ENEMY SHIP (IF POSSIBLE)
//				}

{
  ...
  if (ship_firee == players_ship) {
    if (++status.friendly_fire > __WAR_MFF && ((arm_random() & 3) == 0)) {
      ...
      message_pilot(ship_sdb_ptr->pilot_name,
                    (MTXT__REQUEST_TERMINATION_OF_LEADER), 0);
    } else {
      message_pilot(-1, (MTXT__WARNING_FRIENDLY_FIRE), 0);
    }
  }
  ...
  if (ship->type == MOTHER_SHIP) {
    if (ship->shields <= 64) {
      message_pilot(ship_sdb_ptr->pilot_name,
                    (MTXT__WARNING_MOTHERSHIP_SHIELDS_LOW), 0);
    } else {
      message_pilot(ship_sdb_ptr->pilot_name,
                    (MTXT__WARNING_MOTHERSHIP_UNDER_ATTACK), 0);
    }
  }
  ...
  if (ship->shields <= 32) {
    message_pilot(ship_sdb_ptr->pilot_name,
                  (MTXT__MAYDAY_MAYDAY_REQUEST_ASSISTANCE), 0);
  }
  ...
  // NOW WE KNOW IT'S A FRIENDLY IN FORMATION BEING ATTACKED, LOOK AT THE
  // FORMATION CONTROL MODE TO DECIDE WHAT TO DO

  if (status.formation_mode == SDB_FORMATION_DEFENSIVE) {
    message_pilot(ship_sdb_ptr->pilot_name, (MTXT__FORMATION_UNDER_FIRE), 0);
  } else {
    formation[ship_sdb_ptr->formation_position].occupied = NULL;
    ship_sdb_ptr->formation_position = 0;
    message_pilot(ship_sdb_ptr->pilot_name,
                  (MTXT__UNDER_FIRE_BREAKING_FORMATION), 0);
    war_settoattack(ship, (ship_stack *) ship_firee);
  }
}
...
/************************************************************************************************************************************
 *											UPDATE
 *ENEMY SHIP THAT HAS BEEN FIRED UPON
 **
 ************************************************************************************************************************************/

void
war_update_attacked_enemy(ship_stack *ship, ship_sdb *ship_sdb_ptr)

// Purpose : Decides what to do if an enemy ship has been fired upon (Ship
// cannot be set to IGNORE ATTACK to get here) Accepts : Pointer to ship to
// update, Pointer to it's special data block Returns : Nothing

//	RULE 1 : IF COMMAND OVERRIDE IS SET TO BIG_SHIP_COMMAND_STATIC_FREE
//(STATIC UNTIL FIRED ON) FREE IT ! 	RULE 2 : IF THE SHIP IS BEING FIRED UPON BY
//ANOTHER ENEMY - IGNORE THE ATTACK 	RULE 3 : IF THE SHIP IS BEING FIRED UPON BY
//FRIENDLY (INCLUDING PLAYER) - ATTACK THE FRIENDLY SHIP
...
/************************************************************************************************************************************
 *					SEE IF A SHIP SHOULD GO IN TO DEFEND THE
 *FORMATION IF IT'S LEADER HAS BEEN FIRED UPON
 **
 ************************************************************************************************************************************/
...
// Accepts : Pointer to ship to update, Pointer to it's special data block &
// Pointer to the ship it is following Returns : (1) if ship is now attacking
// the firee, or (0) if it isn't

//	RULE 1 : IF THE LEADER IS BEING FIRED UPON BY SOMEONE OF THE SAME TYPE,
//IGNORE THE ATTACK 	RULE 2 : IF NOT RND(4), THEN IGNORE THE ATTACK AND CARRY ON
//	RULE 3 : IF THE LEADER IS THE PLAYERS SHIP THEN
//				{
//				CHECK FIRST THAT THE FORMATION IS NOT IN
//'DEFENSIVE' MODE 				DISPLAY 'DEFENDING FORMATION' MESSAGE PRIOR TO ATTACKING
...

{
  ...
  if (firee_sdb_ptr->side == ship_following->special_data
                                 ->side) { // If ship is on same side, no attack
    return (0);
  }
  ...
  if (ship_following ==
      players_ship) // If leader was player (we are leaving formation)
  {

    if (status.formation_mode ==
        SDB_FORMATION_DEFENSIVE) { // If in defensive mode, return 0 - we cannot
                                   // leave
      return (0);
    }
    ...
    if (ship_sdb_ptr->formation_position != 0) {
      ...
      ship_sdb_ptr->formation_position = 0;
      message_pilot(ship_sdb_ptr->pilot_name,
                    (MTXT__GOING_IN_TO_DEFEND_FORMATION), 0);
    }
  }

  return (war_settoattack(
      ship, (ship_stack *) ship_firee)); // Set ship to attack and return status
}

/************************************************************************************************************************************
 *												SET
 *A SHIP TO ATTACK ANOTHER SHIP
 **
 ************************************************************************************************************************************/
...
// [IF ATTACKING PLAYER - DO RADAR LOCK MESSAGE] Accepts : Pointer to the ship
// to set to attack, and the ship to be attacked Returns : (1) if ship is now
// attacking the one that hit it, or (0) if it is not

//	NOTE :	 THIS ASSUMES THAT THE ATTACK FLAG HAS ALREADY BEEN CHECKED FOR
//'ATTACK_IGNORE'

//	RULE 1 : DO NOT ATTACK THE SHIP IF OUR SHIP'S ATTACK FLAG IS SET TO
//'ATTACK_KEEPFLIGHTPATH' AND IT IS ON A FLIGHTPATH

{
  ship_sdb *this_ship_sdb_ptr = this_ship->special_data;

  if (!(this_ship_sdb_ptr->control_mode == SDB_MODE_FLIGHTPATH &&
        this_ship_sdb_ptr->fire_instruction == SDB_INCOMING_KEEPFLIGHTPATH)) {
    ...
    if (ship_toattack == players_ship) {
      message_add(MESSAGE_BOTTOM, (MTXT__ENEMY_RADAR_LOCK_DETECTED), ORANGE_15,
                  MESSAGE_SCROLL, 64, MESSAGE_CURSOR2);
    }
...
void
ship_settoflightpoint(ship_stack *this_ship, ship_sdb *this_ship_sdb_ptr)

// Purpose : Sets a ships GOTOXYZ Co-ordinates from it's current point in it's
// flightpath Accepts : Pointer to the ship to set & pointer to it's special
// data block Returns : Nothing

{
  flight_path *this_ship_flight_ptr =
      (flight_path *) this_ship_sdb_ptr
          ->command_address; // Get the flightpath we are on from the SDB

  // Get new X,Y,Z position

  this_ship->goto_x =
      (unsigned) ((this_ship_flight_ptr
                       ->flight_data[this_ship_sdb_ptr->flightpath_point]
                       .x_pos)
                  << 24);
  this_ship->goto_y =
      (unsigned) ((this_ship_flight_ptr
  ...
  if (this_ship_flight_ptr->flight_data[this_ship_sdb_ptr->flightpath_point]
          .command > 0) {
    this_ship->special_data->bomb_todrop =
        this_ship_flight_ptr->flight_data[this_ship_sdb_ptr->flightpath_point]
            .command; // Set bombs
...
void
war_update_multipleattackrequests(ship_stack *ship, ship_sdb *ship_sdb_ptr)
...

{
  ...
  if ((ship_sdb_ptr->target_counter--) ==
      0) // Dec target counter - Realloc targets ?
  {
    ship_sdb_ptr->target_counter = __WAR_TC; // Reset target counter
    ship_setmultipletargets(ship, __WAR_SR_LOCAL,
                            ship_sdb_ptr->side); // Re-allocatate new targets
  }
  ...
  if (ship_sdb_ptr->side == SDB_SIDE_ENEMY) {
    if (weapon_firerequest <
        performance->launch_rate) // LAUNCH PLEB ? ONLY IF ENEMY !
    {
...
// Accepts : Pointer to the ship to set targets for, maxsearch start distance
// and the side of the ship Returns :(0) If No targets found, (1) if one or more
// targets located

{
  ...
  // printf("Doing multiple targets for ship %d, search dist is %d, side is
  ...
  ship_search = (ship_stack *) (ships.info)
                    .start_address;         // Get start of ship linked list
  while ((ship_search->header).status == 1) // While active ship, search
  {
    ...
    {
      if (search_ship_sdb_ptr->alive == SDB_SHIP_ALIVE &&
          search_ship_sdb_ptr->side != search_shipside) {
        xsearch_distance =
            ship_search->x_pos -
            this_ship_xpos; // If Alive & Opponent, consider it as a target
        if (xsearch_distance < 0) {
          xsearch_distance = -xsearch_distance;
        }
        if (xsearch_distance < maxsearch_distance) {
          ysearch_distance = ship_search->y_pos - this_ship_ypos;
          if (ysearch_distance < 0) {
            ysearch_distance = -ysearch_distance;
          }
          if (ysearch_distance < maxsearch_distance) {
            zsearch_distance = ship_search->z_pos - this_ship_zpos;
            if (zsearch_distance < 0) {
              zsearch_distance = -zsearch_distance;
            }
            if (zsearch_distance < maxsearch_distance) {

              // ALL 3 RELATIVE DISTANCES ARE WITHIN MAX SEARCH RANGE - SHOULD
              // IT BE A TARGET ?

              heighest_distance = (xsearch_distance >= ysearch_distance)
                                      ? xsearch_distance
                                      : ysearch_distance;
              if (zsearch_distance > heighest_distance) {
                heighest_distance =
                    zsearch_distance; // Get biggest of 3 distances
              }

              // CAN IT BE 3RD TARGET (OR LESS ?)

              if (heighest_distance <
                  target_distances[2]) // Is it < 3rd distance ?
              {
                target_insert = 2;
                ...
                if (heighest_distance <
                    target_distances[1]) // Is it < 2nd distance ?
                {
                  target_insert = 1;
                ...
                if (heighest_distance <
                    target_distances[0]) // Is it < 1st distance ?
                {
                  target_insert = 0;
                ...
                targets[target_insert] =
                    ship_search; // Register ship as relavent target
                maxsearch_distance =
                    target_distances[2]; // Move in max-search distance
              }
            }
          }
        }
    ...
    ship_search = (ship_stack *) (ship_search->header)
                      .next_address; // Get next ship in linked list
  }
  ...
  // this_ship->target2, this_ship->target3);
...

{
  ...
  if (formation_position < __WAR_MF) { // Is pos to set < max allowed ?
    if (formation[formation_position].occupied ==
        NULL) // Yes, is formation pos EMPTY ?
    {
      formation_ship_sdb_ptr->x_offset =
          formation[formation_position]
              .x_offset; // Yes, set offsets to that of form pos
      ...
      formation[formation_position].occupied =
          formation_ship; // Mark formation pos as occupied
      formation_ship_sdb_ptr->formation_position =
          formation_position; // Set SDB in ship as occupying pos

      formation_ship_sdb_ptr->command_address =
          players_ship; // Set to follow players ship
      formation_ship_sdb_ptr->control_mode =
          SDB_MODE_FORMATION; // Set control mode to FORMATION

      formation_ship_sdb_ptr->distance_check =
          SDB_NO_DISTANCECHECK; // Disable checking from now on

      // Display rejoin message (if print-message flag set)

      if (print_message == 1) {
        message_pilot(formation_ship_sdb_ptr->pilot_name,
                      (MTXT__REQUEST_TO_JOIN_FORMATION), 0);
      }
...

{
  ...
  for (formation_search = 1; formation_search < __WAR_MF; formation_search++) {
    if (war_setformationposition(formation_ship, formation_search,
                                 print_message) == 1) {
      return (1);
    }
  }
...

{
  ...
  for (ship_formationcount = 1; ship_formationcount < __WAR_MF;
       ship_formationcount++) // Scan through formations
  {
    ship_in_formation = formation[ship_formationcount].occupied;
    if (ship_in_formation != NULL) // Is there a ship in this formation pos ?
    {
      formation[ship_formationcount].occupied =
          NULL; // Yes - clear ref in formation block
      ship_in_formation->special_data->formation_position =
          0; // Yes - clear it's ref in sdb
      war_setformationposition(ship_in_formation, ship_formationplace++,
                               0); // Re-add it
    }
  }
}
...
void
war_distancecheckwithplayer(ship_stack *ship, ship_sdb *ship_sdb_ptr)

// Purpose : Checks distance of ship with player, if within range, attacks or
// requests to join formation (depending on side) Accepts : Pointer to ship to
// update, Pointer to it's special data block Returns : Nothing

{
  ...
  dist = dist < 0 ? -dist : dist;
  if (dist > distance_length) {
    return; // Are we too far away in the Z ?
  }

  ship_sdb_ptr->distance_check =
      SDB_NO_DISTANCECHECK; // Disable checking from now on
  ...
  if (ship_sdb_ptr->side ==
      SDB_SIDE_FRIEND) // Is the ship friendly ? If so, ask to join formation
  {
    war_addshiptoformation(ship, 1);
