...
// Returns : (1) if correct keys were pressed to terminate player, or (0) if
// movie just finished

// NOTE : MUSIC & SAMPLE PLAYERS ARE TERMINATED TO FREE RESOURCES FOR DURATION
// OF VIDEO

{
  ...
  status = PlayCPakStream(screen, filename,
                          (PlayCPakUserFn) video_internal_interrupt, NULL);
...
long
video_internal_interrupt(PlayerPtr ctx)

// Purpose : Called by Cinepak main loop - Checks joypad & returns a 0 (no
// event) or 1 (Causes player to stop) Accepts : Pointer to Cinepak player
