...
void
explode_next_frame(long grid_ref)
{
  ...
  if (poly_map[(grid_ref >> 7)][(grid_ref & 127)] == 0 ||
      poly_map[(grid_ref >> 7)][(grid_ref & 127)] >= 64) {
    return;
  }
...
{
  // This updates the list of graphics to explode and blows them up when counter
  // reachs zero
  explosion_stack *explosion;
...
explode_ship(ship_stack *ship)
{
  ...
  if ((ship->type) >> 4 != BONUS && ship->shields != PARACHUTE_NO_EXPLODE) {
    ...
    if (((ship->type) >> 4) == BIG_SHIP ||
        ...
        ((ship->type) >> 4) == SMALL_SHIP || ((ship->type) >> 4) == SATELLITE)

    {
      ...
      if (((arm_random()) & 1) == 0) {
        ...
        add_ship_explosion_section(ship,
                                   (GROUP_1 + GROUP_2 + GROUP_3 + GROUP_4 +
                                    GROUP_5), // Don't release these groups
                                   centre_group);
      }
      ...
      if (((arm_random()) & 1) == 0) {
        ...
        // Explode the remaining groups
        group_mask =
            (group_mask & (GROUP_0 + GROUP_2 + GROUP_3 + GROUP_4 + GROUP_5));

        centre_group = 1;
        add_ship_explosion_section(ship,
                                   (GROUP_0 + GROUP_2 + GROUP_3 + GROUP_4 +
                                    GROUP_5), // Don't release these groups
                                   centre_group);
      }
      ...
      if (((arm_random()) & 1) == 0) {
        ...
        // Explode the remaining groups
        group_mask =
            (group_mask & (GROUP_0 + GROUP_1 + GROUP_3 + GROUP_4 + GROUP_5));

        centre_group = 2;
        add_ship_explosion_section(ship,
                                   (GROUP_0 + GROUP_1 + GROUP_3 + GROUP_4 +
                                    GROUP_5), // Don't release these groups
                                   centre_group);
      }
      ...
      if (((arm_random()) & 1) == 0) {
        ...
        // Explode the remaining groups
        group_mask =
            (group_mask & (GROUP_0 + GROUP_1 + GROUP_2 + GROUP_4 + GROUP_5));

        centre_group = 3;
        add_ship_explosion_section(ship,
                                   (GROUP_0 + GROUP_1 + GROUP_2 + GROUP_4 +
                                    GROUP_5), // Don't release these groups
                                   centre_group);
      }
      ...
      if (((arm_random()) & 1) == 0) {
        ...
        // Explode the remaining groups
        group_mask =
            (group_mask & (GROUP_0 + GROUP_1 + GROUP_2 + GROUP_3 + GROUP_5));
    ...
    for (loop = 0; loop <= explosion_counter; loop += 1) {
      if ((ship->type >> 4) == PARACHUTE) {
        explode_ship_from_collision_box(ship, explosion_velocity, -1, style,
                                        group_mask, loop);
      } else {
        explode_ship_from_collision_box(ship, explosion_velocity, (long) ship,
                                        style, group_mask, loop);
      }
    }
...
explode_static_graphic(long x_grid, long y_grid, long style)
{
  ...
  long bit_mask =
      255; // Set a big bunch of bits to enable all explosion for all groups
  ...
  switch (style) {
    ...
    case REF_STATIC_EXPLODE_MEGATOWER_2_GROUPS:
      ...
      add_explosion_section(x_pos, y_pos, z_pos, 0, 0, (1 << 22), type,
                            bit_mask, group_number);
      break;
    ...
    case REF_STATIC_EXPLODE_PALACE_REL_3_GROUPS:
      ...
      bit_mask =
          GROUP_0 + GROUP_2 + GROUP_3; // Don't plot groups with this bit set
      group_number = 1;                // Centre round this group
      add_explosion_section(x_pos, y_pos, z_pos, 0, 0, (1 << 22), type,
                            bit_mask, group_number);

      bit_mask =
          GROUP_0 + GROUP_1 + GROUP_3; // Don't plot groups with this bit set
      group_number = 2;                // Centre round this group
      add_explosion_section(x_pos, y_pos, z_pos, 0, 0, (1 << 22), type,
                            bit_mask, group_number);

      bit_mask =
          GROUP_0 + GROUP_1 + GROUP_2; // Don't plot groups with this bit set
      group_number = 0;                // Centre round this group
      add_explosion_section(x_pos, y_pos, z_pos, 0, 0, (1 << 22), type,
                            bit_mask, group_number);

      // dont think so ??????????
      // bit_mask = GROUP_1+GROUP_2+GROUP_3 ;			// Don't EXPLODE
      // groups with this bit set
  ...
  explode_static_from_collision_box(x_grid + (y_grid << 7), explosion_velocity,
                                    x_grid + (y_grid << 7),
                                    EXPLOSION_STYLE_NORMAL, bit_mask);
  ...
  //								x_pos +
  //explosion_data->x_pos , 								y_pos + explosion_data->y_pos , 								z_pos +
  //explosion_data->z_pos , 								x_pos + explosion_data->x_pos2 , 								y_pos +
  //explosion_data->y_pos2 , 								z_pos + explosion_data->z_pos2 ,
  //								explosion_data->colour1
  //, 								explosion_data->colour2 , 								explosion_velocity , 								size , 								x_grid + (y_grid<<7)
...
void
add_explosion_line(long x_vel, long y_vel, long z_vel, long start_x,
                   long start_y, long start_z, long end_x, long end_y,
                   long end_z, long colour1, long colour2,
                   long explosion_velocity, long size, long owner, long style)
{
  static long x_rot = 0;
  static long y_rot = 0;
  ...
  for (loop = 0; loop <= counter; loop += 1) {
    ...
    if ((loop & 1) == 0 || style == EXPLOSION_STYLE_MEGA_BOMB ||
        style == EXPLOSION_STYLE_LOADS_OF_BITS) {
      ...
      add_bit(start_x + x_exp, start_y + y_exp, start_z + z_exp, x_vel, y_vel,
              z_vel, colour1, colour2, bit_size, explosion_velocity, owner,
              style);
    }
    ...
    for (loop_2 = 0; loop_2 < 4; loop_2 += 1) {
      ...
      if (size == 0) {
        add_smoke(start_x + (x_exp >> 2), start_y + (y_exp >> 2),
                  start_z + (z_exp >> 2), (x_vel >> 1) + (x_exp >> 3),
                  (y_vel >> 1) + (y_exp >> 3), (z_vel >> 1) + (z_exp >> 3),
                  SMALL_EXPLOSION_SMOKE, 0);
      } else {
        if (bit_size >= 6) {
          add_smoke(start_x + (x_exp), start_y + (y_exp), start_z + (z_exp),
                    (x_vel >> 1) + (x_exp >> 1), (y_vel >> 1) + (y_exp >> 1),
                    (z_vel >> 1) + (z_exp >> 1), BIG_EXPLOSION_SMOKE, 0);
        } else {
          add_smoke(start_x + (x_exp >> 1), start_y + (y_exp >> 1),
                    start_z + (z_exp >> 1), (x_vel >> 1) + (x_exp >> 2),
                    (y_vel >> 1) + (y_exp >> 2), (z_vel >> 1) + (z_exp >> 2),
                    EXPLOSION_SMOKE, 0);
        }
      }
...
void
dent_ground(long x_pos, long y_pos, long damage)
{
  ...
  armburn_addexplosion((unsigned) x_pos >> 24, (unsigned) y_pos >> 24,
                       arm_random() & 3);
  ...
  if (poly_map[y_pos >> 1][x_pos >> 1] != 0 && height_map[y_pos][x_pos] > 17) {
    static_explode((x_pos >> 1) + ((y_pos >> 1) << 7), 255);
  }

  if (height_map[y_pos][x_pos] < 17) {
    return;
  }
  ...
  if (height_map[y_pos][(x_pos - 1) & 255] - new_height > 16) {
    ...
    dent_ground((long) ((((unsigned) (x_pos - 1) & 255) << 24) - (1 << 23)),
                ...
                (long) (height_map[y_pos][(x_pos - 1) & 255] - new_height) -
                    16);
  }
  ...
  if (height_map[(y_pos - 1) & 255][x_pos] - new_height > 16) {
    ...
    dent_ground((long) (unsigned) ((x_pos << 24) - (1 << 23)),
                ...
                (long) (height_map[(y_pos - 1) & 255][x_pos] - new_height) -
                    16);
  }
  ...
  if (height_map[(y_pos + 1) & 255][(x_pos - 1) & 255] - new_height > 16) {
    ...
    dent_ground(
        (long) (unsigned) ((((x_pos - 1) & 255) << 24) - (1 << 23)),
        (long) (unsigned) ((((y_pos + 1) & 255) << 24) - (1 << 23)),
        (long) (height_map[(y_pos + 1) & 255][(x_pos - 1) & 255] - new_height) -
            16);
  }
...
void
undent_ground(long x_pos, long y_pos, long damage)
{
  ...
  if (height_map[y_pos][(x_pos - 1) & 255] - new_height < -16) {
    ...
    undent_ground((long) ((((unsigned) (x_pos - 1) & 255) << 24) - (1 << 23)),
                  (long) (((unsigned) y_pos << 24) - (1 << 23)),
                  (long) (new_height - height_map[y_pos][(x_pos - 1) & 255]) -
                      16);
  }
  ...
  if (height_map[(y_pos - 1) & 255][x_pos] - new_height < -16) {
    ...
    undent_ground((long) (unsigned) ((x_pos << 24) - (1 << 23)),
                  (long) (unsigned) ((((y_pos - 1) & 255) << 24) - (1 << 23)),
                  (long) (new_height - height_map[(y_pos - 1) & 255][x_pos]) -
                      16);
  }
  ...
  if (height_map[(y_pos + 1) & 255][(x_pos - 1) & 255] - new_height < -16) {
    ...
    undent_ground(
        (long) (unsigned) ((((x_pos - 1) & 255) << 24) - (1 << 23)),
        (long) (unsigned) ((((y_pos + 1) & 255) << 24) - (1 << 23)),
        (long) (new_height - height_map[(y_pos + 1) & 255][(x_pos - 1) & 255]) -
            16);
  }
...
ship_stack *
add_explosion_section(long x_pos, long y_pos, long z_pos, long x_vel,
                      long y_vel, long z_vel, long type, long group_mask,
                      long centre_group)
{
  ...
  collision_header =
      (collision_details_header *) (details + type)->collision_adr;

  section = (ship_stack *) armlink_sorttolist(&ships, x_pos);
  if (section != NULL) {

    memset((void *) (((char *) section) + (sizeof(link_header))), 0,
           (sizeof(ship_stack)) - (sizeof(link_header)));

    find_group_centre_point(collision_header, section, centre_group);
...
void
explosion_section_control(ship_stack *section)
{
  ...
  collision_header =
      (collision_details_header *) (details + (section->type - SECTION))
          ->collision_adr;
  collision_data = (collision_details *) (collision_header + 1);
  section_group_data =
      (section_groups *) (details + (section->type - SECTION))->extra_data;
  ...
  if (section_group_data != NULL) {
    ...
    switch (section->misc_counter) {
      case 0:
        section_data_header =
            (section_details_header *) section_group_data->group_0;
        break;
      case 1:
        section_data_header =
            (section_details_header *) section_group_data->group_1;
        break;
      case 2:
        section_data_header =
            (section_details_header *) section_group_data->group_2;
        break;
      case 3:
        section_data_header =
            (section_details_header *) section_group_data->group_3;
        break;
      case 4:
        section_data_header =
            (section_details_header *) section_group_data->group_4;
        break;
      case 5:
        section_data_header =
            (section_details_header *) section_group_data->group_5;
        break;
      case 6:
        section_data_header =
            (section_details_header *) section_group_data->group_6;
        break;
    }
    ...
    if (section_data_header != NULL) {
      ...
      for (loop = 0; loop <= section_data_header->counter; loop += 1) {
        ...
        add_section_smoke(x_pos, y_pos, z_pos, x_pos2, y_pos2, z_pos2, x_vel,
                          y_vel, z_vel, section_data->type);
...
ship_stack *
add_ship_explosion_section(ship_stack *ship, long group_mask, long centre_group)
{
  ...
  collision_header =
      (collision_details_header *) (details + ship->type)->collision_adr;
  ...
  if (section != NULL) {
    ...
    section->x_vel =
        ship->x_vel + ((((arm_random()) & 255) * (1 << 14)) - (1 << 21));
    section->y_vel =
        ship->y_vel + ((((arm_random()) & 255) * (1 << 14)) - (1 << 21));
    section->z_vel =
        ship->z_vel + ((((arm_random()) & 255) * (1 << 14)) - (1 << 21));
...
void
ship_explosion_section_control(ship_stack *section)
{
  ...
  collision_header =
      (collision_details_header *) (details + (section->type - SHIP_SECTION))
          ->collision_adr;
  collision_data = (collision_details *) (collision_header + 1);
  section_group_data =
      (section_groups *) (details + (section->type - SHIP_SECTION))->extra_data;
  ...
  for (loop = 0; loop <= collision_header->counter; loop += 1) {
    ...
    if ((section->group_mask & (1 << (collision_data->group))) == 0 ||
        ((section->type - SHIP_SECTION) >> 4) != BIG_SHIP) {
      // printf("section coll check grop %d\n",explosion_data->bonus);
      node_data.x_pos = collision_data->x_pos;
      node_data.y_pos = collision_data->y_pos;
      node_data.z_pos = collision_data->z_pos;
...
void
find_group_centre_point(collision_details_header *coll_header,
                        ship_stack *section, long which_group)
{
...
void
add_section_smoke(long x_pos, long y_pos, long z_pos, long x_pos2, long y_pos2,
                  long z_pos2, long x_vel, long y_vel, long z_vel, long type)
{
  ...
  add_smoke(x_pos, y_pos, z_pos, x_vel, y_vel, z_vel,
...
void
explode_static_from_collision_box(long grid_ref, long explosion_velocity,
                                  long owner, long style, long bit_mask)
{
  ...
  for (loop = 0; loop <= coll_data_header->counter; loop += 1) {
    ...
    if ((bit_mask & (1 << (coll_data->group))) != 0) {
      ...
      too_many = (z_counter * x_counter * y_counter) +
                 (coll_data_header->counter << 2);
          ...
          for (z_loop = 1; z_loop <= z_counter; z_loop += 1) {
            if ((arm_random() & 63) > too_many) {
              add_bit(x_pos + x_pos_exp, y_pos + y_pos_exp, z_pos + z_pos_exp,
                      (x_pos_exp >> 4), (y_pos_exp >> 4),
                      (z_pos_exp >> 4),     // x_vel , y_vel , z_vel ,
                      coll_data->colour, 0, //  colour1 , colour2 ,
                      bit_size + (arm_random() & 3), (explosion_velocity >> 1),
                      owner, style);
            }
            ...
            if ((arm_random() & 63) > too_many) {
              ...
              for (loop_2 = 0; loop_2 < 2; loop_2 += 1) {
                ...
                if (size == 0) {
                  add_smoke(x_pos + x_pos_exp + (x_exp >> 2),
                            y_pos + y_pos_exp + (y_exp >> 2),
                            z_pos + z_pos_exp + (z_exp >> 2),
                            (x_pos_exp >> 3) + (x_exp >> 3),
                            (y_pos_exp >> 3) + (y_exp >> 3),
                            (z_pos_exp >> 3) + (z_exp >> 3),
                            SMALL_EXPLOSION_SMOKE, 0);
                  ...
                  if (bit_size >= 6) {
                    add_smoke(x_pos + x_pos_exp + (x_exp),
                              y_pos + y_pos_exp + (y_exp),
                              z_pos + z_pos_exp + (z_exp),
                              (x_pos_exp >> 3) + (x_exp >> 1),
                              (y_pos_exp >> 3) + (y_exp >> 1),
                              (z_pos_exp >> 3) + (z_exp >> 1),
                              BIG_EXPLOSION_SMOKE, 0);
                  } else {
                    add_smoke(x_pos + x_pos_exp + (x_exp >> 1),
                              y_pos + y_pos_exp + (y_exp >> 1),
                              z_pos + z_pos_exp + (z_exp >> 1),
                              (x_pos_exp >> 3) + (x_exp >> 2),
                              (y_pos_exp >> 3) + (y_exp >> 2),
                              (z_pos_exp >> 3) + (z_exp >> 2), EXPLOSION_SMOKE,
                              0);
                  }
                }
  ...
  for (loop = 0; loop <= explosion_header->counter; loop += 1) {
    if (explosion_data->colour2 == 8) {
      if ((arm_random() & 7) == 0) {
        add_bonus(
            x_pos + ((explosion_data->x_pos + explosion_data->x_pos) >> 1),
...
void
explode_ship_from_collision_box(ship_stack *ship, long explosion_velocity,
                                long owner, long style, long bit_mask,
                                long bonus_counter)
{
  ...
  for (loop = 0; loop <= coll_data_header->counter; loop += 1) {
    ...
    if ((bit_mask & (1 << (coll_data->group))) != 0) {
      ...
      too_many = (z_counter * x_counter *
                  y_counter); //+(coll_data_header->counter<<2) ;
        ...
        for (y_loop = 1; y_loop <= y_counter; y_loop += 1) {
          ...
          for (z_loop = 1; z_loop <= z_counter; z_loop += 1) {
            if ((arm_random() & 63) > too_many) {
              ...
              add_bit(x_pos + node_data.x_pos, y_pos + node_data.y_pos,
                      z_pos + node_data.z_pos,
                      ship->x_vel + (node_data.x_pos >> 4),
                      ship->y_vel + (node_data.y_pos >> 4),
                      ship->z_vel + (node_data.z_pos >> 4), coll_data->colour,
                      0, //  colour1 , colour2 ,
                      bit_size + (arm_random() & 3), (explosion_velocity >> 1),
                      owner, style);
            }
            ...
            if (section_randy_crystals == 1 && (arm_random() & 15) == 0) {
              add_bonus(x_pos + node_data.x_pos, y_pos + node_data.y_pos,
                        z_pos + node_data.z_pos, ship->x_vel, ship->y_vel,
                        ship->z_vel + (1 << 22), (arm_random() & 7));
            }
  ...
  if (section_randy_crystals == 0) {
    ...
    if (explosion_header->counter >= 0 && bonus_counter == 0 &&
        (ship->type >> 4) != WEAPON) {
      for (loop = 0; loop <= explosion_header->counter; loop += 1) {
        node_data.x_pos =
            ((explosion_data->x_pos + explosion_data->x_pos) >> 1);
        node_data.y_pos =
            ((explosion_data->y_pos + explosion_data->y_pos) >> 1);
        node_data.z_pos =
            ((explosion_data->z_pos + explosion_data->z_pos) >> 1);

        if (explosion_data->colour2 == 8) {
          if ((arm_random() & 7) == 0) {
            if ((ship->type >> 4) == BIG_SHIP) {
              add_bonus(
                  x_pos + node_data.x_pos, y_pos + node_data.y_pos,
                  z_pos + node_data.z_pos, ship->x_vel + (node_data.x_pos >> 3),
                  ship->y_vel, //+ (node_data.y_pos>>5) ,
                  ship->z_vel + (node_data.z_pos >> 3), (arm_random() & 7));
            } else {
              add_bonus(x_pos + (node_data.x_pos << 1),
                        ...
                        ship->z_vel + node_data.z_pos, (arm_random() & 7));
            }
          }
        } else {
          if ((ship->type >> 4) == BIG_SHIP) {
            ...
            add_bonus(
                x_pos + (node_data.x_pos << 1), y_pos + (node_data.y_pos << 1),
                z_pos + (node_data.z_pos << 1), ship->x_vel + node_data.x_pos,
                ship->y_vel, //+ node_data.y_pos ,
                ship->z_vel + node_data.z_pos, ((explosion_data->colour2) & 7));
          }
        }
        explosion_data += 1;
      }
