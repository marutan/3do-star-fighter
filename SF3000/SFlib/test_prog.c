...
void
new_fighter_control(ship_stack *ship)

{
  ...
  // Avoid hills
  if (ship->z_pos < (40 << 24) && ship->z_pos > -(16 << 24) &&
    ...
    for (loop = 0; loop <= 3; loop += 1) {
      ...
      coll_check = find_ground_height(goto_x, goto_y);

      if (coll_check >
          (ship->z_pos + (ship->z_vel << (2 + loop)) - (1 << (23 + loop)))) {
        ...
        // goto_x =	ship->x_pos -( sine_table [ (ship->x_rot)>>10 ] <<14 ) ;
        // goto_y =	ship->y_pos -( cosine_table [ (ship->x_rot)>>10 ] <<14 )
        // ; goto_z = ship->z_pos - (1<<22) ; coll_check = 1 ;
  ...
  if (coll_check == 0) {
    if (ship->command == COMMAND_ATTACK_RUN_AWAY) {
      ...
      node_data.x_pos =
          ((ship->x_pos) -
           (ship->goto_x + (((dist >> 12) * temp_ship->x_vel) >> 14))) >>
          6;

      node_data.y_pos =
          ((ship->y_pos) -
           (ship->goto_y + (((dist >> 12) * temp_ship->y_vel) >> 14))) >>
          6;

      node_data.z_pos =
          ((ship->goto_z + (((dist >> 12) * temp_ship->z_vel) >> 14)) -
           (ship->z_pos)) >>
          6;
    }
  } else {
  ...
  temp_long = (((cosine_table[((x_aim >> 10) & 1023)] *
                 cosine_table[((y_aim >> 10) & 1023)]) >>
                12) *
               ((cosine_table[((x_rot_rel >> 10) & 1023)] *
                 cosine_table[((y_rot_rel >> 10) & 1023)]) >>
                12)) >>
              14;
  ...
  // Climb
  if (ship->y_control > temp_long) {
    ship->y_control = temp_long;
  }
  if (ship->y_control < -temp_long) {
    ship->y_control = -temp_long;
  }
  ...
  // Yaw
  if (ship->z_control > temp_long) {
    ship->z_control = temp_long;
  }
  if (ship->z_control < -temp_long) {
    ship->z_control = -temp_long;
  }
  ...
  if (ship->command == COMMAND_ATTACK_RUN_AWAY) {
    ...
    if (dist < (4 << 24)) {
      ...
      ship->aim_goto_y =
          temp_ship->y_pos - ((cosine_table[(ship->x_dir) >> 10] *
      ...
      // 4096 speed = 1 sprite unit per frame
      ship->aim_goto_x =
          temp_ship->x_pos - ((sine_table[(ship->aim_x_rot) >> 10] *
                               cosine_table[(ship->aim_y_rot) >> 10])
                              << 6);

      ship->aim_goto_y =
          temp_ship->y_pos - ((cosine_table[(ship->aim_x_rot) >> 10] *
                               cosine_table[(ship->aim_y_rot) >> 10])
                              << 6);

      ship->aim_goto_z =
          temp_ship->z_pos + (sine_table[(ship->aim_y_rot) >> 10] << 18);

      temp_long = ((ship->command_counter & 63) - 32) << 6;

      ship->aim_x_rot =
          ((ship->aim_z_rot + ship->aim_x_rot + temp_long) & ROT_LIMIT);
      ship->aim_y_rot = ((ship->aim_y_rot - (temp_long >> 1)) & ROT_LIMIT);

      // Tend to levl
      if (ship->aim_y_rot > (512 << 10)) {
        ship->aim_y_rot -= (1024 << 10);
      }
  ...
  if (ship->thrust_control < 0) {
    ship->thrust_control = 0;
  }
  if (ship->thrust_control > 1024) {
...
void
new_flight_path_control(ship_stack *ship)

{
  ...
  node_data.x_pos = ((ship->x_pos) - (ship->goto_x)) >> 6;

  node_data.y_pos = ((ship->y_pos) - (ship->goto_y)) >> 6;

  node_data.z_pos = ((ship->goto_z) - (ship->z_pos)) >> 6;
  ...
  if (ship->speed > (1024 + 256)) {
    ship->thrust_control -= 256;
  }
...
void
new_formation_control(ship_stack *ship)

{
  ...
  if (ship->command == COMMAND_ENTER_FORMATION) {
    ...
    // Offset for the ships y if in formation with the player when in the mother
    // ship
    if (((temp_ship->type) >> 4) == PLAYERS_SHIP &&
        docked.status != DOCKING_OUT) {
  ...
  node_data.x_pos = ((ship->x_pos) -
                     (temp_ship->x_pos + node_data.x_pos + ship->aim_x_vel)) >>
                    6;

  node_data.y_pos = ((ship->y_pos) -
                     (temp_ship->y_pos + node_data.y_pos + ship->aim_y_vel)) >>
                    6;
