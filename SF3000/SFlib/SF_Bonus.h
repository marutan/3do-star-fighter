...
// Function Prototypes for crystal bonus awards - All accept 1 long, 1 char, 1
// long as a parameter (from award)

long bonus_weapon(long, long, long);      // Award weapon upgrade
long bonus_wingpod(long, long, long);     // Award wingpod
long bonus_performance(long, long, long); // Award engine / control / shields
long bonus_fastcrystal(long, long, long); // Award fast adder crystals

long
bonus_megasetup_start(long, long,
                      long); // Award megaship for a certain ammount of time
void bonus_megasetup_cancel(void); // Reset player from megaship
void bonus_deletewingpods(void);   // Routine to delete all wingpods

void
bonus_downgrade_ship(long damage_rating); // Picks a thing randomly & downgrades
                                          // it, displays message etc.
#endif
