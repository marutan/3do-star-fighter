...
collision_update(void)
{
  ...
  end_of_collision_stack =
      armcol_collisioncheck(start_of_collision_stack, &lasers, &ships);
  ...
  while (start_of_collision_stack < end_of_collision_stack) {
    ...
    if (((ship->type) >> 4) != BONUS) {
      ...
      // ###########################################################
      if (coll_check != 0 &&
          ((ship->type >> 4) != PLAYERS_SHIP || docked.status == DOCKING_OUT)) {
        laser->counter = 0;
        ...
        if ((ship->type >> 4) == WEAPON &&
            ((ship_stack *) laser->who_owns_me) == players_ship &&
            ship->who_owns_me != (long) players_ship) {
        ...
        if (((ship->type) >> 4) == PLAYERS_SHIP && ship->shields <= 0 &&
            ship->damage <= DAMAGE_MEDIUM_COUNTER && (arm_random() & 3) == 2) {
          ship->shields = 10;
        ...
        if ((ship->type >> 4) == PLAYERS_SHIP && (arm_random() & 15) == 0 &&
            ship->shields < 32) {
          bonus_downgrade_ship(0);
        }

        if ((ship->type >> 4) == PLAYERS_SHIP && (arm_random() & 31) == 0) {
          bonus_downgrade_ship(0);
        }

        add_smoke(laser->x_pos, laser->y_pos, laser->z_pos, ship->x_vel,
                  ship->y_vel, ship->z_vel, LASER_HIT_SMOKE, 0);
      }
    }
...
void
bonus_collision(ship_stack *bonus)
{
...
void
parachute_collision(ship_stack *parachute)
{
...
void
missile_collision(ship_stack *missile)
{
  ...
  // This has changed
  if ((other_ship->type >> 4) == PLAYERS_SHIP && docked.status != DOCKING_OUT) {
    other_ship->what_hit_me = 0;
  ...
  if (((other_ship->type) >> 4) == BONUS ||
      ((other_ship->type) >> 4) == PARACHUTE) {
    return;
  }

  // Chance of missing the player
  if ((other_ship->type >> 4) == PLAYERS_SHIP && (arm_random() & 3) == 0) {
  ...
  // The players ship can survive death sometimes
  if (((other_ship->type) >> 4) == PLAYERS_SHIP && other_ship->shields <= 0 &&
      other_ship->damage <= DAMAGE_MEDIUM_COUNTER && (arm_random() & 3) == 0) {
...
void
small_ship_collision(ship_stack *ship)
{
  ...
  if ((other_ship->type >> 4) == BIG_SHIP || (other_ship->type >> 4) == CAR) {
    ...
    if (x_pos < size && y_pos < size && z_pos < size) {
      coll_check = 1;
    }
  }

  // If the players ship is in the docking bay set the flag
  if (coll_check == 2 && ((ship->type) >> 4) == PLAYERS_SHIP &&
      other_ship->special_data->side != SDB_SIDE_ENEMY) {
  ...
  if (coll_check != 0) {
    ...
    if (ship != players_ship ||
      ...
      if (((ship->type >> 4) == PLAYERS_SHIP ||
           (other_ship->type >> 4) == PLAYERS_SHIP) &&
          (arm_random() & 3) == 0) {
