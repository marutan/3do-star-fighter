 ...
 *
 *	History:
 *	10/06/94		dtc		Use the default stream header if
 *one is not available. 09/14/94		dtc		Replaced printf
 *with ERR macro in Debug3DO.h. 12/3/93			jb		Change
 *test for stream header version to != from > 12/2/93			jb
 *Add "close" routines to DismantlePlayer() Set stream clock to zero at start
 *stream time if there is no audio present in the stream to allow silent movies
 *							to play properly.
 *							Add addition new arg to
 *NewDataStream() 11/22/93		jb		Add call to
 *DismantlePlayer() at the end of PlayCPakStream() 10/25/93		jb
 *Fix order of disposals in DismantlePlayer() 10/20/93		jb
 *Do Cinepak only playback; need to get this out by the Developer's Symposium...
               ...
               PlayCPakUserFn userFn, void *userContext)
{
   ...
   */
  DisplayScreen(ctx->screenContextPtr
                    ->sc_ScreenItems[ctx->screenContextPtr->sc_CurrentScreen],
                0);

  /* Get a pointer to the bitmap we will draw into */
  bitmap = ctx->screenContextPtr
               ->sc_Bitmaps[ctx->screenContextPtr->sc_CurrentScreen];
  ...
  while (true) {
    ...
    if (ctx->cpakContextPtr != 0) {
      ...
      /* Free any streamer chunks back to the streamer (we're done with them
       * now) */
...
InitCPakPlayerFromStreamHeader(PlayerPtr ctx, char *streamFileName)
{
  ...
  status =
      NewDataAcq(&ctx->acqContext, streamFileName, ctx->hdr.dataAcqDeltaPri);
  if (status != 0) {
    goto CLEANUP;
  }
  ...
  for (subscriberIndex = 0;
    ...
    switch (subsPtr->subscriberType) {
      case FILM_CHUNK_TYPE:
        ...
        status =
            NewCPakSubscriber(&ctx->cpakContextPtr, 1, subsPtr->deltaPriority);
        ...
        status =
            InitCPakCel(ctx->streamCBPtr,     // Need this for DSGetClock in sub
      ...
      case SNDS_CHUNK_TYPE:
        ...
        status = NewSAudioSubscriber(&ctx->audioContextPtr, ctx->streamCBPtr,
      ...
      case CTRL_CHUNK_TYPE:
        ...
        status = NewCtrlSubscriber(&ctx->controlContextPtr, ctx->streamCBPtr,
                                   subsPtr->deltaPriority);
      ...
      default:
        ERR(("InitCPakPlayerFromStreamHeader() - unknown subscriber in stream "
             "header: '%.4s'\n",
