 ...
 *
 *	History:
 *	10/06/94		dtc		Use the default stream header if one is not available.
 *	09/14/94		dtc		Replaced printf with ERR macro in Debug3DO.h.
 *	12/3/93			jb		Change test for stream header version to != from >
 *	12/2/93			jb		Add "close" routines to DismantlePlayer()
 *							Set stream clock to zero at start stream time if there
 *							is no audio present in the stream to allow silent movies
 *							to play properly.
 *							Add addition new arg to NewDataStream()
 *	11/22/93		jb		Add call to DismantlePlayer() at the end of PlayCPakStream()
 *	10/25/93		jb		Fix order of disposals in DismantlePlayer()
 *	10/20/93		jb		Do Cinepak only playback; need to get this out by the
...
/************************************/
static int32	InitCPakPlayerFromStreamHeader( PlayerPtr ctx, char* streamFileName );
void			DismantlePlayer( PlayerPtr ctx );


/*******************************************************************************************
 * Routine to play a Cinepak stream file. Stream may or may not contain audio data.
...
int32	PlayCPakStream( ScreenContext *screenContextPtr, char* streamFileName,
	...
	while ( true )
		...
		if ( ctx->cpakContextPtr != 0 )
			...
			/* Free any streamer chunks back to the streamer (we're done with them now) */
...
static int32	InitCPakPlayerFromStreamHeader( PlayerPtr ctx, char* streamFileName )
	...
	for ( subscriberIndex = 0; ctx->hdr.subscriberList[ subscriberIndex ].subscriberType != 0;
		...
		switch ( subsPtr->subscriberType )
			...
			default:
				ERR( ( "InitCPakPlayerFromStreamHeader() - unknown subscriber in stream header: '%.4s'\n",
...
void		DismantlePlayer( PlayerPtr ctx )
	...
	if ( ctx->bufferList ) 				FreePtr( ctx->bufferList );
