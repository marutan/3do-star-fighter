...
void	message_initialise (long demo_mode)

// Purpose : Resets al message timers to zero - sets demo mode for message adding
// Accepts : demo mode flag
// Returns : Nothing
...
void	message_pilot (long pilot_name, long message_number, long info)
...
char	pilot_message [48];
	...
	sprintf (pilot_message, "%s  %s", message_decode (MTXT__MESSAGE_FROM, 0), (pilot_name == -1) ? message_decode (MTXT__STRATEGIC_COMMAND, 0) : pilot_names [pilot_name]);
	message_text (MESSAGE_NAME, pilot_message, WHITE_15, MESSAGE_SCROLL, 48, MESSAGE_CURSOR1);
	
	if (message_number == -1)
		{
		sprintf (pilot_message, "%s %s", pilot_names [info], message_decode (MTXT__LOST_IN_DUTY, 0));
		message_text (MESSAGE_BOTTOM, pilot_message, YELLOW_15, MESSAGE_SCROLL, 48, MESSAGE_CURSOR1);
		}
	else
		{
		message_add (MESSAGE_BOTTOM, message_number, YELLOW_15, MESSAGE_SCROLL, 48, MESSAGE_CURSOR1);
		}
}			
...
long	ypos=0,
	...
	if (message_gamestatus == 1 && type != MESSAGE_DEMO)	// Filter out messages depending on game mode
		return;
