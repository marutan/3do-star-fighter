...
long player_crashed_xpos, // To remember where player was when they died /
                          // crashed (for reset in status.c)
    player_crashed_ypos, player_crashed_zpos;

char weapon_names[] = {MTXT__LASER,    MTXT__ATG,        MTXT__ATA,
                       MTXT__MEGABOMB, MTXT__BEAM_LASER, MTXT__MULTI_MISSILE,
                       MTXT__MINES};

/**************************************/

void
control_initialisecameras(void)
...
void
control_startgame(long nothing)
...

{
  ...
  memcpy(&player_performance, &player_resetperformance,
         sizeof(performance_data)); // Reset players performance data
  message_addscore(0);              // Initialise score

  status.lives = (cheat_feature2 == 1) ? 99 : 3; // Set lives
                                                 // // Set no. of lives to 3
...
void
control_startmission(long demo_mode)

// Purpose : Resest SOME players stats at the start of a new mission (position
// etc.) Accepts : Nothing Returns : Nothing

{
  ...
  // armanim_initialise (animate);
  // // Initialise animations for mission [TAKEN OUT]
  ...
  status.clock3 = 0;
  status.formation_mode =
      SDB_FORMATION_NORMAL; // Reset formation control mode to normal
  ...
  if (camera[1].view_ship ==
      NULL) { // Check for unset cameras ? if so, look at players ship
  ...
  for (object_loop = 0; object_loop < mission.groundcount; object_loop++) {
    if (mission.groundlist[object_loop].type == GROUND_DESTROY ||
        mission.groundlist[object_loop].type == GROUND_HIDDENDESTROY) {
  ...
  if (status.flag_allgrounddead == 0 && status.flag_allshipdead == 0 &&
      status.flag_defendland == 0) {
  ...
  for (object_loop = 1; object_loop < MAX_CAMERAS;
       object_loop++) { // Re-init cameras ?
...
void
control_registerdeath(ship_stack *dead_ship)

// Purpose : A ship has died ! Is it the player ? Is it something we were
// looking at ? Accepts : Pointer to the dead ship Returns : Nothing

{
  ...
  // IF SO, START LOOKING AT ANOTHER ONE (OR THE PLAYERS SHIP IF NONE LEFT OF
  // THAT TYPE)
  ...
  if (mission.demo_mission == 1 && dead_ship->type >> 4 == SMALL_SHIP) {
    dead_ship->shields = (dead_ship->performance->shields)
                         << 5;          // Yes - reset shields
    ...
    dead_ship->z_pos = (50 + (arm_random() & 15)) << 24; // Yes - set new Z pos
                                                         // // Yes - return
    return;
  }
  ...
  if (dead_ship->special_data != NULL) {
    ...
    if (dead_ship->special_data->sdb_type == SDB_TYPE_PLEB) {
      message_add(MESSAGE_TOP, (MTXT__ENEMY_FIGHTER_DESTROYED), YELLOW_15,
                  MESSAGE_SCROLL, 64, MESSAGE_CURSOR1);
    }
    ...
    if (dead_ship->special_data->sdb_type == SDB_TYPE_NORMAL) {
      ...
      if (killstatus == SDB_KILL_MUSTSURVIVE ||
        ...
        camera[0].counter2 = 64;
        camera[0].x_pos = dead_ship->x_pos;
        camera[0].y_pos = dead_ship->y_pos;
        camera[0].z_pos = dead_ship->z_pos;

        if (dead_ship->type == MOTHER_SHIP) {
          message_add(MESSAGE_TOP, (MTXT__MOTHERSHIP_DESTROYED), ORANGE_15,
                      MESSAGE_SCROLL, 64, MESSAGE_CURSOR1);
        } else {
          message_add(MESSAGE_TOP, (MTXT__DEFENCE_TARGET_DESTROYED), ORANGE_15,
                      MESSAGE_SCROLL, 64, MESSAGE_CURSOR1);
        }
      ...
      if (killstatus == SDB_KILL_MUSTDIE ||
        ...
        message_add(MESSAGE_TOP, (MTXT__MISSION_SHIP_DESTROYED), ORANGE_15,
                    MESSAGE_SCROLL, 64, MESSAGE_CURSOR1);
  ...
  // WAS IT THE PLAYERS SHIP THAT DIED ? - MUST BE IN CONTINUE STATUS TO
  // REGISTER DEATH

  if (dead_ship == players_ship) {
    player_crashed_xpos =
        dead_ship->x_pos; // Remember where player was when they died
    player_crashed_ypos = dead_ship->y_pos;
    player_crashed_zpos = dead_ship->z_pos;

    camera[0].type =
        CAMERA_TRACKING; // Set the camera so it's tracking the dead player
    camera[0].view_ship = players_ship;
    players_ship->fire_request = WEAPON_NOTHING;
    status.current_camera = 0;

    if (status.status ==
        STATUS_CONTINUE) // In normal mode ? Must be to lose a life etc.
    {
      if (--status.lives < 1) { // Lose a life
        control_gameover();     // If no more lives, set to game over
      } else {
        status_register(STATUS_CRASHED, 64,
                        1); // Else just set status to crashed
      }
    }
    ...
    for (camloop = 0; camloop <= 3;
         camloop++) { // Cancel all crystals picked up
...

{
  ...
  long object_loop,
      ...
      ground_xpos, ground_ypos, ground_zpos,

      ship_xpos, ship_ypos, ship_zpos, distance,
      nearest_distance = -1, new_tracking_type = TRACKING_NOTHING;

  ship_stack *ship_track =
      (ship_stack *) (ships.info)
          .start_address; // Ship Pointer - start of ship linked list
  ...
  while ((ship_track->header).status == 1) // Search for nearest ship
  {
    ...
    if (ship_track->special_data != NULL) {                 // Ship has an SDB ?
      ...
      {
        ship_xpos = (ship_track->x_pos) -
                    (players_ship->x_pos); // Get X,Y,Z pos of enemy target
        if (ship_xpos < 0) {
          ship_xpos = -ship_xpos;
        }
    ...
    ship_track = (ship_stack *) (ship_track->header)
  ...
  for (object_loop = 0; object_loop < mission.groundcount; object_loop++) {
    if (mission.groundlist[object_loop].type == GROUND_DESTROY) {
      ground_xpos =
          ((mission.groundlist[object_loop].xpos) << 24) - players_ship->x_pos;
      if (ground_xpos < 0) {
        ground_xpos = -ground_xpos;
      }
      ground_ypos =
          ((mission.groundlist[object_loop].ypos) << 24) - players_ship->y_pos;
      if (ground_ypos < 0) {
        ground_ypos = -ground_ypos;
      }
      ground_zpos = players_ship->z_pos;
      if (ground_zpos < 0) {
        ground_zpos = -ground_zpos;
      }
      ...
      if (nearest_distance == -1 || distance < nearest_distance) {
        status.track_xpos = mission.groundlist[object_loop]
                                .xpos; // Set to look at ground object
...
void
control_searchcamera(long camera_set, long object_instance)

// Purpose : Sets a camera to view a particular instance of a ship type
// Accepts : Camera number to set and instance of object type to view (ie. 5th
// enemy ship, 3rd weapon etc.) Returns : Nothing

{
  ...
  while ((ship->header).status == 1 &&
         instance_count < object_instance) // While active ship, search
  {
    switch (camera[camera_set].view) {
      ...
      case 1:
        if (ship->special_data != NULL && ship != players_ship) {
      ...
      case 3:
        if (((ship->type) >> 4) == WEAPON && ship->type != WING_POD) {
  ...
  {
    camera[camera_set].instance =
        object_instance; // YES - OBJECT INSTANCE FOUND, SET INSTANCE
    camera[camera_set].view_ship =
        last_ship; // YES - SET ADDRESS OF OBJECT INSTANCE
  }

  if (instance_count <
      object_instance) // WE DID NOT FIND THE INSTANCE WE WANTED !
  {
    if (instance_count >= 0) // O.K. Did we find any of this type ?
    {
      control_searchcamera(camera_set,
                           0); // YES - Call again to get 1st instance

      // camera [camera_set].instance = instance_count;			// YES -
      // SET TO LAST ONE WE FOUND camera [camera_set].view_ship = last_ship;
      // // YES - SET ADDRESS OF OBJECT INSTANCE
    } else if (camera[camera_set].view !=
               0) { // NO - SET TO -ve INSTANCE OF TYPE (Override)
...
void
control_changecamera(long new_camera)

// Purpose : Sets status camera to new camera number & Displays camera
// description message Accepts : New camera to view Returns : Nothing
...
void
control_recheckcamera(ship_stack *ship)

// Purpose : Rechecks cameras when a new ship has been added - If we were
// pending view, watch it Accepts : Ship added Returns : Nothing
  ...
  for (camloop = 0; camloop < MAX_CAMERAS; camloop++) // Search all cameras
  {
    if (camera[camloop].instance == -1) { // Is this camera pending object ?
      switch (camera[camloop].view) {
        ...
        case 0:
          if (((ship->type) >> 4) ==
              PLAYERS_SHIP) // Waiting for this type of ship [PLAYER]
          {
        ...
        case 2:
          if (((ship->type) >> 4) ==
              SMALL_SHIP) { // Waiting for this type of ship [ENEMY SHIP] ?
            if (ship->special_data != NULL) {
              if (ship->special_data->side == SDB_SIDE_ENEMY) {
        ...
        case 3:
          if (((ship->type) >> 4) ==
              WEAPON) { // Waiting for this type of ship [WEAPON] ?
            if (ship->who_owns_me ==
                (long) players_ship) // Yes - is it owned by the player ?
            {
              camera[camloop].instance = 0; // YES - Look at it
              camera[camloop].view_ship = ship;
              break;
...

{
  ...
  message_text(MESSAGE_BOTTOM, weapon_string, ORANGE_15, MESSAGE_SCROLL, 32,
               MESSAGE_CURSOR1);
  message_addweapon();
}
...

{
  ...
  if (status.status == STATUS_MISSIONCOMPLETEDWAIT ||
      status.status == STATUS_MISSIONCOMPLETED) {
    return;
  }

  message_add(MESSAGE_BIG, (MTXT__YOU_HAVE_FAILED_YOUR_MISSION), ORANGE_15,
              MESSAGE_SCROLL, 64, MESSAGE_CURSOR1);
  message_add(MESSAGE_BOTTOM, (MTXT__GAME_OVER), YELLOW_15, MESSAGE_FLASH, 64,
              MESSAGE_NOCURSOR);
...
void
control_setupfromconfigure(void)
...

{
  ...
  sound_setmastervolume((configuration.sound_volume + 1) *
                        16); // Set master volume level for SFX
  music_setmastervolume((configuration.music_volume + 1) *
                        16); // Set master volume level for MUSIC

  BS_SetStickMinX(
      &joystick,
      (short) configuration.stick_x_min); // Set up joystick boundaries
...

{
  ...
  return (nvram_save("StarFighter.Config", "SFC:", (char *) &configuration,
                     sizeof(game_configuration)));
}

/**************************************/

long
control_showmission(long show_mode, long show_var)
...
#define BRIEF_ENTER 0x2
  ...
  static long
      textcel_addrate; // Counter before add next text line at top of screen
  static long textcels_added; // Next line to add at top of screen
  static long text_topcheck;  // Top check limit for text
  static long
      lastlineoffthetop;     // Last line that went off the top of the screen
  ...
  static long textcel_lastcolouroffthetop; // Colour of last item off the top of
                                           // the screen
  static long textcel_lastcolouroffthebot; // Colour of last item off the bot of
                                           // the screen
  ...
  if (show_mode < 2) {
    ...
    // DISPLAY MISSION DETAILS, ENTER MENU & START STREAMING OF VOICE OVER [IF
    // BRIEF]
    ...
    if (show_mode == 0) {
      ...
      if (configuration.sound_on == 1) {
        music_terminate();
        music_initialise(1);
        music_playvoice("%s%s%c/Miss_%d", RESOURCES_ROOT, VOICE_ROOT,
                        decode_level(parameters.level),
                        1 + return_mission(&pyramids[parameters.level]));
      }
    }
  ...
  if (show_mode == 2) {
    if (keypad_constant & ControlUp || keypad_constant & ControlLeftShift) {
      text_direction = -1; // Up ?
    }
    if (keypad_constant & ControlDown || keypad_constant & ControlRightShift) {
      text_direction = +1; // Down ?
    }
  }
  ...
  do {
    ...
    if (text_direction > 0) // Are we scrolling text down ?
    {
      if (--textcel_addrate < 0) // Add a line from the text file ?
      {
        ...
        if (text_overflow >= 8) {
          ...
          if (textcel_lastcolouroffthetop !=
              -1) // Get last colour that went off the top
          {
    ...
    if (textcel_newline & BRIEF_ENTER) {
      textcel_case = (mission_linetouse == -1) ? -1
                     : (show_mode == 3)        ? 0
                                               : mission_linetouse;
      ...
      textcel = 1;
      do {
        if ((((cel_text[textcel]->tc_CCB)->ccb_Flags) &
             CCB_SKIP)) {              // Is cel inactive ?
      ...
      switch (textcel_case) {
        ...
        case 0:
          ...
          break;
        case 1:
          sprintf(mission_line, "Location : '%s'",
                  message_decode(mission_linetouse, 2));
          mission_linecolour = RED_15;
          break;
        case 2:
          sprintf(mission_line, "Date : %s A.D",
                  message_decode(mission_linetouse, 2));
          mission_linecolour = RED_15;
          break;
        default:
          ...
          if (*(message_decode(mission_linetouse, 2)) == 0) {
            textcel_fore =
                (textcel_fore == DARKGREEN1_15) ? DARKGREEN2_15 : DARKGREEN1_15;
          }
          mission_linecolour = textcel_fore;
          break;
      }

      SetTextCelColor(cel_text[textcel_slottouse], 0,
                      mission_linecolour); // Set colour
      UpdateTextInCel(cel_text[textcel_slottouse], TRUE,
                      mission_line); // Render text cel
      GetTextCelSize(cel_text[textcel_slottouse], &cel_width,
                     &cel_height); // Get it's size
      SetTextCelCoords(cel_text[textcel_slottouse], 160 - (cel_width >> 1),
                       textcel_newypos); // Set it's position
      ((cel_text[textcel_slottouse]->tc_CCB)->ccb_PRE1) &=
          (~1023); // Set it's Width to zero (If we are scrolling it)
    ...
    for (textcel = 1; textcel < FONT_MAXMENU; textcel++) {
            ...
            CCB_SKIP)) // Is cel active ?
      {
        ...
        if (text_direction != 0) {
          ...
          ((cel_text[textcel]->tc_CCB)->ccb_YPos) +=
              (text_direction << 16); // Yes, move it

          if ((cel_text[textcel]->tc_CCB)->ccb_YPos <=
              text_topcheck) // Off top of screen ?
          {
            ((cel_text[textcel]->tc_CCB)->ccb_Flags) |=
                CCB_SKIP; // Yes, take it out
            textcel_lastcolouroffthetop =
                (long) cel_text[textcel]->tc_userData; // Remember colour !
          ...
          if ((cel_text[textcel]->tc_CCB)->ccb_YPos >=
              text_bottomcheck) // Off bottom of screen ?
          {

            ((cel_text[textcel]->tc_CCB)->ccb_Flags) |=
                CCB_SKIP; // Yes, take it out
            textcel_lastcolouroffthebot =
                (long) cel_text[textcel]->tc_userData; // Remember colour !
  ...
  menu_status.cel_textccb =
      cel_text[1]->tc_CCB; // Set menutext to start display at mission brief
  ...
  if (show_mode == 2) {
    menu_status.cel_textccb =
        cel_text[0]->tc_CCB; // Set menutext to start display at menu title
    menu_display(RED_PAL);   // Display title
  }

  if (show_mode == 3 &&
      keypad != 0) { // If doing credits & keypress, return quit
    return (BRIEF_QUIT);
  }
...

{
  ...
  if (status.lives < 7) {
    message_add(MESSAGE_BIG, MTXT__EXTRA_LIFE, WHITE_15, MESSAGE_SCROLL, 32,
                MESSAGE_NOCURSOR);
    status.lives += 1;
