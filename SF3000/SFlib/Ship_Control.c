...
if ( ship->fire_request != WEAPON_NOTHING && ship->counter == 0 )
{
	...
	if (ship->fire_request != WEAPON_LASER && ((ship->type)>>4) == PLAYERS_SHIP )
	{
		...
		if (ship->performance->weapons[ship->fire_request] <= 0)
		{
			...
			if (ship->fire_request == WEAPON_ECM)
				message_add(MESSAGE_BOTTOM , MTXT__NO_ECMS , RED_15 , MESSAGE_SCROLL , 24 , MESSAGE_CURSOR1);
			ship->fire_request = WEAPON_NOTHING;
		}
		else
		{
	...
	// atg missiles can be treated as ata missiles when in shooting trucks n' tanks
