...
update_docking(ship_stack *ship)
{
  ...
  switch (docked.status) {
    ...
    case DOCKING_EXIT:
      ...
      ship->x_vel = -(((sine_table[(ship->x_dir) >> 10] *
                        cosine_table[(ship->y_dir) >> 10]) >>
                       12) *
                      (ship->speed));

      ship->y_vel = -(((cosine_table[(ship->x_dir) >> 10] *
                        cosine_table[(ship->y_dir) >> 10]) >>
                       12) *
                      (ship->speed));
...

{
  ...
  x_control = ((ship->x_control +
                ((ship->x_control * ship->performance->control) >> 3)) *
               temp_long) >>
              11;
  y_control = ((ship->y_control +
                ((ship->y_control * ship->performance->control) >> 3)) *
               temp_long) >>
              11;
  z_control = ((ship->z_control +
                ((ship->z_control * ship->performance->control) >> 3)) *
               temp_long) >>
              11;
  ...
  // There is a max change of x rot of 180 degrees when yer flip at the bottom /
  // top the z roll must change to keep up How about z roll rate of change = x
  // change * sine y rot ???

  // Find rate of change of x rot
  temp_long = (ship->x_rot - target.x_rot);
  if (temp_long > 512 * 1024) {
    temp_long -= 1024 * 1024;
  }
  if (temp_long < -512 * 1024) {
    temp_long += 1024 * 1024;
  ...
  ship->z_rot =
      ((ship->z_rot + ((sine_table[ship->y_rot >> 10] * temp_long) >> 10)) &
       ROT_LIMIT);
  ...
  ship->x_rot =
      (ship->x_rot
       ...
       -
       ((sine_table[ship->z_rot >> 10] * cosine_table[ship->y_rot >> 10]) >> 12)

           ) &
      ROT_LIMIT;

  // change z rot based on x control and z_roller - (set by laser / missile
  // hits)
  ...
  ship->x_vel = -(
      ((sine_table[(ship->x_dir) >> 10] * cosine_table[(ship->y_dir) >> 10]) >>
       12) *
      (ship->speed));
  ...
  if (ship->thrust_control > 0) {
    ...
    ship->speed +=
        (((20 + ((ship->performance)->engine << 2)) * (ship->thrust_control)) >>
         10);

    // check if it can be seen n do thrusters if true
    if (ship->can_see) {
      fighter_thrusters(ship);
    }
  } else {
  ...
  // This counter is set when you fire - therefore you can only shoot laser
  // every other frame and missiles every 4 frames etc.......
  if (ship->counter > 0) {
    ship->counter -= 1;
  }
  ...
  if (ship->fire_request != WEAPON_NOTHING && ship->counter == 0) {
    ...
    if (ship->fire_request != WEAPON_LASER &&
      ...
      if (ship->performance->weapons[ship->fire_request] <= 0) {
        ...
        if (ship->fire_request == WEAPON_ECM) {
          message_add(MESSAGE_BOTTOM, MTXT__NO_ECMS, RED_15, MESSAGE_SCROLL, 24,
                      MESSAGE_CURSOR1);
        }
        ship->fire_request = WEAPON_NOTHING;
      } else {
        // If the weapon requires a lock (ie. missiles ) check its not null
        // before allowing it
        if ((ship->fire_request == WEAPON_ATA ||
             ship->fire_request == WEAPON_ATG) &&
            ship->target == NULL) {
          ship->fire_request = WEAPON_NOTHING;
        } else {
          if (ship->fire_request == WEAPON_MINE ||
              ship->fire_request == WEAPON_MULTI_MISSILE) {
            if (ship_setmultipletargets(ship, __WAR_SR_LOCAL,
                                        (ship->special_data)->side) != 0) {
    ...
    switch (ship->fire_request) {
      ...
      case WEAPON_ATG:
        ...
        // atg missiles can be treated as ata missiles when in shooting trucks
        // n' tanks
      ...
      case WEAPON_ATA:
        ...
        if ((ship_stack *) ship->target == players_ship) {
          message_add(MESSAGE_TOP, MTXT__WARNING_INCOMING_ATAM, RED_15,
                      MESSAGE_FLASH, 24, MESSAGE_NOCURSOR);
      ...
      case WEAPON_ECM:
        ...
        make_sound(ship->x_pos, ship->y_pos, ship->z_pos,
                   SMALL_EXPLOSION_SOUND);

        sprintf(ecm_message, "%s - %ld %s", message_decode(MTXT__ECM_FIRED, 0),
                (ship->performance->weapons[ship->fire_request]),
                message_decode(MTXT__REMAINING, 0));
        message_text(MESSAGE_BOTTOM, ecm_message, RED_15, MESSAGE_SCROLL, 32,
                     MESSAGE_CURSOR2);

        while ((temp_ship->header).status == 1) {
          if ((temp_ship->type >> 4) == WEAPON &&
              temp_ship->type != MEGA_BOMB && temp_ship->type != WING_POD) {
            temp_ship->shields = 0;
          }
        ...
        for (loop = 0; loop < 64; loop += 1) {
          add_smoke(
              ship->x_pos, ship->y_pos, ship->z_pos,
  ...
  if (((ship->type) >> 4) == PLAYERS_SHIP &&
      ship->fire_request != WEAPON_BEAM_LASER &&
...
check_shippy_collision(ship_stack *ship)
{
  ...
  height_temp = find_ground_height(
      ((ship->x_pos) - (sine_table[(ship->x_rot) >> 10] << 11)),
  ...
  new_rot = (((find_rotation(-height_temp, (1 << 13))) << 1) - ship->y_dir) &
            ROT_LIMIT;
...

{
  ...
  if (satellite->fire_request != WEAPON_NOTHING) {
    ...
    target =
        (ship_stack *) ((ship_sdb *) satellite->special_data)->command_address;
    ...
    if (dist < (32 * (1 << 24))) {
      aim_laser(
          satellite->x_pos, satellite->y_pos, satellite->z_pos, target->x_pos,
          target->y_pos, target->z_pos, satellite->x_vel, satellite->y_vel,
          satellite->z_vel, target->x_vel, target->y_vel, target->z_vel,
          (satellite->performance)->weapons[WEAPON_LASER], (long) satellite);
    }
  }
...
void
parachute_control(ship_stack *parachute)

{
  ...
  if (parachute->what_hit_me != 0) {
    parachute_collision(parachute);
  }

  // check for collisions with ground objects n' ground
  coll_check =
      check_collision(parachute->x_pos, parachute->y_pos, parachute->z_pos);
...

{
  ...
  if (car->type == (CAR << 4)) {
    ...
    // calculate the x,y,z velocities based on x , y direction and speed
    car->x_vel = -(
        ((sine_table[(car->x_rot) >> 10] * cosine_table[(car->y_rot) >> 10]) >>
         12) *
        (car->speed));

    car->y_vel = -(((cosine_table[(car->x_rot) >> 10] *
                     cosine_table[(car->y_rot) >> 10]) >>
                    12) *
                   (car->speed));
    ...
    height_temp = find_ground_height(
        ((car->x_pos) - (sine_table[(car->x_rot) >> 10] << 11)),
        ((car->y_pos) - (cosine_table[(car->x_rot) >> 10] << 11)));

    // Find the relative height diff between height at front and middle
    height_temp = (height_temp - car->z_pos) >> 10;

    // Find the y rotation based on the height and find the relative rot between
    // real and ideal
    rot_temp = (find_rotation(-height_temp, (1 << 13))) - car->y_rot;
    if (rot_temp > 512 * 1024) {
      rot_temp -= 1024 * 1024;
    }
    if (rot_temp < -512 * 1024) {
      rot_temp += 1024 * 1024;
    ...
    height_temp = find_ground_height(
        ((car->x_pos) - (cosine_table[(car->x_rot) >> 10] << 11)),
...

  {
    ...
    if (car->fire_request != WEAPON_NOTHING) {
      ...
      if (dist < (32 << 24)) {
        aim_laser(car->x_pos, car->y_pos, car->z_pos + (1 << 23), ship->x_pos,
                  ship->y_pos, ship->z_pos, car->x_vel, car->y_vel, car->z_vel,
                  ship->x_vel, ship->y_vel, ship->z_vel,
                  (car->performance)->weapons[WEAPON_LASER], (long) car);
      }
    }
  }
}
...
void
big_ship_control(ship_stack *ship)

{
  ...
  switch ((ship->special_data)->command_override) {
    ...
    case BIG_SHIP_COMMAND_NORMAL:
      ...
      // calculate the x,y,z velocities based on x , y direction and speed
      ship->x_vel = -(((sine_table[(ship->x_rot) >> 10] *
                        cosine_table[(ship->y_rot) >> 10]) >>
                       12) *
                      (ship->speed));

      ship->y_vel = -(((cosine_table[(ship->x_rot) >> 10] *
                        cosine_table[(ship->y_rot) >> 10]) >>
                       12) *
                      (ship->speed));
  ...
  if (ship->can_see &&
    ...
    if (ship->type != ((BIG_SHIP << 4) + 2)) {
      ...
      thruster_header =
          (thruster_details_header *) (details + (ship->type))->thruster_data;
      ...
      if (thruster_header != NULL) {
        ...
        for (loop = 0; loop <= thruster_header->counter; loop += 1) {
          ...
          add_smoke((ship->x_pos) + node_data.x_pos,
                    ...
                    (ship->z_pos) + node_data.z_pos, -ship->x_vel, -ship->y_vel,
                    -ship->z_vel, BIG_THRUSTER_SMOKE, 0);
          thruster_data += 1;
        }
      }
    }
    ...
    if (ship->shields < 128) {
      smoke_header =
          (smoke_details_header *) (details + (ship->type))->smoke_data;
  ...
  if (ship->fire_request == WEAPON_LASER && ship->counter == 0 &&
      ship->target != NULL) {
    laser_header =
        (laser_details_header *) (details + (ship->type))->laser_data;
    ...
    for (loop = 0; loop <= laser_header->counter; loop += 1) {
      ...
      if (laser_type >= LASER_POWER_6) {
        laser_type = LASER_POWER_6;
      }
      ...
      aim_laser(ship->x_pos + node_data.x_pos, ship->y_pos + node_data.y_pos,
                ship->z_pos + node_data.z_pos, target_ship->x_pos,
                target_ship->y_pos, target_ship->z_pos, ship->x_vel,
                ship->y_vel, ship->z_vel, target_ship->x_vel,
                target_ship->y_vel, target_ship->z_vel, laser_type,
                (long) ship);
      laser_data += 1;
    }
    ship->counter = 2;
  ...
  if (ship->fire_request == WEAPON_ATA && ship->counter == 0 &&
      ship->target != NULL) {
    missile_header =
        (missile_details_header *) (details + (ship->type))->missile_data;
    if (missile_header != NULL) {
      ...
      for (loop = 0; loop <= missile_header->counter; loop += 1) {
        ...
        node_data.z_rot = ship->z_rot;
        rotate_node_from_c(&node_data);
        add_sam(ship->x_pos + node_data.x_pos, ship->y_pos + node_data.y_pos,
                ship->z_pos + node_data.z_pos, (ship_stack *) ship->target,
                (long) ship);
        missile_data += 1;
      }
  ...
  // if ( (players_ship->fire_request)!=WEAPON_NOTHING )
  {
    ...
    if (ship_data != NULL) {
      ...
      rotate_node_from_c(&node_data);
      pleb_ship = add_ship(
          ship->x_pos + node_data.x_pos, ship->y_pos + node_data.y_pos,
          ship->z_pos + node_data.z_pos,
          ((ship->x_rot) - 512 * 1024) & ROT_LIMIT, (SMALL_SHIP << 4) + 3,
          &pleb_special_data, &mission.performances[3 + 1]);
      if (pleb_ship != NULL) {
...

{
  ...
  if (ship != NULL) {
    ...
    memset((void *) (((char *) ship) + (sizeof(link_header))), 0,
           (sizeof(ship_stack)) - (sizeof(link_header)));

    ship->x_pos = x_pos;
    ship->y_pos = y_pos;
    ship->z_pos = z_pos;
    ship->x_rot = (x_rot & ROT_LIMIT);
    ship->y_rot = 0;
    ship->z_rot = 0;
    ship->type = type;
    ...
    if ((type >> 4) == SMALL_SHIP) {
      test_the_fucking_fighters += 1;
    }
    ...
    // #define	SDB_MODE_ATTACK				0
    // // Ship control modes #define SDB_MODE_FLIGHTPATH 1 #define
    // SDB_MODE_FORMATION 			2
