...
long SAMPLER_INITIALISED =
         0, // Current status of sound player (1 = initialised & ready)
...
static char *gainname_left[] = {"LeftGain0", "LeftGain1", "LeftGain2",
                                "LeftGain3", "LeftGain4", "LeftGain5",
                                "LeftGain6", "LeftGain7"};

static char *gainname_right[] = {"RightGain0", "RightGain1", "RightGain2",
                                 "RightGain3", "RightGain4", "RightGain5",
                                 "RightGain6", "RightGain7"};

static char *channelname[] = {"Input0", "Input1", "Input2", "Input3",
...
/************************************************************************************************************************************
 *										       MAIN
 *SOUND INIT / TERMINATE CONTROL CODE
 **
 ************************************************************************************************************************************/

void
sound_loadsamples(void)
...

{
  ...
  sample_list sample_files[] = {
      ...
      {100, "Missile.aiff"},   {100, "Thud.aiff"},
      {150, "Engine.aiff"},    {100, "Beep.aiff"},
      {100, "Click.aiff"},     {100, "Explosion.aiff"},
      {150, "Wind.aiff"},      {100, "Chime.aiff"},
      {100, "LockOn.aiff"},    {100, "PickUp.aiff"},
      {100, "Incoming.aiff"},  {100, "Alarm.aiff"},
      {100, "BeamLaser.aiff"}, {0, NULL},
  };
  ...
  while (sample_files[SAMPLES_LOADED].name != NULL) // Load in list
  {
    ...
    sprintf(sample_file, "%s%s%s", RESOURCES_ROOT, SAMPLE_ROOT,
            sample_files[SAMPLES_LOADED].name);

    // LOAD INDIVIDUAL SAMPLES & STORE DATA

    samples[SAMPLES_LOADED].sample_item =
        LoadSample(sample_file); // Load in laser sample & store item
    samples[SAMPLES_LOADED].priority =
        sample_files[SAMPLES_LOADED].priority; // Store priority of sample
    SAMPLES_LOADED++;
  }
}
...
/************************************************************************************************************************************
 *										      SAMPLE
 *PLAYING INIT / TERMINATE CONTROL CODE
 **
 ************************************************************************************************************************************/
...

{
  ...
  for (loop = 0; loop < MAX_CHANNELS; loop++) {
    instruments[loop] =
        LoadInstrument(SOUND_INSTRUMENT, 0, 100); // Load instrument
    ConnectInstruments(instruments[loop], "Output", mixer,
                       channelname[loop]); // Connect instrument to mixer
    attachments[loop].sample_playing =
        -1; // Set attachment for channel to notplaying
  }
...
// Purpose : UnLoads and deletes sample playing instruments + attachments (if
// they exist) Accepts : Nothing Returns : Nothing

{
  ...
  for (loop = 0; loop < MAX_CHANNELS; loop++) {
    DisconnectInstruments(instruments[loop], "Output", mixer,
                          channelname[loop]); // Disconnect instrument
...
long
sound_playsample(long sample, long pitch, long volume, long stereo_pos)

// Purpose : Plays a sample
// Accepts : Sample Number to Play, Pitch and Volume
// Returns : Channel sound is being played on ( 0 .. <MAX_CHANNELS, -1 for sound
// not being played)

{
  ...
  static TagArg Attachment_Tags[] = {
      {AF_TAG_SET_FLAGS,
       (void *)
           AF_ATTF_FATLADYSINGS}, //	STOP INSTRUMENT WHEN ATTACHMENT CLEAR
      {TAG_END, 0}                //	END OF LIST
  };
  ...
  // FIND A FREE CHANNEL (CLEAR ANY THAT ARE STILL MARKED AS 'ACTIVE' BUT
  // FINISHED)

  do {

    if (attachments[channel_search].sample_playing >=
        0) // Is attachment playing a sample ?
    {
      GetAudioItemInfo(instruments[channel_search], &Query_Tags[0]);
      if ((long) Query_Tags[0].ta_Arg == AF_STOPPED) {
        DetachSample(
            attachments[channel_search]
                .attachment_item); // Finished ?	Yes - Delete item
        attachments[channel_search].sample_playing =
            -1; //				Yes - set to nosamp
      }
    }

    if (attachments[channel_search].sample_playing ==
        -1) {                        // Is attachment Not Playing ?
  ...
  while (free_channel == -1 &&
         channel_search < MAX_CHANNELS); // Until finished or channel found
  ...
  attachments[free_channel].attachment_item = AttachSample(
      instruments[free_channel], samples[sample].sample_item, NULL);
  attachments[free_channel].sample_playing = sample;
  SetAudioItemInfo(attachments[free_channel].attachment_item,
                   &Attachment_Tags[0]);
  ...
  Sample_Tags[0].ta_Arg =
      (void *) SAMPLE_MASTERVOL; // Set to max volume, (volume is now adjusted
                                 // using left & right gain scales)
...
long
sound_stopsample(long channel_number)
...

{
  ...
  // Check to see if the instrument has finished, but just not been de-linked in
  // flags else Release it

  GetAudioItemInfo(instruments[channel_number], &Query_Tags[0]);
  if ((long) Query_Tags[0].ta_Arg == AF_STOPPED) {
    DetachSample(attachments[channel_number]
                     .attachment_item); // Finished ?	Yes - Delete item
    attachments[channel_number].sample_playing =
        -1; // & Register channel as not playing
  } else {
    StopInstrument(instruments[channel_number],
                   NULL); // If unfinished, release it
    // ReleaseInstrument  (instruments [channel_number], NULL);
...
long
sound_pitchbend(long channel_number, long pitch_bend)

// Purpose : Sets the pitch bend value of instrument (<<16) - 65536 is NORMAL
// FREQUENCY Accepts : Channel number to bend pitch of, and new pitch bend value
...
void
sound_alter(long channel_number, long new_volume, long new_stereoposition)

// Purpose : Sets a new volume & stereo position for a sample playing on a
// channel Accepts : Channel number, volume& stereo position Returns : nothing
...
void
sound_setmastervolume(long new_mastervolume)
...
void
sound_switchonoroff(long new_switchstatus)
