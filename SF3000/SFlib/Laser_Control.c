...

{
  ...
  while ((laser->header).status == 1) {
    ...
    if (temp_long != 0 && laser->who_owns_me != temp_long) {
      add_smoke(laser->x_pos, laser->y_pos, laser->z_pos, 0, 0, 0,
                LASER_HIT_SMOKE, 0);
      laser->counter = 0;

      if (bonus_collision_ref >= 2 &&
          laser->who_owns_me == (long) players_ship &&
          (temp_long & 16383) == temp_long) {
        add_bonus_from_collision_box(bonus_collision_ref, temp_long);
      }

      // If the collision check returns a positive value then this means you
      // have hit a static ground object with the address offset returned Call
      // static_explode to update the objects hit counter and blow up and delete
      // the object. Call with object ref and damage ( Max hits counter 254 -
      // average 100)
      if (temp_long > 0) {
...
void
aim_laser(long x_pos, long y_pos, long z_pos, long x_aim, long y_aim,
          long z_aim, long your_x_vel, long your_y_vel, long your_z_vel,
          long target_x_vel, long target_y_vel, long target_z_vel, long type,
          long ref)
{
  ...
  add_laser(x_pos, y_pos, z_pos, your_x_vel, your_y_vel, your_z_vel,
            target.x_rot, target.y_rot, type, ref, LASER_TYPE_GUN_BASE);
}

void
add_laser(long x_pos, long y_pos, long z_pos, long x_vel, long y_vel,
          long z_vel, long x_rot, long y_rot, long type, long ref,
          long laser_ref)

{
...

  {
    ...
    if (laser_ref == LASER_TYPE_GUN_BASE) {
      ...
      x_dir = (sine_table[x_rot >> 10] * cosine_table[y_rot >> 10]) >> 11;
      y_dir = (cosine_table[x_rot >> 10] * cosine_table[y_rot >> 10]) >> 11;
      z_dir = (sine_table[y_rot >> 10]) << 1;
    } else {
      // Max value = 4096 ( << 12 = 1 map unit )
      x_dir = (sine_table[x_rot >> 10] * cosine_table[y_rot >> 10]) >> 12;
      y_dir = (cosine_table[x_rot >> 10] * cosine_table[y_rot >> 10]) >> 12;
...
update_beam_lasers()
{
  ...
  for (loop = 0; loop < 32; loop += 1) {
    ...
    target.x_rot -= x_rot;
    if (target.x_rot > 512 * 1024) {
      target.x_rot -= 1024 * 1024;
    }
    if (target.x_rot < -512 * 1024) {
      target.x_rot += 1024 * 1024;
    }
    if (target.x_rot > 128 * 1024) {
      target.x_rot = 128 * 1024;
    }
    if (target.x_rot < -128 * 1024) {
      target.x_rot = -128 * 1024;
    }
    ...
    target.y_rot -= y_rot;
    if (target.y_rot > 512 * 1024) {
      target.y_rot -= 1024 * 1024;
    }
    if (target.y_rot < -512 * 1024) {
      target.y_rot += 1024 * 1024;
    }
    if (target.y_rot > 128 * 1024) {
      target.y_rot = 128 * 1024;
    }
    if (target.y_rot < -128 * 1024) {
      target.y_rot = -128 * 1024;
    }
    ...
    x_rot = (x_rot +
             (cosine_table[((loop << 6) + (beam_laser.x_rot_bend >> 8)) & 1023]
              << 3)) &
            ROT_LIMIT;
    y_rot = (y_rot +
             (sine_table[((loop << 6) + (beam_laser.y_rot_bend >> 8)) & 1023]
              << 3)) &
            ROT_LIMIT;
    ...
    laser->z_pos = z_pos;
    if (laser->z_pos < 0) {
      laser->z_pos = -laser->z_pos;
    }

    x_pos -= ((sine_table[x_rot >> 10] * cosine_table[y_rot >> 10]) << 1);
    y_pos -= ((cosine_table[x_rot >> 10] * cosine_table[y_rot >> 10]) << 1);
  ...
  if (beam_laser.y_rot_bend > (128 * 1024)) {
    beam_laser.y_rot_bend = (128 * 1024);
  }
  if (beam_laser.y_rot_bend < -(128 * 1024)) {
    beam_laser.y_rot_bend = -(128 * 1024);
  }
...
void
small_fighter_lasers(ship_stack *ship)
{
  ...
  // The first point defined is the centre laser the next 2 are left / right
  // wing There are 7 different laser upgrades 0-6 (ship->performance)->lasers

  // Laser Number		Number to fire			Power
  // Total Power
  //		0					1
  //2					1 		1
  //1				3					2 		2
  ...
  // printf("laser count %d\n",laser_header->counter) ;

  // add_small_fighter_laser( laser_details* laser_data , ship_stack* ship ,
  // long power )
...
add_small_fighter_laser(laser_details *laser_data, ship_stack *ship, long power)
{
  ...
  add_laser((ship->x_pos) + node_data.x_pos, (ship->y_pos) + node_data.y_pos,
            (ship->z_pos) + node_data.z_pos, ship->x_vel, ship->y_vel,
            ship->z_vel, ship->x_rot, ship->y_rot, power, (long) ship,
