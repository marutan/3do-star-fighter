...
void
update_mushroom(smoke_stack *smoke)
{
  ...
  for (loop2 = ((7 - smoke->counter) << 1);
       loop2 <= (((7 - smoke->counter) << 1) + 1); loop2 += 1) {
    ...
    for (loop = 0; loop <= counter; loop += 1) {
      ...
      add_smoke(x_pos, y_pos, z_pos, 0, 0, 0, MUSHROOM_SMOKE, loop2 >> 1);
...
void
add_smoke(long x_pos, long y_pos, long z_pos, long x_vel, long y_vel,
          long z_vel, long type, long colour)
...
void
fighter_thrusters(ship_stack *ship)
{
  ...
  for (loop = 0; loop <= thruster_header->counter; loop += 1) {
    ...
    if (ship->thrust_control > (arm_random() & 1023)) {
      ...
      add_smoke((ship->x_pos) + node_data.x_pos,
                ...
                ship->z_vel, SMALL_THRUSTER_SMOKE, (ship->misc_counter) >> 2);
    }
    thruster_data += 1;
  }

  // Inc smoke counter
  // if (ship->misc_counter < ((ship->thrust_control)>>7) ) ship->misc_counter
  // += 1 ;
