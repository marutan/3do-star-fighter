...
BS_CountPeripherals(void)
{
  ...
  /* send the message */
  if ((sent = SendMsg(brokerPortItem, queryMsgItem, &queryHeader,
                      sizeof(queryHeader))) < 0) {
  ...
  if (podlist->pdl_Header.ebh_Flavor != EB_DescribePodsReply) {
    printf("BS_CountPeripherals():  message data was wrong flavor!(0x%x,%d)\n",
           podlist->pdl_Header.ebh_Flavor, podlist->pdl_Header.ebh_Flavor);
        ...
        switch (frame->ef_EventNumber) {
          ...
          case EVENTNUM_MouseUpdate:
            ...
            *(BS_GetMouseRaw(mdp[podI])) =
            ...
            BS_SetMouseDaisyNum(mdp[podI], frame->ef_PodPosition);
            BS_SetMouseNum(mdp[podI], frame->ef_GenericPosition);
            break;
#endif /* __BS_MOUSE_H */
#ifdef __BS_CPAD_H
          ...
          case EVENTNUM_ControlButtonUpdate:
            ...
            BS_SetCPadRawButtons(cpdp[podI],
                                 ((ControlPadEventData *) (frame->ef_EventData))
                                     ->cped_ButtonBits);
            BS_SetCPadDaisyNum(cpdp[podI], frame->ef_PodPosition);
            BS_SetCPadNum(cpdp[podI], frame->ef_GenericPosition);
            break;
#endif /* __BS_CPAD_H */
#ifdef __BS_LGUN_H
          ...
          case EVENTNUM_LightGunFireTracking:
            ...
            *(BS_GetLGunRaw(lgdp[podI])) =
            ...
            BS_SetLGunDaisyNum(sdp[podI], frame->ef_PodPosition);
            BS_SetLGunNum(sdp[podI], frame->ef_GenericPosition);
            break;
#endif /* __BS_LGUN_H */
          ...
          case EVENTNUM_StickMoved:
            ...
            *(BS_GetStickRaw(sdp[podI])) =
            ...
            BS_SetStickDaisyNum(sdp[podI], frame->ef_PodPosition);
            BS_SetStickNum(sdp[podI], frame->ef_GenericPosition);
            break;
...
BS_ConnectEventBroker(void)
{
  ...
  if ((queryMsgItem = CreateItem(MKNODEID(KERNELNODE, MESSAGENODE), msgTags)) <
      0) {
...
BS_WatchingJoyStick(void)
{
  ...
  return config.cr_TriggerMask[0] &
...
BS_WatchingCPad(void)
{
  ...
  return config.cr_TriggerMask[0] &
         (EVENTBIT0_ControlButtonPressed | EVENTBIT0_ControlButtonUpdate |
          EVENTBIT0_ControlButtonArrived | EVENTBIT0_ControlButtonReleased);
}
...
BS_WatchingLightGun(void)
{
  ...
  return config.cr_TriggerMask[0] &
         (EVENTBIT0_LightGunButtonPressed | EVENTBIT0_LightGunButtonReleased |
          EVENTBIT0_LightGunUptdate | EVENTBIT0_LightGunDataArrived |
...
BS_DisconnectBroker(void)
{
  ...
  if (SendMsg(msgPortItem, msgItem, &config, sizeof(config))) {
  ...
  if ((Err) msgPtr->msg_Result < 0) {
    printf(
        "BS_DisconnectBroker(): Event broker could not accept configuration\n");
    return 0;
  }
