 ...
 *stream header is not available and use the default. 10/20/93		jb
 *New today
 *
 *******************************************************************************************/
...
FindAndLoadStreamHeader(DSHeaderChunkPtr headerPtr, char *fileName)
{
  ...
  status =
      AsynchReadBlockFile(&blockFile,              /* block file */
  ...
  DeleteItem(ioReqItem); // Added 16/6/95 to prevent O/S message 'Closing device
                         // w/o del item' - T.P
