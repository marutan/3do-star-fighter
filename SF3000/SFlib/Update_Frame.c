...

{
  ...
  static long view_wot = 0;
  static long dead_player_counter;
  ...
  if (test_mode == 1) {
    ...
    if (camera_y_position > 0) {
      camera_y_position = 0;
    }

    if (camera_y_position < -(32 << 24)) {
      camera_y_position = -(32 << 24);
    }

    if (camera_x_position < -(8 << 24)) {
      camera_x_position = -(8 << 24);
    }
    if (camera_z_position < -(8 << 24)) {
      camera_z_position = -(8 << 24);
    }
    if (camera_x_position > (8 << 24)) {
      camera_x_position = (8 << 24);
    }
  ...
  if (players_ship->shields > 0) {
    // When the player is using a digital input (such a the 3DO joypad /
    // keyboard etc.) use the following code which will slowly build up the
    // control rate and auto centre in a nice kind of fashion

    // If a proportional joystick is being used then set the x_control and
    // y_control to the joystick position (+ve / -ve 1024) thrust_control
    // (0-1024)
    ...
    if (configuration.control_method == 0) {
      ...
      if (keypad & keypad_iomap[configuration.flight_controls[FLIGHT_THRUST]]) {
        // If the thruster is on then increase engine power and limit to max
        // val.
        players_thrust_control += 256;
        if (players_thrust_control > 1024) {
          players_thrust_control = 1024;
        }
      } else {
        ...
        if (players_thrust_control < 0) {
          players_thrust_control = 0;
        }
      }

      if (keypad & ControlRight) {
        // if the current roll is against the control then speed up the control
        // rate and auto centre the controls quicker
      ...
      if (players_x_control > 1024) {
        players_x_control = 1024;
      }
      if (players_y_control > 1024) {
        players_y_control = 1024;
      }
      if (players_z_control > 1024) {
        players_z_control = 1024;
      }
    ...
    // Check the player fire key and set selected weapon to fire if required
    if (keypad &
        keypad_iomap[configuration.flight_controls[FLIGHT_FIRE_WEAPON]]) {
      players_ship->fire_request = status.weapon;
      players_ship->last_fire_request = status.weapon;
    }
  ...
  if (mission.defence_timer <= 0 && players_ship->shields > 0 &&
  ...
  if (players_ship->shields > 0) {
    ...
    if (which_graphics_set != SPACE_GRAPHICS) {
      ...
      if (players_ship->z_pos <
          find_ground_height(players_ship->x_pos, players_ship->y_pos)) {
        players_ship->z_pos =
            find_ground_height(players_ship->x_pos, players_ship->y_pos);
    ...
    if (dead_player_counter < 16) {
      explode_ship_from_collision_box(players_ship, 8, (long) players_ship,
                                      EXPLOSION_STYLE_LOADS_OF_BITS, 255, 1);
    } else {
  ...
  // if ( (arm_random()&63) == 0 )
  //{
  //	add_ship( arm_random(), arm_random() , (120<<24) , 0 , (PARACHUTE<<4) ,
  //0 , 0 );
  //}
  ...
  // make_sound ( players_ship->x_pos , players_ship->y_pos ,
  // players_ship->z_pos , ENGINE_SOUND ) ;
  ...
  if (((arm_random()) & 1023) < planet_info.comet_rate &&
    ...
    add_bit(
        camera_x_position + ((arm_random() & ((64 << 24) - 1)) - (32 << 24)),
        camera_y_position + ((arm_random() & ((64 << 24) - 1)) - (32 << 24)),
        temp_long, (arm_random() & ((1 << 24) - 1)) - (1 << 23),
        (arm_random() & ((1 << 24) - 1)) - (1 << 23), -(1 << 18), 2, 2, 1, 0, 0,
        EXPLOSION_STYLE_COMET);
  }
}
