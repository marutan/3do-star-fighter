...

{
  ...
  char font_file[128];
  char *font_files[] = {"Message", ""};

  while (*font_files[font_handles] != 0) // Load in list
  {

    sprintf(font_file, "%s%s%s", RESOURCES_ROOT, FONT_ROOT,
            font_files[font_handles]);
...
void
font_initialise(TextCel **cel_text)
...

{
  ...
  for (font_loop = 0; font_loop < (FONT_MAXMENU - 1); font_loop++) {
    ((*(cel_text + font_loop))->tc_CCB)->ccb_NextPtr =
        (*(cel_text + font_loop + 1))->tc_CCB;
  }

  // Create message cels

  for (font_loop = 0; font_loop < FONT_MAXMESSAGE; font_loop++) {
    messages[font_loop].textcel =
        CreateTextCel(*(font_list + MESSAGE_FONT), 0, 0, 0);
  }

  // Link message cels together

  for (font_loop = 0; font_loop < (FONT_MAXMESSAGE - 1); font_loop++) {
    (messages[font_loop].textcel->tc_CCB)->ccb_NextPtr =
        messages[font_loop + 1].textcel->tc_CCB;
...

{
  ...
  for (font_loop = 0; font_loop < FONT_MAXMESSAGE;
       font_loop++) { // Delete message text cels
    DeleteTextCel(messages[font_loop].textcel);
  }

  for (font_loop = 0; font_loop < FONT_MAXMENU;
       font_loop++) { // Delete menu entry text cels
    DeleteTextCel(*(cel_text + font_loop));
  }

  for (font_loop = font_handles - 1; font_loop >= 0;
       font_loop--) { // Delete fonts
...
void
font_paint(long font_bc, long font_fc, long font_xp, long font_yp,
           long font_num, long screen_type, char *font_str)

// Purpose : Paints a font immediately on the screen using GCON - THIS IS VERY
// SLOW Accepts : Stuff Returns : Nothing

{
  ...
  if (screen_type == SCR_UPDATE) {
    DrawTextString(*(font_list + font_num), &gcon,
                   screen->sc_BitmapItems[screen->sc_CurrentScreen], font_str);
  } else {
    DrawTextString(*(font_list + font_num), &gcon,
                   screen->sc_BitmapItems[1 - screen->sc_CurrentScreen],
                   font_str);
  }
}
