...
draw_frame(int32_t camera_number)
{
  ...
  if ((camera[camera_number].type) == CAMERA_NORMAL && docked_camera == 0 &&
      camera[camera_number].counter2 == 0) {

    // Get the ships y rotation between positive and negative 512*1024 (+/- 180
    // degrees) and multiply the camera x rotation cosine to make side view
    // level

    // add on the camera y rot and limit the y rotation
    camera_y_rotation =
        ((view_ship->y_rot) + (camera[camera_number].y_rot)) & ROT_LIMIT;

    if (camera_y_rotation > 512 * 1024) {
      camera_y_rotation -= 1024 * 1024;
    }
    ...
    // Move the camera to the left or right based on climb / dive when banked
    // over
    node_data.x_pos =
        ((view_ship->y_control) * (sine_table[(view_ship->z_rot) >> 10])) >> 1;

    // Move the camera back from the ship based on the zoom and duration thrust
    // is used
    node_data.y_pos =
        -(((camera[camera_number].zoom * ship_size) >> 10) + engine_zoom) << 12;
    ...
    node_data.z_pos =
        ((((camera[camera_number].zoom * ship_size) >> 10) >> 2) *
         (cosine_table[((camera[camera_number].y_rot) & ROT_LIMIT) >> 10])) +
        (((view_ship->y_control) * (cosine_table[(view_ship->z_rot) >> 10])) >>
         3);
    ...
    if ((view_ship->type >> 4) == BIG_SHIP) {
      ...
      // over
      node_data.x_pos =
          ((view_ship->y_control) * (sine_table[(view_ship->z_rot) >> 10])) >>
          2;

      // Move the camera back from the ship based on the zoom and duration
      // thrust is used
      node_data.y_pos =
          -(((camera[camera_number].zoom * ship_size) >> 10) + engine_zoom)
          << 12;
      ...
      node_data.z_pos =
          ((((camera[camera_number].zoom * ship_size) >> 10) >> 2) *
           (cosine_table[((camera[camera_number].y_rot) & ROT_LIMIT) >> 10])) +
          (((view_ship->y_control) *
            (cosine_table[(view_ship->z_rot) >> 10])) >>
           4);
    }
    ...
    camera_x_rotation = ((512 + ((camera[camera_number].x_rot) >> 10) +
                          ((view_ship->x_rot) >> 10)) &
                         1023);
    camera_y_rotation = camera_y_rotation >> 10;

    // Limit the rate of change of x rot when on external cam to avoid captain
    // flipper
    ...
    // node_data.x_rot =
    // ((view_ship->x_rot)+(camera[camera_number].x_rot))&ROT_LIMIT ;
    ...
    if ((view_ship->type >> 4) == BIG_SHIP ||
      ...
      camera_z_rotation =
          (sine_table[(view_ship->z_rot) >> 10] *
               cosine_table[((camera[camera_number].x_rot) & ROT_LIMIT) >>
                            10] >>
           19) &
          1023;
    } else {
  ...
  if ((camera[camera_number].type) == CAMERA_TRACKING ||
      (camera[camera_number].type) == CAMERA_FLYBY || docked_camera != 0 ||
      camera[camera_number].counter2 > 0) {
    ...
    if ((camera[camera_number].type) == CAMERA_FLYBY &&
        camera[camera_number].counter2 == 0) {
      if ((camera[camera_number].type) != last_camera ||
        ...
        if (((view_ship->type) >> 4) == BIG_SHIP) {
          ...
          camera_x_position =
              (view_ship->x_pos) + ((view_ship->x_vel) << 6) + temp_long;
          ...
          camera_y_position =
              (view_ship->y_pos) + ((view_ship->y_vel) << 6) + temp_long;
          ...
          camera_z_position =
              (view_ship->z_pos) + ((view_ship->z_vel) << 6) + temp_long;
          ...
          camera_x_position =
              (view_ship->x_pos) + ((view_ship->x_vel) << 4) + temp_long;
          ...
          camera_y_position =
              (view_ship->y_pos) + ((view_ship->y_vel) << 4) + temp_long;
          ...
          camera_z_position =
              (view_ship->z_pos) + ((view_ship->z_vel) << 4) + temp_long;
    ...
    if ((camera[camera_number].type) != CAMERA_TRACKING &&
      ...
      node_data.x_pos = 0;
      node_data.y_pos =
          ((1 << 23) + (1 << 22) + (1 << 21)); // - (docked.counter<<21) ;
    ...
    if (camera[camera_number].counter2 > 0) {
      dead_cam_rot += 16;
      if (dead_cam_rot >= 1024) {
        dead_cam_rot -= 1024;
      }

      camera_x_position =
          camera[camera_number].x_pos + (cosine_table[dead_cam_rot] << 15);
      camera_y_position =
          camera[camera_number].y_pos + (sine_table[dead_cam_rot] << 15);
      camera_z_position = camera[camera_number].z_pos + (8 << 24);
    }

    // Are we in space
    if (planet_info.space_mission != 1) {
      // Check that the camera is above the hill level if less than set to min
      // height

      coll_check = find_ground_height(camera_x_position, camera_y_position);
      if (camera_z_position < (coll_check + (1 << 24))) {
        camera_z_position = coll_check + (1 << 24);
      }
    }
  ...
  if ((camera[camera_number].type) == CAMERA_INTERNAL && docked_camera == 0 &&
      camera[camera_number].counter2 == 0) {
    node_data.x_pos = 0;
    node_data.y_pos = 0;
  ...
  if (collision_box_colour_adder < 0) {
    collision_box_colour_adder = 7;
  }
