...
long damage_variables[11][2] = {{WEAPON_LASER, MTXT__LASER},
...
void
bonus_parachute(long display_message)
...

{
  ...
  crystal_awards[award].function(crystal_awards[award].parameter1,
                                 crystal_awards[award].parameter2,
                                 crystal_awards[award].parameter3);

  if (display_message == 1) {
    message_add(MESSAGE_BOTTOM, (MTXT__PARACHUTE_COLLECTED), CYAN_15,
...
void
bonus_addpickup(long pickup_type)
...
{
  long pickup_loop, award_check, pickup_status, crystal_shift;
  ...
  for (award_check = 0; award_check < MAX_AWARDS; award_check++) {
    ...
    do {
      ...
      if (crystal_awards[award_check].award[pickup_loop] == 0) {
        ...
        if (crystal_awards[award_check].award[pickup_loop] !=
            status.pickups[pickup_loop]) {
          pickup_status = 2;
        }
      }
    ...
    if (pickup_status != 2) // IS THIS OUR AWARD ?
    {
      ...
      for (crystal_shift = 3; crystal_shift >= (3 - (pickup_loop + 1));
           crystal_shift--) {
        status.pickups[crystal_shift] =
            status.pickups[crystal_shift - (2 - pickup_loop)];
      }

      // DELETE BOTTOM CRYSTALS

      for (crystal_shift = 0; crystal_shift < (2 - pickup_loop);
           crystal_shift++) {
      ...
      if (crystal_awards[award_check].function(
              crystal_awards[award_check].parameter1,
              crystal_awards[award_check].parameter2,
              crystal_awards[award_check].parameter3) == 1)

      {
        message_add(MESSAGE_BOTTOM, (MTXT__CRYSTAL_AWARD), CYAN_15,
                    MESSAGE_FLASH, 64, MESSAGE_NOCURSOR);
        message_addweapon();
      }
...
/************************************************************************************************************************************
 *													CRYSTAL
 *BONUS AWARD ROUTINES
 **
 ************************************************************************************************************************************/

long
bonus_weapon(long type, long mttx_weapontextnumber, long value)
...

{
  ...
  sprintf(crystal_string, "%s %s : %ld",
          message_decode(mttx_weapontextnumber, 0),
          message_decode(MTXT__UPGRADE, 0), value);
  message_text(MESSAGE_BIG, crystal_string, YELLOW_15, MESSAGE_SCROLL, 64,
               MESSAGE_CURSOR2);
...
long
bonus_wingpod(long value, long nothing1, long nothing2)
...

{
  ...
  if ((players_ship->ref & POD_4) != 0) {
    wing_pod_count += 1; // Check how many of the pod slots are free
  }
  if ((players_ship->ref & POD_3) != 0) {
    wing_pod_count += 1;
  }
  if ((players_ship->ref & POD_2) != 0) {
    wing_pod_count += 1;
  }
  if ((players_ship->ref & POD_1) != 0) {
    wing_pod_count += 1;
  }

  if (wing_pod_count >=
      4) { // if wing_pod_count = 4 then no room for any more pods
    return (0);
  }

  message_add(MESSAGE_BIG, (MTXT__WINGPOD_ADDED), YELLOW_15, MESSAGE_SCROLL, 64,
              MESSAGE_CURSOR2);
...
long
bonus_performance(long value, long mtxt_bonustype, long nothing2)

// Purpose : Increases players engine / control / shields by value clipping at
// max / min Accepts : Nothing Returns : Nothing

{
  ...
  switch (mtxt_bonustype) {
    case MTXT__ENGINE:
      item_pointer = &player_performance
                          .engine; // Get correct pointer for performance adjust
      break;
  ...
  if (*item_pointer <= 1 &&
      value < 0) { // If we are reducing performance & already on min, return
    return (0);
  }

  if (*item_pointer >= 7 &&
      value > 0) { // If we are increasing performance & already on max, return
    return (0);
  }

  // Print message

  sprintf(item_message, "%s %s\n", message_decode(mtxt_bonustype, 0),
          (value < 0) ? message_decode(MTXT__DAMAGED, 0)
                      : message_decode(MTXT__UPGRADE, 0));
  message_text(MESSAGE_BIG, item_message, YELLOW_15, MESSAGE_SCROLL, 64,
               MESSAGE_CURSOR2);
  ...
  *item_pointer += value; // Adjust performance parameter
  ...
  if (players_ship->shields >
      (player_performance.shields << 5)) { // Check for limiting shields
...
long
bonus_fastcrystal(long value, long nothing1, long nothing2)
...
long
bonus_megasetup_start(long value, long nothing1, long nothing2)
...

{
  ...
  message_add(MESSAGE_BIG, (MTXT__MEGASHIP_AWARDED), YELLOW_15, MESSAGE_SCROLL,
              64, MESSAGE_CURSOR2);

  if (players_ship->performance ==
      &player_performance) // Only do this if using normal setup
  {

    players_ship->performance =
        &mission.performances[0]; // Use performance block in missions file
                                  // (with megasetup)
    memcpy(&mission.performances[0], &player_megaperformance,
           sizeof(performance_data)); // Reset mega performance data

    player_performance.weapons[WEAPON_WING_POD] =
        0; // Count up how many wingpods the player had
    ...
    players_ship->shields =
        (players_ship->performance->shields)
        << 5;                 // Give max shields to mega performance setting
...

{
  ...
  players_ship->performance =
      &player_performance; // Use proper performance datablock
  bonus_deletewingpods();  // Delete ships wingpods
  for (loop = 0; loop < player_performance.weapons[WEAPON_WING_POD];
       loop++) { // Put back original number of wingpods
    add_wing_pod(players_ship);
  }
  players_ship->shields =
      (players_ship->performance->shields)
      << 5; // Give max shields to original performance setting
...
{
  ship_stack *ship = (ship_stack *) (ships.info).start_address;
  ship_stack *ship_next, *other_ship;
  ...
  while ((ship->header).status == 1) // Delete all players wingpods
  {
    if (ship->type == WING_POD) {
      ...
      other_ship =
          (ship_stack *)
              ship->who_owns_me; // If so then update the owners pointer things
...
void
bonus_downgrade_ship(long damage_rating)
...

{
  ...
  long stat_todamage, minimum, takeoff, original_value, *stat_pointer;
  ...
  switch (stat_todamage) // Set pointer to correct value to reduce
  {
    ...
    case 7:
      stat_pointer =
          &player_performance.weapons[damage_variables[stat_todamage][0]];
      break;
  ...
  takeoff = (stat_todamage <= 6)
                ? arm_randomvalue((*stat_pointer) >> (3 - damage_rating))
                : damage_rating >> 1;

  if (takeoff <= 0 ||
      *stat_pointer <= minimum) { // Check we are actually taking something off
                                  // & there's something to take off
    return;
  }
  ...
  *stat_pointer -= takeoff;      // Reduce statistic
  ...
  if (*stat_pointer ==
      original_value) { // If clipped & same as original value, return
    return;
  }
  ...
  if (players_ship->shields > 0) {
    sprintf(item_message, "%s %s\n",
            message_decode(damage_variables[stat_todamage][1], 0),
            message_decode(MTXT__DAMAGED, 0));
    message_text(MESSAGE_BIG, item_message, YELLOW_15, MESSAGE_SCROLL, 64,
                 MESSAGE_CURSOR2);
  }
