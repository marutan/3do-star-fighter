...
void
sound_loadsamples(void); // Load in game samples & setup mixer [START OF GAME]
void
sound_unloadsamples(void); // Deletes all samples & kills mixer [END OF GAME]

void sound_initialise(void); // Makes sample player active [MAKE SFX READY]
void sound_terminate(
    void); // Makes sample player inactive [SUSPEND SFX TO FREE DSP MEMORY]
