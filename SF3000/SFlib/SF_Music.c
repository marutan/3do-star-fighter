...
// Alter music filenames in *music_tracks array. Code assumes all music files
// are in the same Directory, (MUSIC_ROOT) this is defined in"SF_Allocation.h"

// Music commands are :	music_initialise ()				// MUST
...
char *music_tracks[] = {"Obie130.aifc", // TRACK LIST
                        "DeathBS.aifc", "PlanetMa.aifc", "Star1.aifc",
                        "Starhed.aifc", "FreeFall.aifc", "Floating.aifc",
                        "Higher.aifc"};

/***************************************************************************************************/

// Music Status Definitions (Combinations of these can be set, ie. REPEAT +
// PLAY)

#define PLAYER_PLAYING                                                         \
  0x1 // Music Status Definitions (Combinations of these can be set, ie. REPEAT
      // + PLAY)
...
long
music_initialise(long sampleinstrumenttoplay)
...

{
  ...
  MUSIC_BUFFERSIZE = 40 * 2048;        // Buffer size [32 blocks of 2k each]
...

{
  ...
  if (MUSIC_PLAYERTASKACTIVE == 1) {
    ...
    WaitSignal(
        MUSIC_SIGNALPARENT); // Wait for player to signal that it's thrown a 7
...
void
music_addtoplaylist(long track_to_add)

// Purpose : Adds a track to the music play list (if it's not already in the
// list) Accepts : Track to add Returns : Nothing

{
  long track_found = -1, loop;
...
void
music_takefromplaylist(long track_to_takeout)
...
{
  long track_found = -1, track_shift = 0, loop;
  ...
  // Have we deleted the one we were listening to ? If so, play next (same
  // position in list)
  ...
  // Have we deleted a track prior to the one we are listening to ? If so, take
  // 1 of playtrack number
...
void
music_play(long music_setormove, long music_trackval, long trackstoplay)

// Purpose : Plays track list with optional repeat
// Accepts : Type of play (MOVE TRACK, OR PLAY FROM TRACK), Track value (START
// FROM OR MOVE BY) Returns : Nothing

{
  ...
  {
    if (music_setormove ==
        MUSIC_PLAYFROMTRACK) {      // ARE WE PLAYING FROM A SPECIFIED TRACK ?
...
{
  if (player_status & PLAYER_PLAYING && MUSIC_TRACK >= 0 &&
...
void
music_setmastervolume(long new_volume)
...
/****************************************************************************************************
 *									VOICE
 *OVER CONTROL ROUTINES
 **
 ****************************************************************************************************/

void
music_playvoice(char *fmt, ...)
...

{
  ...
  music_sampler =
      LoadInstrument(MUSIC_SAMPLERINSTRUMENTS[MUSIC_SAMPLERINSTRUMENTTYPE], 0,
                     100); // Load sample player
  musicsampler_volume = GrabKnob(
      music_sampler, "Amplitude"); // Grab volume knob for sample player
  music_output =
      LoadInstrument(MUSIC_OUTPUTINSTRUMENT, 0, 100); // Load output instrument
  ...
  if (MUSIC_SAMPLERINSTRUMENTTYPE == 0) {
    ...
    ConnectInstruments(music_sampler, "RightOutput", music_output,
                       "InputRight");
  } else {
  ...
  spCreatePlayer(&music_player, music_sampler, MUSIC_BUFFERS, MUSIC_BUFFERSIZE,
                 NULL); // Create sample player
  music_playersignal =
      spGetPlayerSignalMask(music_player); // Get wait signal for player
  ...
  MUSIC_COMMAND = MUSIC_PLAY | MUSIC_PAUSE | MUSIC_RESUME | MUSIC_STOP |
                  MUSIC_TERMINATE | MUSIC_MAKETIME | MUSIC_PLAYVOICE;

  MUSIC_PLAYERTASKACTIVE = 1;     // Set player flag to ACTIVE
  player_status = PLAYER_STOPPED; // Set player status to stopped

  SendSignal(parent_taskref,
             MUSIC_SIGNALPARENT); // Indicate to parent that we have setup
  ...
  do {
    ...
    music_signal = WaitSignal(MUSIC_MAKETIME | MUSIC_PLAY | MUSIC_TERMINATE |
                              MUSIC_PLAYVOICE);
    ...
    while (player_status & PLAYER_PLAYING) {
      ...
      // WAIT FOR SIGNAL (EITHER FROM PLAYER, OR PARENT TASK), THEN CALL MUSIC
      // COMMAND TO SEE IF IT'S A CONTROL COMMAND
      ...
      // IF COMMAND TO SERVICE SOUND PLAYER, THEN UPDATE IT & CHECK SPOOLERS
      // STATUS

      if ((player_status & PLAYER_PLAYING) &&
          (music_signal & music_playersignal)) {
        spService(music_player, music_signal); // Update spooler

        if (MUSIC_MAKETIMEFLAG ==
            1)                    // Is parent waiting to load on buffer fill ?
        {                         // if so, signal them
          MUSIC_MAKETIMEFLAG = 0; // Clear flag
          SendSignal(parent_taskref, MUSIC_SIGNALPARENT); // Let parent carry on
        }

        if (!(spGetPlayerStatus(music_player) &
              SP_STATUS_F_BUFFER_ACTIVE)) // Has player finished track ?
        {
  ...
  DisconnectInstruments(music_sampler, "LeftOutput", music_output,
                        "InputLeft"); // Disconnect instruments
  DisconnectInstruments(music_sampler, "RightOutput", music_output,
                        "InputRight"); // Disconnect instruments
  ...
  SendSignal(parent_taskref,
             MUSIC_SIGNALPARENT); // Indicate to parent that we have terminated
  return;
}
...
void
music_trackgetnext(long track_mover)
...
{
  if (MUSIC_PROGRAMLENGTH <= 0 ||
      MUSIC_TRACKSLEFT == 0) // If no tracks in play list, return
  {
  ...
  if (MUSIC_TRACKSLEFT >
      0) { // Take 1 off number of tracks left to play (if >0)
    MUSIC_TRACKSLEFT -= 1;
  }
  ...
  while (MUSIC_TRACK <
         0) // If track pointer before start of list, wrap around from end
  {
    MUSIC_TRACK += MUSIC_PROGRAMLENGTH;
  }

  while (MUSIC_TRACK >= MUSIC_PROGRAMLENGTH) // If track pointer after end of
                                             // list, wrap around from start
  {
    MUSIC_TRACK -= MUSIC_PROGRAMLENGTH;
  }
...
void
music_loadin(long play_voiceover)
...

{
  ...
  if (play_voiceover == 0) {
    sprintf(music_file, "%s%s%s", RESOURCES_ROOT, MUSIC_ROOT,
            music_tracks[music_loadtrack]); // Set to play track ?
  } else {
    sprintf(music_file, "%s", MUSIC_VOICEOVERTOPLAY); // Or voice over file ?
  }

  result =
      spAddSoundFile(&music_sample, music_player, music_file); // Creates sound
...

{
  ...
  if (MUSIC_PLAYERTASKACTIVE == 1) {
    ...
    if (command & MUSIC_STOP || command & MUSIC_TERMINATE) {
      ...
      player_status = (command & MUSIC_STOP)
                          ? PLAYER_STOPPED
                          : PLAYER_TERMINATED; // Set player status
    }
  ...
  if (command & MUSIC_MAKETIME) {
    ...
    if ((player_status & PLAYER_PLAYING) &&
        (result & SP_STATUS_F_PLAYING)) // Are we playing ?
    {
