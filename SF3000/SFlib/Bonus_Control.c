...

{
  ...
  if (bonus->what_hit_me != 0) {
    bonus_collision(bonus);
  }
...
add_crystal_from_static(long x_grid, long y_grid, long chance)
{
  ...
  switch (chance) {
    ...
    case REF_STATIC_CRYSTAL_CHANCE_VERY_POOR:
      ...
      if (((arm_random()) & 7) == 0) {
        add_bonus(x_pos, y_pos, z_pos, 0, 0, (1 << 22), ((arm_random()) & 1));
      }
    ...
    case REF_STATIC_CRYSTAL_CHANCE_POOR:
      ...
      if ((((arm_random()) & 3) + ((arm_random()) & 1)) == 0) {
        add_bonus(x_pos, y_pos, z_pos, 0, 0, (1 << 22), ((arm_random()) & 3));
      }
    ...
    case REF_STATIC_CRYSTAL_CHANCE_AVERAGE:
      ...
      if (((arm_random()) & 3) == 0) {
        add_bonus(x_pos, y_pos, z_pos, 0, 0, (1 << 22),
    ...
    case REF_STATIC_CRYSTAL_CHANCE_GOOD:
      ...
      if (((arm_random()) & 3) == 0) {
        add_bonus(x_pos, y_pos, z_pos, (1 << 22), 0, (1 << 22),
                  ((arm_random()) & 7));
        add_bonus(x_pos, y_pos, z_pos, -(1 << 22), 0, (1 << 22),
                  ((arm_random()) & 7));
      }
    ...
    case REF_STATIC_CRYSTAL_CHANCE_VERY_GOOD:
      if (((arm_random()) & 1) == 0) {
        add_bonus(x_pos, y_pos, z_pos, (1 << 22), (1 << 22), (1 << 22),
                  ((arm_random()) & 3));
        add_bonus(x_pos, y_pos, z_pos, -(1 << 22), (1 << 22), (1 << 22),
                  ((arm_random()) & 7));
        add_bonus(x_pos, y_pos, z_pos, (1 << 22), -(1 << 22), (1 << 22),
    ...
    case REF_STATIC_CRYSTAL_CHANCE_VERY_VERY_GOOD:
      ...
      add_bonus(x_pos, y_pos, z_pos, -(1 << 22), -(1 << 22), (1 << 22),
                ((arm_random()) & 7));
      add_bonus(x_pos, y_pos, z_pos, 0, 0, (1 << 22), ((arm_random()) & 7));
      break;
  }
}
...
void
update_bonus_crystal_adder(void)
{

  if (bonus_crystal_adder.counter >= 0) {
    if ((bonus_crystal_adder.counter & 31) == 0) {
      add_bonus(bonus_crystal_adder.x_pos, bonus_crystal_adder.y_pos,
                bonus_crystal_adder.z_pos, 0, 0, (1 << 22),
                ((arm_random()) & 7));
    }
    ...
    if (bonus_crystal_adder.counter == 0) {
      poly_map[(unsigned) bonus_crystal_adder.y_pos >> 25]
              [(unsigned) bonus_crystal_adder.x_pos >> 25] = OBJECT_NOTHING;
    }
  }
}
...

{
  ...
  if (bonus != NULL) {
    ...
    memset((void *) (((char *) bonus) + (sizeof(link_header))), 0,
...
add_bonus_from_collision_box(long ref, long grid_ref)
{
  ...
  add_bonus(x_pos, y_pos, z_pos, 0, 0, (1 << 22), ((toggle - (ref - 2)) & 7));
