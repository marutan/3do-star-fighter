	...
	for (charloop = 0; charloop < cel_alphacount; charloop++)
		...
		if (cel_alphamap [charloop].delay > 0)
			...
			if (update == 1)
				...
				cel_alphamap [charloop].x_dist = (cel_alphamap [charloop].x_dist*13) / 16 ;				// Update X Offset
				cel_alphamap [charloop].y_dist = (cel_alphamap [charloop].y_dist*13) / 16 ;				// Update Y Offset
				cel_alphamap [charloop].size_dist = (cel_alphamap [charloop].size_dist*13) / 16 ;		// Update Size Offset
