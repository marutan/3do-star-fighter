...
char alpha_widths[] = {8,  6,  8,  8,  8,  8,  8,  8,  8,  8,  11, 10, 10,
                       10, 9,  9,  11, 10, 4,  7,  11, 9,  12, 10, 12, 9,
                       12, 10, 10, 9,  10, 10, 13, 10, 10, 9,  11, 11, 10};
...
// Purpose : Allocates memory for list of temporary display cels and 16x16,
// 32x32 sprite areas etc. Accepts : external global variable pointers Returns :
// 0 if failed, or 1 if successful

{
  ...
  // Allocate memory for fixed game cels, 32x32 and 16x16 landscape / polygon
  // cels and creation cels

  cel_game = (char *) AllocMem(sizeof(char) * 1024 * CEL_MAXGAME,
                               MEMTYPE_CEL); // Game sprites area
  cel_creation = (char *) AllocMem(sizeof(char) * 264 * (CEL_MAXCREATION),
                                   MEMTYPE_CEL); // Created sprites area
  cel_palette = (char *) AllocMem(
      sizeof(char) * 1024, MEMTYPE_CEL); // Monochrome polygon palette area
  cel_codedpalette = (char *) AllocMem(sizeof(char) * 64,
                                       MEMTYPE_CEL); // 32entry coded palette
  cel_plutdata =
      (char *) AllocMem(sizeof(char) * 64, MEMTYPE_CEL); // Game plut area
  cel_list32 = (char *) AllocMem(sizeof(char) * (1024 * CEL_MAX32),
                                 MEMTYPE_CEL); // Area of 32x32 textures
  cel_list16 = (char *) AllocMem(sizeof(char) * (1024 * CEL_MAX16),
                                 MEMTYPE_CEL); // Area of 16x16 textures

  map512 = (char *) AllocMem(sizeof(char) * (1024 * 192),
                             MEMTYPE_CEL); // Area for 512x512 map
  map128 = (char *) AllocMem(sizeof(char) * (1024 * 12),
                             MEMTYPE_CEL); // Area for 128x128 map
  map32 = (char *) AllocMem(sizeof(char) * (768),
                            MEMTYPE_CEL); // Area for 32x32 map

  // NOTE : map512 (Large Area Map) can also be used temporarily for the storage
  // of gamecels
...
void
cel_rectangle(long xpos, long ypos, long xpos2, long ypos2, long colour,
              long type, long trans)
...

{
  ...
  if (type & REC_BORDER) {
    ...
    cel_quad.y_pos0 = ypos2 - 120;
    cel_quad.x_pos1 = xpos2 - 160;
    cel_quad.y_pos1 = ypos2 - 120;
    cel_quad.x_pos2 = (xpos2 - REC_BORDERSIZE) - 160;
    cel_quad.y_pos2 = (ypos2 + REC_BORDERSIZE) - 120;
    cel_quad.x_pos3 = (xpos + REC_BORDERSIZE) - 160;
    cel_quad.y_pos3 = (ypos2 + REC_BORDERSIZE) - 120;
    ...
    cel_quad.x_pos0 = xpos - 160;
    cel_quad.y_pos0 = ypos2 - 120;
    cel_quad.x_pos1 = (xpos + REC_BORDERSIZE) - 160;
    cel_quad.y_pos1 = (ypos2 + REC_BORDERSIZE) - 120;
    cel_quad.x_pos2 = (xpos + REC_BORDERSIZE) - 160;
    cel_quad.y_pos2 = (ypos - REC_BORDERSIZE) - 120;
    cel_quad.x_pos3 = xpos - 160;
    cel_quad.y_pos3 = ypos - 120;
    cel_quad.shade = REC_SHADE_LEFT;
    arm_addmonocel(&cel_quad, 0, colour, trans);

    // DRAW BOTTOM REC_BORDERSIZE

    cel_quad.x_pos0 = (xpos + REC_BORDERSIZE) - 160;
    cel_quad.y_pos0 = (ypos - REC_BORDERSIZE) - 120;
    cel_quad.x_pos1 = (xpos2 - REC_BORDERSIZE) - 160;
    cel_quad.y_pos1 = (ypos - REC_BORDERSIZE) - 120;
    cel_quad.x_pos2 = xpos2 - 160;
    cel_quad.y_pos2 = ypos - 120;
    cel_quad.x_pos3 = xpos - 160;
    cel_quad.y_pos3 = ypos - 120;
    ...
    cel_quad.x_pos0 = (xpos2 - REC_BORDERSIZE) - 160;
    cel_quad.y_pos0 = (ypos2 + REC_BORDERSIZE) - 120;
    cel_quad.x_pos1 = xpos2 - 160;
    cel_quad.y_pos1 = ypos2 - 120;
    cel_quad.x_pos2 = xpos2 - 160;
    cel_quad.y_pos2 = ypos - 120;
    cel_quad.x_pos3 = (xpos2 - REC_BORDERSIZE) - 160;
    cel_quad.y_pos3 = (ypos - REC_BORDERSIZE) - 120;
    cel_quad.shade = REC_SHADE_LEFT;
    arm_addmonocel(&cel_quad, 0, colour, trans);
  }
...
void
cel_paint(long xpos, long ypos, long shade, long cel)
...
void
cel_setgamearea(long areatype)

// Purpose : Sets the pointer to the gamecels
// Accepts : (0) for standard, (1) for temporary super-large 192k 512x512 map
// area Returns : nothing
...
void
cel_movecharacterby(long char_to_move, long move_x, long move_y)
...
void
cel_getalphastart(alpha_char *alpha_character)

// Purpose : Calculates a starting position & size for a character being added
// from a string, depends on add-mode & count Accepts : Pointer to alpha
// character block Returns : nothing

{
  ...
  switch (cel_alphaaddmode) {
    ...
    case ALPHA_TOP: // Zoom from top of screen ?
      alpha_character->x_dist =
          ((cel_alphaaddmodecount * 16) - alpha_character->end_x_pos);
    ...
    case ALPHA_CIRCLE: // Arrange in a circle ?
      ...
      alpha_character->x_dist = ((((160 * (*(sine_table + ang))) >> 12) + 160) -
                                 alpha_character->end_x_pos);
      alpha_character->y_dist = ((((120 * cosine_table[ang]) >> 12) + 120) -
                                 alpha_character->end_y_pos);
      alpha_character->size_dist = (8192 - alpha_character->end_size);
      break;
...
long
cel_addalphastring(char *alpha_string, long alpha_xpos, long alpha_ypos,
                   long alpha_size)

// Purpose : Adds a string at x,y,size on screen starting from char places
// dictated by alpha add mode Accepts : Pointer to string, string xpos, ypos and
// end size Returns : Number of next character

{
  ...
  while (*alpha_string != 0) {
    ...
    if (*alpha_string >= 'A' &&
        *alpha_string <= 'Z') { // Rendering a character ?
      char_toadd = (*alpha_string) - 55;
    }

    if (char_toadd != 255 &&
        cel_alphacount < MAX_ALPHACHARS) // Is char valid & room for it ?

    {
      ...
      cel_getalphastart(&cel_character);    // Set start points for character,
                                            // depending on add mode
      cel_character.character = char_toadd; // Store char to add
      cel_character.delay =
          (cel_alphadelay++) >> 1; // Set delay prior to update
      memcpy(&cel_alphamap[cel_alphacount++], &cel_character,
             sizeof(alpha_char)); // Add character
...
long
cel_getalphastringlen(char *alpha_string)
...
void
cel_setalphaaddmode(long addmodetype)
...
void
cel_addalphadelay(long alphadelay)
...
void
cel_deleteletters(long num_todelete)
...
void
cel_rendercharactermap(long update)

// Purpose : Renders entire character map list to gamecels
// Accepts : Update pointer (0 = No update, just plot, 1 = Update sizes &
// positions) Returns : nothing

{
  ...
  for (charloop = 0; charloop < cel_alphacount; charloop++) {
    if (cel_alphamap[charloop].delay > 0) {
      ...
      cel_quad.x_pos0 = cel_alphamap[charloop].end_x_pos +
                        cel_alphamap[charloop].x_dist - 160;
      cel_quad.y_pos0 = cel_alphamap[charloop].end_y_pos +
                        cel_alphamap[charloop].y_dist - 120;
      ...
      if (update == 1) {
        ...
        cel_alphamap[charloop].x_dist =
            (cel_alphamap[charloop].x_dist * 13) / 16; // Update X Offset
        cel_alphamap[charloop].y_dist =
            (cel_alphamap[charloop].y_dist * 13) / 16; // Update Y Offset
        cel_alphamap[charloop].size_dist =
            (cel_alphamap[charloop].size_dist * 13) / 16; // Update Size Offset
      }
    }
  }
}
