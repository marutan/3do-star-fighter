...

{
  ...
  while ((bit->header).status == 1) {
    ...
    if (((bit->type) & 3) == 1) {
      add_smoke(bit->x_pos, bit->y_pos, bit->z_pos, ((bit->x_vel) >> 3),
                ((bit->y_vel) >> 3), ((bit->z_vel) >> 3), EXPLOSION_SMOKE, 0);
    }
    ...
    if (coll_check != 0) {
      ...
      if (coll_check > 0) {
        ...
        // Its hit the ground
        armburn_addexplosion((bit->x_pos) >> 24, (bit->y_pos) >> 24,
                             ((bit->x_pos) >> 14) & 3);

        dent_ground(bit->x_pos, bit->y_pos, 1 + ((bit->type) >> 1));

        add_smoke(bit->x_pos, bit->y_pos, bit->z_pos, 0, 0, 0, EXPLOSION_SMOKE,
                  0);
        bit->z_vel = -bit->z_vel;
        bit->counter -= 16;
        ...
        bit->x_vel +=
            (((coll_check - coll_check_2) >> 14) * ((bit->z_vel) >> 14));
...

{
  ...
  velocity = (explosion_velocity >> 1) +
             ((explosion_velocity * ((arm_random()) & 15)) >> 4);
