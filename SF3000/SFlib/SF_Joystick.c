...
BS_StickData *
    joystick_pointers[JOYSTICK_MAXJOYSTICKS]; // Array of pointers to stick data
...
{
  if (JOYSTICK_READERTASKACTIVE ==
      1) { // If reader already running, return success
    return (1);
  }

  JOYSTICK_SIGNALPARENT = AllocSignal(0); // Allocate continue signal
  joystick_taskref = CreateThread("joystick_reader", 101, joystick_thread,
                                  4096); // Start reader thread
...

{
  if (JOYSTICK_READERTASKACTIVE == 1) {
    ...
    WaitSignal(JOYSTICK_SIGNALPARENT); // Wait for reader to signal that it's
                                       // thrown a 7
    FreeSignal(JOYSTICK_SIGNALPARENT); // Deallocate signal
  }
}
...
joystick_thread(void)
{
  ...
  memset(joystick_pointers, 0,
         sizeof(joystick_pointers)); // Set all pointers to joysticks to null
  joystick_pointers[0] =
      &joystick; // Set 1st pointer to point to our joystick data block
  ...
  SendSignal(parent_taskref,
             JOYSTICK_SIGNALPARENT); // Indicate to parent that we have setup
  ...
  while (JOYSTICK_READERTASKACTIVE == 1) {
    ...
    eCode =
        BS_NiceWaitEvent(joystick_pointers, NULL, NULL, NULL); // Wait for event
  ...
  SendSignal(
      parent_taskref,
      JOYSTICK_SIGNALPARENT); // Indicate to parent that we have terminated
}

/**************************************/

void
joystick_command(long command)
...

{
  ...
  joystick_count =
      BS_GetPeripheralCount(BS_STICK); // No. of joysticks currently connected
