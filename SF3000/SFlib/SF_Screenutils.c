...
screen_base(int32_t updateflags)
{
  ...
  if (updateflags & S_IMAGE) {
    CopyVRAMPages(VRAMIOReq,
                  (screen->sc_Bitmaps[screen->sc_CurrentScreen])->bm_Buffer,
                  backdrop, screen->sc_NumBitmapPages, -1);
  }

  if (updateflags & S_FLASH) {
    SetVRAMPages(VRAMIOReq,
                 (screen->sc_Bitmaps[screen->sc_CurrentScreen])->bm_Buffer, 0,
                 screen->sc_NumBitmapPages, -1);
  }
}
...
screen_update(int32_t updateflags)
{
  ...
  if ((updateflags & S_MENU) && menu_status.hidden == 0) {
    DrawCels(screen->sc_BitmapItems[screen->sc_CurrentScreen],
             menu_status.cel_textccb);
  }

  // DRAW GAME INFO TEXT ? (If Also drawing menu, skip first 3 (Score, Weapon
  // Count & Mission Timer)

  if (updateflags & S_INFO) {
    if (updateflags & S_MENU || updateflags & S_DEMO) {
      DrawCels(screen->sc_BitmapItems[screen->sc_CurrentScreen],
               messages[(updateflags & S_MENU) ? 3 : 7].textcel->tc_CCB);
    } else {
      DrawCels(screen->sc_BitmapItems[screen->sc_CurrentScreen],
               messages[0].textcel->tc_CCB);
    }
  }
  ...
  if (updateflags & S_BANK) {
    ...
    DisplayScreen(screen->sc_ScreenItems[screen->sc_CurrentScreen],
                  0);                                        // Swap screen
  ...
  if (updateflags & S_WAITSPACE) {
    WaitVBLDefer(VBLIOReq, DISPLAY_FRAMERATESPACE);
  }
...

{
  ...
  if (screen_type == SCR_UPDATE) {
    FillRect(screen->sc_BitmapItems[screen->sc_CurrentScreen], &GCon,
             &rectangle);
  } else {
    FillRect(screen->sc_BitmapItems[1 - screen->sc_CurrentScreen], &GCon,
             &rectangle);
  }
}
...

{
  ...
  switch (swap_type) {
    case 1:
      FadeToBlack(screen, 20); // SCREENSWAP 1 ?
      break;
    case 2:
      screenswap_blurscreen(); // SCREENSWAP 2 ?
      break;
    case 3:
      screenswap_zoominbox(); // SCREENSWAP 3 ?
      break;
  }
}
...

{
  ...
  switch (swap_type) {
    case 1:
      FadeFromBlack(screen, 20); // SCREENSWAP 1 ?
      break;
  }
...

{
  ...
  screen_fade.ccb_SourcePtr =
      (CelData *) ((screen->sc_Bitmaps[screen->sc_CurrentScreen])->bm_Buffer);
  for (fade_loop = 0; fade_loop < 170; fade_loop += 8) {
    ...
    DrawCels(screen->sc_BitmapItems[1 - (screen->sc_CurrentScreen)],
             &screen_fade);
  }
}
...

{
  ...
  screen_fade.ccb_SourcePtr =
      (CelData *) ((screen->sc_Bitmaps[1 - screen->sc_CurrentScreen])
                       ->bm_Buffer);
  screen_fade.ccb_HDY = 0;
  screen_fade.ccb_VDX = 0;
  screen_fade.ccb_HDDX = 0;
