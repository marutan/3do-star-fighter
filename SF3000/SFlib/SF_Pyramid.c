...
long flash_count, planet_count, pyramid_count, planet_sine, pyramid_pause,
    selector_x, selector_y, pause_description;
...
void
pyramid_initialiselevel(long level_number)

// Purpose : Clears a game level pyramid (if -ve, Clears current pyramid, as in
// parameters.level) Accepts : Nothing Returns : Nothing
...
void
pyramid_initialiseselect(long level_number)
...
long
pyramid_read(long keypad, long level_number)
...

{
  ...
  long sublevel_loop, sublevel_count, mission_loop, planet_size, planet_scalar,
      planet_posscalar, sub_cosine, mission_num, planet_number, temp_selector_x;
  ...
  for (sublevel_loop = 0; sublevel_loop <= pyramids[level_number].sub_levels;
       sublevel_loop++) {
    ...
    for (mission_loop = 0; mission_loop <= sublevel_loop; mission_loop++) {
      ...
      mission_num =
          (sub_cosine >= 512) ? mission_loop : sublevel_loop - mission_loop;
      planet_size =
          (planet_posscalar << 3) - ((*(sine_table + sub_cosine) >> 4) *
                                     (mission_num - (sublevel_loop >> 1)));
      planet_number = sublevel_count + mission_num;

      // CALCULATE PLANETS X,Y POSITION AND SHADE

      cel_quad.x_pos0 = (((((mission_num * planet_posscalar) -
                            ((sublevel_loop * planet_posscalar) >> 1)) /
                           3) *
                          (cosine_table[sub_cosine])) >>
                         12) -
                        (planet_size >> 6);
      cel_quad.y_pos0 = 9 +
                        (-(((sublevel_loop * 8) -
                            ((pyramids[level_number].sub_levels * 8) >> 1)) *
                           planet_posscalar) >>
                         5) -
                        (planet_size >> 6);
      cel_quad.shade =
          (mission_num == selector_x && sublevel_loop == selector_y)
              ? 16
              : 5 + ((planet_scalar >> 5) -
                     ((*(sine_table + sub_cosine) >> 10) *
                      (mission_num - (sublevel_loop >> 1))));
      ...
      if ((mission_loop + sublevel_loop) & 1) {
        arm_addgamecel(
            &cel_quad,
            BASECEL_PYRAMID_PLANETS +
                (((planet_count >> 2) + ((sublevel_loop + mission_num) * 7)) &
                 15),
        ...
        arm_addgamecel(
            &cel_quad,
            BASECEL_PYRAMID_PLANETS +
                ((((sublevel_loop + mission_num) * 7) - (planet_count >> 2)) &
                 15),
            planet_size, planet_size);
      }
      ...
      if (mission_num == selector_x && sublevel_loop == selector_y) {
        arm_addgamecel(&cel_quad,
                       BASECEL_PYRAMID_POINTER + ((pyramid_pause >> 4) & 3),
                       planet_size, planet_size);
      }

      // DRAW NUMBERS ON SCREEN (ONLY UNCOMPLETED)

      if ((flash_count >> 3) > planet_number &&
          pyramids[parameters.level].mission[planet_number] ==
              MISSION_UNFINISHED) {
        cel_quad.x_pos0 += 16;
        cel_quad.y_pos0 += 16;
        ...
        arm_addgamecel(&cel_quad, BASECEL_PYRAMID_NUMBERS + planet_number,
                       planet_size, planet_size);
      }

      // DRAW FLAG IF COMPLETED

      if (pyramids[parameters.level].mission[planet_number] ==
          MISSION_FINISHED) {
        cel_quad.shade = 7;
        arm_addgamecel(&cel_quad, BASECEL_PYRAMID_COMPLETED + sublevel_loop,
                       planet_size, planet_size);
      }
    }
  }
  ...
  if (keypad & (ControlStart | ControlA)) {
    if (pyramids[level_number]
            .mission[return_missionpos(selector_x, selector_y)] ==
        MISSION_UNFINISHED) {
      pyramid_showmission();
  ...
  // NOTE : UP, DOWN & FINISH ARE ONLY AVAILABLE IF CHEAT FEATURE 1 HAS BEEN
  // ENABLED

  if (cheat_feature1 == 1) {
    if ((keypad & ControlUp) && selector_y < 4) {
      selector_y += 1; // Moving up ?
    }
  ...
  if ((keypad & ControlLeft) &&
      selector_x > 0) // Moving Left ? Find next available
  {
    temp_selector_x = selector_x;
    do {
      if (pyramids[level_number]
              .mission[return_missionpos(--temp_selector_x, selector_y)] ==
          MISSION_UNFINISHED) {
  ...
  if ((keypad & ControlRight) &&
      selector_x < selector_y) // Moving Right ? Find next available
  {
    temp_selector_x = selector_x;
    do {
      if (pyramids[level_number]
  ...
  if (selector_x != pyramids[level_number].x_level ||
      selector_y != pyramids[level_number].y_level) {
    pyramids[level_number].x_level =
        selector_x; // Set new x position in pyramid
    pyramids[level_number].y_level =
        selector_y;         // Set new y position in pyramid
...

{
  ...
  if (pyramids[parameters.level].mission[mission_num] ==
    ...
    sprintf(message_decode(menus[MISSION_MENU].title, 1), "- %s -",
            message_decode(60, 3)); // Yes
  } else {
    sprintf(message_decode(menus[MISSION_MENU].title, 1), "- %s -",
            message_decode((parameters.level * 15) + mission_num, 3)); // No
  }
...
long
pyramid_registercompleted(void)
...

{
  ...
  // CHECK TO SEE IF WE HAVE COMPLETED ALL MISSIONS ON SUB-LEVEL, IF SO, MOVE UP
  // & RETURN (1), IF ALL PYRAMID FINISHED, RETURN (2)

  for (x_loop = 0; x_loop <= pyramids[current_level].y_level; x_loop++) {
    if (pyramids[current_level].mission[return_missionpos(
            x_loop, pyramids[current_level].y_level)] != MISSION_FINISHED) {
      all_complete = 0;
    }
  }
