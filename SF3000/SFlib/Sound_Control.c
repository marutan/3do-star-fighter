...
void
make_sound(long x_pos, long y_pos, long z_pos, long type)
{
  long pitch, sample;
  ...
  // ############## bug fizx
  if (sample == SOUND_LASER) {
    if (laser_sound_counter > 1) {
      return;
    }
    laser_sound_counter += 1;
  }

  if (sound_data.volume < 20) {
    return;
  }
...
void
get_sound_details(sound_details *sound_data, long x_pos, long y_pos, long z_pos)
{
  long dist;
  ...
  // Use the cosine of the relative x rot to get the stereo pos (+ve / -ve
  // 16384)
...
update_engine_sounds(void)
{
  ...
  if (sound_small_ship != NULL) {
    ...
    get_sound_details(&sound_data, sound_small_ship->x_pos,
                      sound_small_ship->y_pos, sound_small_ship->z_pos);
    ...
    sound_alter(sound_channel_small_ship, sound_data.volume,
                sound_data.stereo_pos);

    if (pitch_bend > (12 * 1024)) {
      pitch_bend = (12 * 1024);
    }
    if (pitch_bend < -(12 * 1024)) {
      pitch_bend = -(12 * 1024);
    ...
    if (pitch_bend > (48 * 1024) || pitch_bend < -(48 * 1024)) {
      pitch_bend = 0;
    }
    ...
    if (sound_sample_small_ship == SMALL_SHIP_ENGINE_ON) {
      sound_pitchbend(sound_channel_small_ship,
                      (9 * 1024) + (sound_small_ship->thrust_control << 2) +
                          (sound_small_ship->speed << 4) + pitch_bend);
      sound_alter(sound_channel_small_ship, sound_data.volume,
                  sound_data.stereo_pos);
    } else {
      sound_pitchbend(sound_channel_small_ship,
                      (9 * 1024) + (sound_small_ship->speed << 5) + pitch_bend);
      sound_alter(sound_channel_small_ship, sound_data.volume,
                  sound_data.stereo_pos);
    }
  }
  ...
  if (sound_big_ship != NULL) {
    ...
    get_sound_details(&sound_data, sound_big_ship->x_pos, sound_big_ship->y_pos,
    ...
    sound_alter(sound_channel_big_ship, sound_data.volume,
                sound_data.stereo_pos);
    ...
    sound_pitchbend(sound_channel_big_ship,
                    (12 * 1024) + (sound_big_ship->speed << 4) + pitch_bend);
  }
