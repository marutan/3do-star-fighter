...
#define MENU_YOFFS 32 // Y Offset for menu text
...
char pilot_widths[16] = {0,  27, 39, 31, 33, 33, 43, 27,
                         21, 31, 35, 23, 27, 39, 24, 24};
...
menu menus[MAX_MENUS] =

    {
        ...
        {MMNU0 + 0,
         ...
         MAIN_MENU,
         3,
         {{BLUE_PAL, 0, 1}, {RED_PAL, 4, 5}, {GREY_PAL, 7, 7}},
        ...
        {MMNU2 + 0,
         ...
         0,
         {
             ...
             {-1,
              ...
              0,
              ORANGE_15,
              {MMNU2 + 3, MMNU2 + 4, MMNU2 + 5}},
         }},

        // Camera Player Position (3)
        // --------------------------
        ...
        // Configure Menu (11) - NOTE : SELECT LANGUAGE OPTION REMOVED FOR
        // AMERICAN VERSION (16/10/95)
        // -------------------------------------------------------------------------------------------
        ...
        {MMNU14 + 0,
         ...
         MAIN_MENU,
         3,
         {{RED_PAL, 0, 2}, {BLUE_PAL, 4, 5}, {GREY_PAL, 7, 7}},
        ...
        {MMNU15 + 0,
         ...
         CONFIGURE_MENU,
         3,
         {{BLUE_PAL, 0, 1}, {RED_PAL, 3, 4}, {GREY_PAL, 8, 8}},

         4,
         0,
         {{BASECEL_CONTROL_TICK,
           32,
           ITEM_PR,
           ITEM_TE,
           0,
        ...
        {MMNU16 + 0,
         ...
         DEBRIEF_MENU,
         2,
         {{RED_PAL, 0, 1}, {BLUE_PAL, 4, 6}},

         -1,
         0,
         {{-1, -1, -1, ITEM_TN, 0, ITEM_OK, 0, NULL, 0, 0, 0, 0, {0}}}},

        // Set Language Menu (14)
        // ----------------------
...

{
  ...
  // If we are moving up from flightcontrol menu, check there are no clashes, if
  // there are, do not move
...
void
menu_resetmenutext(void)

// Purpose : Clears all menu text cels to save / defrag memory
// Accepts : Nothing
// Returns : Nothing
...
void
menu_enter(long current_menu, long reset_to_top)

// Purpose : Setup menu parameters & text when we enter a new menu (or close all
// menus) Accepts : New menu to enter (-1 to close all), and whether to reset
// cursor position to top (1 = yes) Returns : Nothing

{
  ...
  if (menu_status.current_menu != current_menu) {
    message_initialise(0); // If entering different menu, reset textcels
    menu_status.hidden = 0;

    if (menu_status.current_menu ==
        PAUSE_MENU) { // If returning from pause menu to game, reset # weapon
                      // count textcel
      message_addweapon();
    }
  ...
  switch (current_menu) {
    ...
    case PAUSE_MENU:
      if (menus[PAUSE_MENU].cur_option != 4) {
        menus[PAUSE_MENU].items[4].cur_selection =
            0; // If we're not on 'Quit' then reset it !
        menus[PAUSE_MENU].items[4].item_colour = ORANGE_15;
      }

      menus[PAUSE_MENU].items[0].selectable = (menu_displayformation(0) == 0)
                                                  ? ITEM_NOTOK
                                                  : ITEM_OK; // Form Control ?
      menus[PAUSE_MENU].tick_position =
          (status.current_camera != 0)
              ? status.current_camera
              : -1; // Set tick for menu (1 or 2) depending on camera
      break;

      // If formation control, make formation controls unselectable if control
      // mode is not normal

    case FORMATION_MENU:

      menus[FORMATION_MENU].items[0].cur_selection =
          status.formation_mode; // Reset menu to formation control mode
    ...
    case CONTROLS_MENU:
      ...
      // If control is set to flightstick & flightstick not connected, then set
      // back to joypad
      ...
      menus[CONTROLS_MENU].items[1 - configuration.control_method].selectable =
          ITEM_OK;
      menus[CONTROLS_MENU].items[configuration.control_method].selectable =
          ITEM_OK;
    ...
    case FLIGHTCONTROL_MENU:
      sprintf(message_decode(menus[FLIGHTCONTROL_MENU].items[8].string[0], 1),
              "%s",
              (menu_endflightc(0) == 0) ? message_decode(MMNU19_EXITS + 0, 1)
                                        : message_decode(MMNU19_EXITS + 1, 1));
      break;
    ...
    case JUKEBOX_MENU:
      for (option_draw = 0; option_draw <= 7; option_draw++) {
        menus[JUKEBOX_MENU].items[option_draw].tick_status =
            (configuration.music_tracks[option_draw] == 1) ? ITEM_TM_S
                                                           : ITEM_TM_C;
      }
      break;

    case CONFIGURE_MENU:
      menus[CONFIGURE_MENU].items[2].cur_selection = configuration.video_on;
      break;

    case SOUND_MENU:
      menus[SOUND_MENU].items[0].cur_selection = configuration.music_on;
      menus[SOUND_MENU].items[1].cur_selection = configuration.sound_on;
      break;
  }
  ...
  UpdateTextInCel(cel_text[cel_base], TRUE,
                  message_decode(menus[current_menu].title, 1));
  GetTextCelSize(cel_text[cel_base], &width, &height);
  SetTextCelCoords(cel_text[cel_base], 160 - (width >> 1),
                   (long) menus[current_menu].ypos - 2);
  ...
  if (menus[current_menu].max_option >= 0) {
    for (option_draw = 0; option_draw <= menus[current_menu].max_option;
         option_draw++) {
      ...
      // SHOULD WE SHADE THE ITEM BECAUSE IT'S ONE OF A SET NOT CURRENTLY
      // SELECTED ?

      if (item_draw->tick_status == ITEM_TM_S ||
          item_draw->tick_status == ITEM_TN ||
          (item_draw->tick_status == ITEM_TE &&
           option_draw == menus[current_menu].tick_position)) {
      ...
      UpdateTextInCel(
          ...
          message_decode(item_draw->string[item_draw->cur_selection], 1));

      // ARE WE DISPLAYING FLIGHT CONTROLS OR CAMERAS IN PAUSE MENU (IF SO, DRAW
      // ON LEFT)

      if ((current_menu == FLIGHTCONTROL_MENU && option_draw <= 7) ||
          (current_menu == PAUSE_MENU &&
           (option_draw == 1 || option_draw == 2 || option_draw == 3))) {

        SetTextCelCoords(cel_text[cel_base], 50,
                         MENU_YOFFS + (MENU_YSIZE * item_draw->y_position) +
                             menus[current_menu].ypos);
      } else {
        GetTextCelSize(cel_text[cel_base], &width, &height);
        SetTextCelCoords(cel_text[cel_base], 160 - (width >> 1),
                         MENU_YOFFS + (MENU_YSIZE * item_draw->y_position) +
    ...
    if (current_menu == FLIGHTCONTROL_MENU) {
      for (option_draw = 0; option_draw <= 7; option_draw++) {
        ...
        UpdateTextInCel(
            cel_text[cel_base], TRUE,
            message_decode(MMNU19_CONTROLS +
                               configuration.flight_controls[option_draw] +
                               ((option_draw == 7) ? 7 : 0),
                           1));
        GetTextCelSize(cel_text[cel_base], &width, &height);
    ...
    // Draw camera display options for pause menu ?

    if (current_menu == PAUSE_MENU) {
      for (option_draw = 1; option_draw <= 3; option_draw++) {
        switch (option_draw) {
          case 1: // Play cam type
            item_variable = MMNU3 + camera[1].type;
            break;

          case 2: // Game cam type
            item_variable = MMNU3 + camera[2].type;
            break;

          default: // Game cam view
            item_variable = MMNU4 + camera[2].view;
            break;
        }
        ...
        SetTextCelColor(cel_text[cel_base], 0, WHITE_15);
        UpdateTextInCel(cel_text[cel_base], TRUE,
                        message_decode(item_variable, 1));
        GetTextCelSize(cel_text[cel_base], &width, &height);
        SetTextCelCoords(cel_text[cel_base], 270 - width,
                         MENU_YOFFS + (MENU_YSIZE * item_draw->y_position) +
                             menus[current_menu].ypos);
  ...
  if (current_menu == DEBRIEF_MENU) {
    for (option_draw = 0; option_draw <= 4; option_draw++) {
      ...
      SetTextCelColor(cel_text[cel_base], 0,
                      (option_draw < 2) ? WHITE_15 : GREEN_15);
      UpdateTextInCel(cel_text[cel_base], TRUE,
                      message_decode(MMNU16_INFO + option_draw, 1));
      SetTextCelCoords(cel_text[cel_base], 50,
                       MENU_YOFFS + (MENU_YSIZE * item_ypos) +
                           menus[current_menu].ypos);
      (cel_text[cel_base]->tc_CCB)->ccb_Flags &= ~CCB_LAST;
      cel_base++;
...
long
menu_update(long keypad, long keypad_constant)
...

{
  ...
  // UNHIDE MENU ?
  if (!(keypad_constant & menu_status.hidden)) {
    menu_status.hidden = 0;
  }
  ...
  switch (menu_status.current_menu) {
    case PLAYERPOS_MENU:
      menu_updatecamerapos(keypad_constant, keypad,
                           1); // Update PLAYER camera position menu ?
      break;
    case GAMEPOS_MENU:
      menu_updatecamerapos(keypad_constant, keypad,
                           2); // Update GAME camera position menu ?
      break;
    case SOUND_MENU:
      menu_updatesoundmenu(
          keypad, current_menu->cur_option); // Update Sound & Music menu ?
      break;
    case JUKEBOX_MENU:
  ...
  if (current_menu->max_option >= 0) {
    ...
    item_update =
        &(current_menu->items[current_menu->cur_option]); // Current selection
    ...
    if ((keypad & ControlA) && item_update->selectable == ITEM_OK) {
      ...
      if (current_menu == &menus[menu_status.current_menu]) {
        menu_enter(menu_status.current_menu,
                   0); // Re-do menu (IF WE HAVEN'T JUST CHANGED)
      }

      if (menu_status.current_menu != PAUSE_MENU) {
        return (item_update->command); // Return (Except if pause menu, in which
                                       // case, carry on)
      }
    }
  ...
  // MAIN POST - UPDATE SWITCH

  switch (menu_status.current_menu) {
    case PAUSE_MENU:
      menu_updateviewpresets(keypad, keypad_constant);
      break;
  }
...
void
menu_display(long title_background)

// Purpose : Renders shaded boxes and menu pointer to the screen cel list, prior
// to display Accepts : Nothing Returns : Nothing

{

  long item_width, item_halfwidth, menu_displayloop, look_option;
  ...
  if (menu_status.hidden == 0) {
    ...
    cel_rectangle(25, current_menu->ypos + 16, 319 - 25, current_menu->ypos - 8,
                  title_background, REC_BORDER | REC_BOX, 0);

    // DRAW BACKGROUND BORDER SHADES FOR REST OF MENU
    for (menu_displayloop = 0; menu_displayloop < current_menu->border_count;
         menu_displayloop++) {
      cel_rectangle(40,
                    current_menu->ypos +
                        (MENU_YSIZE *
                         current_menu->borders[menu_displayloop].option_end) +
                        MENU_YOFFS + 17,
                    319 - 40,
                    current_menu->ypos +
                        (MENU_YSIZE *
                         current_menu->borders[menu_displayloop].option_start) +
                        MENU_YOFFS - 7,
                    current_menu->borders[menu_displayloop].colour,
                    REC_BORDER | REC_BOX, 1);
    }

    // HIGHLIGHT CURRENT SELECTION ?
    if (current_menu->max_option >= 0) {
      look_option = current_menu->cur_option;
      ...
      if ((menu_status.current_menu == FLIGHTCONTROL_MENU &&
           look_option <= 7) ||
          (menu_status.current_menu == PAUSE_MENU &&
           (look_option == 1 || look_option == 2 || look_option == 3))) {

        cel_rectangle(
            40,
            current_menu->ypos + (MENU_YSIZE * item_display->y_position) +
                MENU_YOFFS + 17,
            50 + item_width,
            current_menu->ypos + (MENU_YSIZE * item_display->y_position) +
                MENU_YOFFS - 7,
            RED_PAL, REC_BORDER | REC_BOX, 1);
      } else {
        cel_rectangle(
            160 - item_halfwidth,
            current_menu->ypos + (MENU_YSIZE * item_display->y_position) +
    ...
    if (current_menu->tick_position >= 0) {
      ...
      if (item_display->item_icon >= 0 &&
          (item_display->item_plottype & ITEM_PR)) {
        cel_quad.x_pos0 = (menu_status.current_menu == PAUSE_MENU) ? -20 : 70;
        arm_addgamecel(&cel_quad, item_display->item_icon, 1024, 1024);
      }

      if (item_display->item_icon >= 0 &&
          (item_display->item_plottype & ITEM_PL)) {
    ...
    for (menu_displayloop = 0; menu_displayloop <= current_menu->max_option;
         menu_displayloop++) {
      item_display = &current_menu->items[menu_displayloop];
      if (item_display->tick_status == ITEM_TM_S ||
          (item_display->item_plottype & ITEM_PA)) {
        cel_quad.y_pos0 =
            (current_menu->ypos + (MENU_YSIZE * item_display->y_position)) +
            MENU_YOFFS - 3 - 120;
...
void
menu_setlevel(long level_select)
...
{
  if (confirmed == 1) {
    menus[PAUSE_MENU].items[4].item_colour =
        WHITE_15; // Set 'Confirm' colour to white
  } else {
...
/***************************************************************************************************/
/*									   SET
 * CAMERA-VIEW MENU FUNCTIONS */
/***************************************************************************************************/

void
menu_updateviewpresets(long keypad, long keypad_constant)

// Purpose : Resets up menu according to camera statistics
// Accepts : Selected camera type
// Returns : Nothing

{
  switch (menus[PAUSE_MENU].cur_option) {

    case 1: // PLAYCAM CAMERA TYPE CONTROLS

      if (keypad &
          ControlLeft) // Changing playcam type (ie. tracking / flyby etc.) ?
      {
        if ((--(camera[1].type)) < 0) {
          camera[1].type = 3;
        }
        ...
        message_addcamera(1);
        menu_status.hidden = keypad_constant; // Set to blank menu
      }

      if (keypad &
          ControlRight) // Changing playcam type (ie. tracking / flyby etc.) ?
      {
        if ((++(camera[1].type)) > 3) {
          camera[1].type = 0;
        }
      ...
      if (keypad & ControlA) // Entering move camera menu ?
      {
        control_changecamera(1);
        message_addcamera(1);

        if (camera[1].type == CAMERA_NORMAL) {
          menu_enter(PLAYERPOS_MENU, 1);
          menu_status.hidden = keypad;
        }
      }
      break;

    case 2: // GAMECAM CAMERA TYPE CONTROLS

      if (keypad &
          ControlLeft) // Changing gamecam type (ie. tracking / flyby etc.) ?
      {
        if ((--(camera[2].type)) < 0) {
          camera[2].type = 3;
        }
      ...
      if (keypad &
          ControlRight) // Changing gamecam type (ie. tracking / flyby etc.) ?
      {
        if ((++(camera[2].type)) > 3) {
          camera[2].type = 0;
        }
      ...
      if (keypad & ControlA) // Entering move camera menu ?
      {
        ...
        if (camera[2].type == CAMERA_NORMAL) {
          menu_enter(GAMEPOS_MENU, 1);
          menu_status.hidden = keypad;
        }
      }
      break;

    case 3: // GAMECAM LOOK AT TYPE CONTROLS

      if (keypad & ControlLeft) // Changing gamecam look at type ? (ie. weapon /
                                // enemy etc.) ?
      {
        if ((--(camera[2].view)) < 0) {
          camera[2].view = 3;
        ...
        control_searchcamera(2, 0); // Get 1st instance of this new type
        control_changecamera(2);
        message_addcamera(2);                 // Display new camera message
        menu_status.hidden = keypad_constant; // Set to blank menu
      }

      if (keypad & ControlRight) // Changing gamecam look at type ? (ie. weapon
                                 // / enemy etc.) ?
      {
        if ((++(camera[2].view)) > 3) {
          camera[2].view = 0;
      ...
      if (keypad &
          ControlRightShift) // Do we want to select next instance of type ?
      {
      ...
      if (keypad &
          ControlLeftShift) // Do we want to select next instance of type ?
      {
        control_searchcamera(2, --camera[2].instance);
        control_changecamera(2);
        message_addcamera(2);
  ...
  if (keypad != 0 && menu_status.current_menu == PAUSE_MENU) {
    menu_enter(PAUSE_MENU, 0); // If any keypresses, re-enter menu
  }
}
...
void
menu_updatecamerapos(long keypad_constant, long keypad, long camera_num)

// Purpose : Allow player to position camera around view for either PLAYER or
// GAME camera Accepts : Keypad presses Returns : Nothing

{
  ...
  if (keypad_constant & ControlLeftShift) { // Zoom OUT from view ?
  ...
  if (keypad_constant & ControlRightShift) { // Zoom IN to view ?
                                             // (Zoom speed depends on distance)
  ...
  menu_status.hidden = keypad_constant;
}
...
void
menu_changeformmode(long new_mode)
...
long
menu_displayformation(long formation_display)

// Purpose : Display formation's current positions on screen & Returns number of
// formation Accepts : (0 to just count & return number, 1 to display as well)
// Returns : Nothing

{
  ...
  while ((ship->header).status == 1) {
    if (ship->special_data != NULL) {
      if (ship->special_data->side == SDB_SIDE_FRIEND &&
        ...
        if (ship->special_data->formation_position == 0 &&
            friendly_totalout < 8) {
  ...
  for (form_search = 1; form_search < __WAR_MF; form_search++) {
    if (formation[form_search].occupied != NULL) {
      ...
      cel_quad.y_pos0 += 8 + pilot_toggley;
      cel_quad.x_pos0 +=
          16 - (pilot_widths[formation[form_search]
                                 .occupied->special_data->pilot_name] >>
                1);
      arm_addgamecel(
          &cel_quad,
          BASECEL_PILOTS +
              formation[form_search].occupied->special_data->pilot_name,
          1024, 1024);

      pilot_toggley = 8 - pilot_toggley;
    }
  }
  ...
  for (friendly_loop = 0; friendly_loop < friendly_totalout; friendly_loop++) {
    cel_quad.x_pos0 =
        2 + ((34 * friendly_loop) - ((friendly_totalout * 34) >> 1));
    cel_quad.y_pos0 = 25;
    arm_addgamecel(&cel_quad, BASECEL_FORMATION + 1, 1024, 1024);
    ...
    cel_quad.y_pos0 += pilot_toggley;
    pilot_toggley = 8 - pilot_toggley;

    cel_quad.x_pos0 += 9 - (pilot_widths[names_attack[friendly_loop]] >> 1);
    arm_addgamecel(&cel_quad, BASECEL_PILOTS + names_attack[friendly_loop],
                   1024, 1024);
  }
...
void
menu_formationgroup(long nothing)
...

{
  ...
  ship = (ship_stack *) (ships.info)
             .start_address;         // Get start of ship linked list
  while ((ship->header).status == 1) // While active ship, search
  {
    ...
    if (ship_sdb_ptr != NULL) {                    // Does ship have a SDB ?
      if (ship_sdb_ptr->alive == SDB_SHIP_ALIVE) { // Yes, is it alive ?
        if (ship_sdb_ptr->sdb_type !=
            SDB_TYPE_PLAYER) { // Yes, is it not player ?
    ...
    ship = (ship_stack *) (ship->header)
               .next_address; // Get next ship in linked list
  }
}
...
void
menu_formationattack(long all)

// Purpose : Send friendly formation into attack
// Accepts : Flag to send ALL formation into attack (1), or just enough for a 1
// to 1 attack (0) Returns : Nothing

{
  ...
  for (formation_scan = 0; formation_scan < __WAR_MF;
       formation_scan++) // Scan through formation
  {
    ship = formation[formation_scan].occupied;
    if (ship != NULL) { // Formation pos occupied ?
      if (ship_setmultipletargets(ship, __WAR_SR_AREA,
                                  ship->special_data->side) ==
          1) { // Can we find target ?
        if (war_settoattack(ship, (ship_stack *) ship->target) ==
            1) // Could we attack it ?
        {
          ship->special_data->formation_position =
              0; // Clear it's formation pos in SDB
          formation[formation_scan].occupied =
              NULL; // Clear ref in formation block
        }
      }
    }
  }
...
void
menu_trackset(long track_number)
...
{
  configuration.music_tracks[track_number] =
      1 - configuration.music_tracks[track_number]; // Toggle track status
  control_saveconfigure();                          // Try to save it to NVRAM

  if (configuration.music_tracks[track_number] ==
      0) { // Taking track out of PL ?
    music_takefromplaylist(track_number);
  } else {
    music_addtoplaylist(track_number); // Putting track in PL !
    if (music_query() == -1 &&
        configuration.music_on == 1) {        // Not playing ? Music on ?
...
void
menu_musicset(long music_status)
...

{
  configuration.music_on =
      music_status;        // Set music on / off status in configure file
  control_saveconfigure(); // Try to save it to NVRAM
  if (music_status == 1) {
...
void
menu_soundset(long sound_status)
...
{
  configuration.sound_on =
      sound_status; // Set sound on / off status in configure file
...

{
  ...
  cel_quad.shade = (configuration.music_on == 1)
                       ? 11
                       : 4; // Set shade depending on on / off status
  ...
  cel_quad.shade = (configuration.sound_on == 1)
                       ? 11
                       : 4; // Set shade depending on on / off status
...
void
menu_updatesoundmenu(long keypad, long selection)
...
void
menu_updatejukeboxmenu(long keypad)
...
void
menu_displayjukebox(void)

// Purpose : Displays current music status in jukebox menu (currently playing
// track) Accepts : Nothing Returns : Nothing

{

  long loop, track_no = music_query();
  ...
  for (loop = 0; loop < 8; loop++) {
    ...
    cel_quad.y_pos0 =
        menus[JUKEBOX_MENU].ypos +
        (MENU_YSIZE * menus[JUKEBOX_MENU].items[loop].y_position) + MENU_YOFFS -
        3 - 120;
    cel_quad.x_pos0 = 90;

    if (loop == track_no) {
      arm_addgamecel(&cel_quad, BASECEL_MUSIC_CD + ((menu_countdown >> 1) & 3),
                     1024, 1024);
  ...
  for (loop = 0; loop < MUSIC_PROGRAMLENGTH; loop++) {
    ...
    cel_quad.y_pos0 =
        menus[JUKEBOX_MENU].ypos +
        (MENU_YSIZE * menus[JUKEBOX_MENU].items[track_no].y_position) +
        MENU_YOFFS - 1 - 120;
...
void
menu_loadgame(long slot_toload)
...

{
  ...
  delete_loadfile = cheat_feature3;

  // Load file (if delete feature not set)

  if (cheat_feature3 == 0) {
    if (nvram_load(
            message_decode(menus[LOAD_MENU].items[slot_toload].string[0], 1),
            GAMEFILE_PREFIX, (char *) &save_gamefile, sizeof(gamefile)) == 1) {
      ...
      if (save_gamefile.savefile_version == GAMEFILE_VERSION) {
        ...
        memcpy(&player_performance, &save_gamefile.savefile_performance,
               sizeof(performance_data));
        memcpy(&status, &save_gamefile.savefile_status, sizeof(game_status));
        memcpy(&camera[1], &save_gamefile.savefile_cameras[0],
               sizeof(camera_data));
        memcpy(&camera[2], &save_gamefile.savefile_cameras[1],
               sizeof(camera_data));

        for (loop = 0; loop < 4; loop++) {
          memcpy(&pyramids[loop], &save_gamefile.savefile_pyramids[loop],
                 sizeof(pyramid));
        }

        load_success = MAIN_LOADSTART;
      } else {
        printf("GAMEFILE VERSION IS DIFFERENT - SETTING FLAG TO DELETE. FILE "
               "[%ld], CGFV [%d]\n",
  ...
  if (delete_loadfile == 1) {
    printf("DELETE LOADFILE IS SET - DELETING FILE !\n");
    nvram_delete(
        message_decode(menus[LOAD_MENU].items[slot_toload].string[0], 1),
        GAMEFILE_PREFIX);
  }
...
void
menu_savegamefile(long nothing)

// Purpose : Gets filename to save file as - Then saves file OR enters SAVEGAME
// menu if an existing file needs to be replaced Accepts : Nothing Returns :
// Nothing

{
  menu_getname(user_savefilename, 16, message_decode(MMNU_GETNAME + 2, 1),
               0); // Get filename

  if (user_savefilename[0] == 0) { // Check for null filename
    return;
  }

  if (nvram_directory(SAVE_MENU, 0, 6, ITEM_OK, GAMEFILE_PREFIX) >=
      7) {                    // How many already in ?
    menu_movedown(SAVE_MENU); // Replace file ?
  } else {
    menu_savegame(-1); // Or just save straight
  }
}
...
void
menu_savegame(long slot_tosave)

// Purpose : Deletes any file already in the slot (unless slot < 0), then Save a
// saved game to NVRAM Accepts : Menu item number holding saveslot Returns : (0)
// If failed, (1) If successful

{
  ...
  memcpy(&save_gamefile.savefile_performance, &player_performance,
         sizeof(performance_data));
  ...
  for (loop = 0; loop < 4; loop++) {
    memcpy(&save_gamefile.savefile_pyramids[loop], &pyramids[loop],
           sizeof(pyramid));
  }
  ...
  if (slot_tosave >= 0) {
    printf("RESULT OF NVRAM DELETE IS %ld\n",
           nvram_delete(
               message_decode(menus[SAVE_MENU].items[slot_tosave].string[0], 1),
               GAMEFILE_PREFIX));
  }

  // SAVE FILE TO NVRAM
  printf("RESULT OF NVRAM SAVE IS %ld\n",
         nvram_save(user_savefilename, GAMEFILE_PREFIX, (char *) &save_gamefile,
                    sizeof(gamefile)));
...
void
menu_setcontrol(long control_type)
...

{
  ...
  long keypad, icon_flash = 0, pro_x, pro_y;
  ...
  do {
    switch (calibrate_stage) {
      ...
      case 3:
        ...
        sprintf(message_decode(menus[CALIBRATE_JOYSTICK_MENU].title, 1), "%s",
                message_decode(MMNU20_MODE + 0, 1));
        break;

      case 4:
        sprintf(message_decode(menus[CALIBRATE_JOYSTICK_MENU].title, 1), "%s",
                message_decode(MMNU20_MODE + 1, 1));
        break;

      case 5:
        sprintf(message_decode(menus[CALIBRATE_JOYSTICK_MENU].title, 1), "%s",
                message_decode(MMNU20_MODE + 2, 1));
        break;
    }
    ...
    do {
      ...
      cel_rectangle(-24 + 160, 24 + 120, 24 + 160, -24 + 120,
                    (pro_stick.x >= (-24) + 160 && pro_stick.x <= (24) + 160 &&
                     pro_stick.y >= (-24) + 120 && pro_stick.y <= (24) + 120)
                        ? RED_PAL
                        : BLUE_PAL,
                    REC_BORDER | REC_BOX, 1);
  ...
  configuration.stick_x_min =
      (long) BS_GetStickMinX(&joystick); // Put in config file
...
void
menu_setflightc(long dir_tomove)
...
{
  long control_key =
      configuration.flight_controls[menus[FLIGHTCONTROL_MENU].cur_option] +
      dir_tomove;
...
void
menu_updateflightc(long keypad)
...
void
menu_configdefault(long nothing)
...

{
  ...
  control_initialisecameras();               // Reset default cameras
  ...
  menu_usermessage(USER_DEFAULTCONFIG); // Display 'config reset' message
}

/**************************************/

void
menu_setvideo(long new_videoflag)

// Purpose : Toggles video clips on / off
// Accepts : New video flag
// Returns : Nothing

{
  configuration.video_on = new_videoflag;
  control_saveconfigure(); // Try to save it to NVRAM
}
...
void
menu_setlanguage(long language_toset)
...

{
  ...
  message_initialiselanguagefiles(
      configuration.language); // Initialise correct message files
  control_saveconfigure();     // Try to save it to NVRAM

  if (configure_waiting ==
      0) // Is this 1st menu screen coz couldn't load config file from NVR
  {
...
/***************************************************************************************************/
/*											GET
 * FILENAME / HISCORE NAME 						   */
/***************************************************************************************************/

long
menu_getname(char *filename_ptr, long maxsize, char *get_message,
             long insert_default)

// Purpose : Gets name from user
// Accepts : Pointer to string to put filename in, max size of name, pointer to
// message to display, default name flag Returns : Length of string entered

{

  long charloop, cel_charoff;
  ...
  long cursor_x = 0, cursor_y = 0, nextcharpos = 0,

       flash_count = 0, filename_length = 0, finished = 0;

  long stringlength_new = 0, stringlength_old = 0, string_moveby,
       alpha_filenamestart, shift_loop;
  ...
  for (charloop = 0; charloop < 13; charloop++) {
    ...
    cel_addalphastring(charselect, 160 + cel_charoff + ((charloop - 6) << 4),
                       90, 1024);
  ...
  cel_charoff = -(cel_getalphastringlen(charselect) >> 1);
  cel_addalphastring(charselect, 160 + cel_charoff + ((11 - 6) << 4), 150,
                     1024);

  charselect[0] = 'a'; // Add DONE
  cel_charoff = -(cel_getalphastringlen(charselect) >> 1);
  cel_addalphastring(charselect, 160 + cel_charoff + ((12 - 6) << 4), 150,
                     1024);

  cel_addalphadelay(128);
  cel_setalphaaddmode(ALPHA_CIRCLE);
  cel_addalphastring(
      get_message, 160 - (cel_getalphastringlen(get_message) >> 1), 198, 1024);
  ...
  if (insert_default == 1) {
    ...
    while (*default_pointer != 0) {
      nextcharpos =
          160 + cel_getalphastringlen(filename_ptr) + 2; // Next char pos
      charselect[0] = *default_pointer++; // Get character from default
      *(filename_ptr + filename_length++) =
          charselect[0];                     // Put it in the filename
      *(filename_ptr + filename_length) = 0; // Terminate string with 0
      cel_addalphastring(charselect, nextcharpos, 40,
                         1024); // Add the character
    }
  }
  ...
  do {
    ...
    stringlength_new =
        cel_getalphastringlen(filename_ptr) + 2; // Get length of string now
    if (stringlength_new !=
        stringlength_old) // Should we shift chars along to centre ?
    {
      string_moveby =
          (stringlength_new - stringlength_old); // Get difference in size
      stringlength_old = stringlength_new;       // Copy new length to old

      for (shift_loop = alpha_filenamestart;
           shift_loop < cel_returnnextcharnumber(); shift_loop++) {
    ...
    cel_rectangle(32, 30, 319 - 32, 60, RED_PAL, REC_BORDER | REC_BOX,
                  1); // Top filename
    cel_rectangle(32, 220, 319 - 32, 190, BLUE_PAL, REC_BORDER | REC_BOX,
                  0); // Message
    cel_rectangle(32, 170, 319 - 32, 80, GREY_PAL, REC_BORDER | REC_BOX,
                  1); // Chars
...

    {
      ...
      cel_rectangle(152 + ((cursor_x - 6) << 4), 106 + (cursor_y * 30),
                    155 + ((cursor_x - 6) << 4) + 15, 85 + (cursor_y * 30),
                    BLUE_PAL, REC_BOX, 0); // Cursor
      cel_rectangle(152 + ((cursor_x - 6) << 4), 106 + (cursor_y * 30),
                    155 + ((cursor_x - 6) << 4) + 15, 85 + (cursor_y * 30),
                    RED_PAL, REC_BORDER, 0); // Cursor
    ...
    if (keypad & (ControlUp | ControlDown | ControlLeft |
                  ControlRight)) { // Beep if moved
      sound_playsample(SOUND_BEEP, 60, 80, 0);
    }
    ...
    if (keypad & ControlA) // Selecting letter / icon ?
    {
      ...
      if (cursor_y < 2 ||
          (cursor_y == 2 && cursor_x < 11)) // ARE WE ENTERING A LETTER ?
      {
      ...
      if (cursor_y == 2 && cursor_x == 11 &&
          filename_length > 0) // ARE WE DELETING A LETTER ?
      {
...
void
menu_setpilotname(long cheat)

// Purpose : Allows player to enter default pilots name or select cheat
// passwords Accepts : nothing Returns : nothing

{
  char pilot_entry[24];

  if (cheat == 1) {
    menu_getname(pilot_entry, 16, message_decode(MMNU_GETNAME + 0, 1),
                 0); // Are we entering a cheat word ?

    if (strcmp(pilot_entry, message_decode(MMNU_CHEATS + 0, 1)) ==
        0) { // CHECK FOR CHEAT 1 - PYRAMID MOVE
      cheat_feature1 = 1;
    }

    if (strcmp(pilot_entry, message_decode(MMNU_CHEATS + 1, 1)) ==
        0) { // CHECK FOR CHEAT 2 - MEGASHIP
      memcpy(&player_performance, &player_megaperformance,
             sizeof(performance_data));
    }

    if (strcmp(pilot_entry, message_decode(MMNU_CHEATS + 2, 1)) ==
        0) // CHECK FOR CHEAT 3 - SELECT LEVEL
    {
      menus[MAIN_MENU].max_option = 5;
      menus[MAIN_MENU].borders[0].option_end = 2;
    }

    if (strcmp(pilot_entry, message_decode(MMNU_CHEATS + 3, 1)) ==
        0) { // CHECK FOR CHEAT 4 - 99 LIVES
      cheat_feature2 = 1;
    }

    if (strcmp(pilot_entry, message_decode(MMNU_CHEATS + 4, 1)) ==
        0) // CHECK FOR CHEAT 5 - STRAIGHT INTO DEMO
    {
      do {
        game_demo();
    ...
    if (strcmp(pilot_entry, message_decode(MMNU_CHEATS + 5, 1)) ==
        0) // CHECK FOR CHEAT 6 - PLAY ALL VIDEOS
    {
      do {
        video_play("StarLogo");
        video_play("Intro");
    ...
    if (strcmp(pilot_entry, message_decode(MMNU_CHEATS + 6, 1)) ==
        0) { // CHECK FOR CHEAT 7 - LOAD DELETES NVRAM FILES
      cheat_feature3 = 1 - cheat_feature3;
    }

    if (strcmp(pilot_entry, message_decode(MMNU_CHEATS + 7, 1)) ==
        0) { // CHECK FOR CHEAT 8 - SHOWS ALL BONUSES ON MAP
      cheat_feature4 = 1 - cheat_feature4;
    }

    if (strcmp(pilot_entry, message_decode(MMNU_CHEATS + 8, 1)) ==
        0) { // CHECK FOR CHEAT 9 - DISPLAY CREDITS
      game_missionstart(1);
    }

  } else {

    // NOTE - IF LANGUAGE SELECTION IS PUT BACK IN, CHANGE ITEMS [2] TO ITEMS
    // [3] TO SET CHEAT BECAUSE OF THE EXTRA INSERTED LINE IN THE MENU.

    menu_getname(pilot_entry, 16, message_decode(MMNU_GETNAME + 1, 1),
                 1); // Or entering default name ?
    if (strcmp(pilot_entry, message_decode(MMNU_CHEATS + 9, 1)) ==
        0) { // Was default name cheat activator ?
      menus[CONFIGURE_MENU].items[3].function_value =
          1; // Yes - from now on, cheat !
    } else {
      strcpy(configuration.pilot,
             pilot_entry); // No - normal name - save it in config file
      control_saveconfigure();
    }
  }
...
void
menu_usermessage(long message_type)

// Purpose : Displays user interaction message
// Accepts : Message type to display
// Returns : Nothing

{
  ...
  // Draw backdrop for message

  cel_rectangle(160 - 100, 120 + 40, 160 + 100, 120 - 40, BLUE_PAL,
                REC_BORDER | REC_BOX, 1);
  ...
  switch (message_type) {
    case USER_NVRAMFULL:
      font_paint(0, WHITE_15, 120 - 8, 110 - 5, MENU_FONT, SCR_UPDATE,
                 message_decode(MMNU_USER + 0, 1));
      font_paint(0, YELLOW_15, 120 - 45, 130 - 5, MENU_FONT, SCR_UPDATE,
                 message_decode(MMNU_USER + 1, 1));
      break;

    case USER_DEFAULTCONFIG:
      font_paint(0, WHITE_15, 120 - 39, 120 - 5, MENU_FONT, SCR_UPDATE,
                 message_decode(MMNU_USER + 2, 1));
      break;
  }

  screen_setswap(1); // Set screen swap
  screen_update(S_WAIT | S_BANK);
