...
menu			menus [MAX_MENUS] =
			
	{
		...
		{-1,	-1,	-1,	ITEM_TN,	4,	ITEM_OK,	0,			menu_savegamefile,			0,				0,	0,	ORANGE_15, 	{MMNU16 + 2}},
		{-1,	-1,	-1,	ITEM_TN,	5,	ITEM_OK,	0,			menu_movedown,				LOAD_MENU,		0,	0,	ORANGE_15, 	{MMNU0 + 3}},
		...
		NO_MENU,
		4,	{{RED_PAL, 0,0},	{BLUE_PAL,	2,2},	{BLUE_PAL,	4,5},	{GREY_PAL,	7,7}},
				
		4,
		0,
			{
		{-1,			-1,	-1,			ITEM_TN,	0,	ITEM_OK,	0,	menu_movedown,	FORMATION_MENU,	0,	0,	YELLOW_15, 	{MMNU2_FORMATION}},
		{BASECEL_TICK,	32,	ITEM_PR,	ITEM_TE,	2,	ITEM_OK,	0,	NULL,			0,				0,	0,	YELLOW_15, 	{MMNU2 + 1}},
		{BASECEL_TICK,	32,	ITEM_PR,	ITEM_TE,	4,	ITEM_OK,	0,	NULL,			0,				0,	0,	YELLOW_15, 	{MMNU2 + 2}},
		{BASECEL_TICK,	32,	ITEM_PR,	ITEM_TE,	5,	ITEM_OK,	0,	NULL,			0,				0,	0,	YELLOW_15, 	{MMNU4_LOOKAT}},
		{-1,			-1,	-1,			ITEM_TN,	7,	ITEM_OK,	0,	menu_quitgame, 			-1,				2,	0,	ORANGE_15,	{MMNU2 + 3, MMNU2 + 4, MMNU2 + 5}},
			}
		},
		...
		{-1,	-1,	-1,	ITEM_TM_S,	7,	ITEM_OK,	0,	menu_trackset,	7,	0,	0,	YELLOW_15,	{MMNU11_NEW}},
		...
		// Configure Menu (11) - NOTE : SELECT LANGUAGE OPTION REMOVED FOR AMERICAN VERSION (16/10/95)
		// -------------------------------------------------------------------------------------------
		...
		{-1,	-1,	-1,	ITEM_TN,	2,	ITEM_OK,	0,	menu_setvideo,		-1,				1,	1,	WHITE_15,	{MMNU11_NEW+1, MMNU11_NEW+2}},
...
menu_item	*item_draw;
	...
	if (menu_status.current_menu != current_menu)
		...
		if (menu_status.current_menu == PAUSE_MENU)		// If returning from pause menu to game, reset # weapon count textcel
			message_addweapon ();
	...
	switch (current_menu)
		...
		case PAUSE_MENU :
			if (menus [PAUSE_MENU].cur_option != 4)
				{
				menus [PAUSE_MENU].items [4].cur_selection = 0;				// If we're not on 'Quit' then reset it !
				menus [PAUSE_MENU].items [4].item_colour = ORANGE_15;
				}
			
			menus [PAUSE_MENU].items [0].selectable = (menu_displayformation (0) == 0) ? ITEM_NOTOK : ITEM_OK;	// Form Control ?
			menus [PAUSE_MENU].tick_position = (status.current_camera != 0) ? status.current_camera : -1;		// Set tick for menu (1 or 2) depending on camera
			break;
		...
		case FORMATION_MENU :
			...
			menus [FORMATION_MENU].items [0].cur_selection = status.formation_mode;	// Reset menu to formation control mode
	...
	if (menus [current_menu].max_option >=0)
		...
		for (option_draw=0; option_draw <= menus [current_menu].max_option; option_draw++)
			...
			// ARE WE DISPLAYING FLIGHT CONTROLS OR CAMERAS IN PAUSE MENU (IF SO, DRAW ON LEFT)
			
			if ((current_menu == FLIGHTCONTROL_MENU && option_draw <=7) ||
				(current_menu == PAUSE_MENU && (	option_draw == 1 ||
													option_draw == 2 ||
													option_draw == 3)))
		...
		if (current_menu == PAUSE_MENU)
			for (option_draw = 1; option_draw <=3; option_draw++)
				...
				item_draw = & (menus [current_menu].items [option_draw]);
				SetTextCelColor(cel_text [cel_base], 0, WHITE_15);
				UpdateTextInCel(cel_text [cel_base], TRUE, message_decode (item_variable, 1));
				GetTextCelSize (cel_text [cel_base], &width, &height);
				SetTextCelCoords(cel_text [cel_base], 270-width, MENU_YOFFS + (MENU_YSIZE*item_draw->y_position)+menus [current_menu].ypos);
				(cel_text [cel_base] -> tc_CCB) -> ccb_Flags &= ~CCB_LAST;
				cel_base++;
	...
	if (current_menu == DEBRIEF_MENU)
		...
		for (option_draw = 0; option_draw <=4; option_draw++)
			...
			SetTextCelCoords(cel_text [cel_base], 50, MENU_YOFFS + (MENU_YSIZE*item_ypos)+menus [current_menu].ypos);
			(cel_text [cel_base] -> tc_CCB) -> ccb_Flags &= ~CCB_LAST;
			cel_base++;
	...
	if (current_menu->max_option >=0)
		...
		if ((keypad & ControlA) && item_update->selectable == ITEM_OK)
			...
			if (menu_status.current_menu != PAUSE_MENU)
				return (item_update -> command);			// Return (Except if pause menu, in which case, carry on)
			}
...
menu_item	*item_display;
	...
	if (menu_status.hidden == 0)
		...
		for (menu_displayloop=0; menu_displayloop < current_menu->border_count; menu_displayloop++)
			cel_rectangle (40, current_menu->ypos + (MENU_YSIZE*current_menu->borders [menu_displayloop].option_end) + MENU_YOFFS + 17, 319-40, current_menu->ypos + (MENU_YSIZE*current_menu->borders [menu_displayloop].option_start) + MENU_YOFFS - 7, current_menu->borders [menu_displayloop].colour, REC_BORDER | REC_BOX, 1);
		...
		if (current_menu->max_option >=0)
			...
			if ((menu_status.current_menu == FLIGHTCONTROL_MENU && look_option <=7) ||
				(menu_status.current_menu == PAUSE_MENU && (	look_option == 1 ||
																look_option == 2 ||
																look_option == 3)))
												
				cel_rectangle (40, current_menu->ypos + (MENU_YSIZE*item_display->y_position) + MENU_YOFFS + 17, 50+item_width, current_menu->ypos + (MENU_YSIZE*item_display->y_position) + MENU_YOFFS - 7, RED_PAL, REC_BORDER | REC_BOX, 1);
			else
				cel_rectangle (160-item_halfwidth, current_menu->ypos + (MENU_YSIZE*item_display->y_position) + MENU_YOFFS + 17, 160+item_halfwidth, current_menu->ypos + (MENU_YSIZE*item_display->y_position) + MENU_YOFFS - 7, RED_PAL, REC_BORDER | REC_BOX, 1);
			}
		...
		if (current_menu->tick_position >=0)
			...
			cel_quad.y_pos0 = (current_menu->ypos + (MENU_YSIZE*item_display->y_position)) + MENU_YOFFS - 3 - 120;
...
{
	if (confirmed == 1)
		menus [PAUSE_MENU].items [4].item_colour = WHITE_15;	// Set 'Confirm' colour to white
	else
		{
...
void	menu_updateviewpresets (long keypad, long keypad_constant)
...

{
	switch (menus [PAUSE_MENU].cur_option)
		...
		case 1 :											// PLAYCAM CAMERA TYPE CONTROLS
			...
			if (keypad & ControlLeft)						// Changing playcam type (ie. tracking / flyby etc.) ?
				{
				if ((--(camera [1] . type)) < 0)			
					camera [1].type = 3;
			...
			if (keypad & ControlRight)						// Changing playcam type (ie. tracking / flyby etc.) ?
				{
				if ((++(camera [1]. type)) > 3)				
					camera [1].type = 0;
				control_changecamera (1);
		...
		case 2 :											// GAMECAM CAMERA TYPE CONTROLS
			...
			if (keypad & ControlLeft)						// Changing gamecam type (ie. tracking / flyby etc.) ?
				{
				if ((--(camera [2] . type)) < 0)			
					camera [2].type = 3;
			...
			if (keypad & ControlRight)						// Changing gamecam type (ie. tracking / flyby etc.) ?
				{
				if ((++(camera [2]. type)) > 3)				
					camera [2].type = 0;
		...
		case 3 :											// GAMECAM LOOK AT TYPE CONTROLS
			...
			if (keypad & ControlLeft)						// Changing gamecam look at type ? (ie. weapon / enemy etc.) ?
				{
				if ((--(camera[2].view)) < 0)
					camera [2].view = 3;
			...
			if (keypad & ControlRight)						// Changing gamecam look at type ? (ie. weapon / enemy etc.) ?
				{
				if ((++(camera[2].view)) > 3)
					camera [2].view = 0;
			...
			if (keypad & ControlRightShift)					// Do we want to select next instance of type ?
				{
			...
			if (keypad & ControlLeftShift)					// Do we want to select next instance of type ?
				{
				control_searchcamera (2, --camera[2].instance);
				control_changecamera (2);
				message_addcamera (2);
...
long	loop,
	...
	for (loop = 0; loop < 8; loop++)
		...
		cel_quad.y_pos0 = menus[JUKEBOX_MENU].ypos + (MENU_YSIZE*menus[JUKEBOX_MENU].items [loop].y_position) + MENU_YOFFS - 3 - 120;
		cel_quad.x_pos0 = 90;
	...
	for (loop = 0; loop < MUSIC_PROGRAMLENGTH; loop++)
		...
		cel_quad.y_pos0 = menus[JUKEBOX_MENU].ypos + (MENU_YSIZE*menus[JUKEBOX_MENU].items [track_no].y_position) + MENU_YOFFS - 1 - 120;
...
{
	menu_getname (user_savefilename, 16, message_decode (MMNU_GETNAME+2, 1), 0);	// Get filename
	
	if (user_savefilename [0] == 0)													// Check for null filename
		return;
		
	if (nvram_directory (SAVE_MENU, 0, 6, ITEM_OK, GAMEFILE_PREFIX) >=7)			// How many already in ?
...
void	menu_setvideo (long new_videoflag)
...
char	pilot_entry [24]; 
	...
	if (cheat == 1)
		...
		// NOTE - IF LANGUAGE SELECTION IS PUT BACK IN, CHANGE ITEMS [2] TO ITEMS [3] TO SET CHEAT BECAUSE OF THE EXTRA
		// INSERTED LINE IN THE MENU.
...
void	menu_usermessage (long	message_type)
...
long	keypad;
	...
	cel_rectangle (160-100, 120+40, 160+100, 120-40, BLUE_PAL, REC_BORDER | REC_BOX, 1);
	...
	switch (message_type)
		{
		case USER_NVRAMFULL :
			font_paint(0, WHITE_15, 120-8, 110-5, MENU_FONT, SCR_UPDATE, message_decode (MMNU_USER+0, 1));
			font_paint(0, YELLOW_15, 120-45, 130-5, MENU_FONT, SCR_UPDATE, message_decode (MMNU_USER+1, 1));
			break;
			
		case USER_DEFAULTCONFIG :
			font_paint(0, WHITE_15, 120-39, 120-5, MENU_FONT, SCR_UPDATE, message_decode (MMNU_USER+2, 1));
			break;
		}
