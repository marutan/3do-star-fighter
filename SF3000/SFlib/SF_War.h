...
#define __WAR_SR_LOCAL                                                         \
  40 << 24 // Max search distance (Local) for opponent targets (in any
           // direction, +x, -x, etc.)
#define __WAR_SR_AREA                                                          \
  127 << 24 // Max search distance (Wide area) for opponent targets
#define __WAR_MF                                                               \
  8 // Max number of people in formation (including null entry for player)
#define __WAR_MDC_FRIEND                                                       \
  4 // Max search distance (in sprites) [against] player distance check	FRIENDLY
#define __WAR_MDC_ENEMY                                                        \
  8 // Max search distance (in sprites) [against] player distance check	ENEMY
...
{
  ship_stack *occupied; // Is this formation position occupied ? - if not NULL,
                        // then address of ship
...
void war_updatemissionship(ship_stack *,
                           ship_sdb *); // Update a particular mission ship

void war_updateship_attack(ship_stack *,
                           ship_sdb *); // Update a ship in ATTACK MODE
void war_updateship_flightpath(ship_stack *,
                               ship_sdb *); // Update a ship in FLIGHTPATH MODE
void war_updateship_formation(ship_stack *,
                              ship_sdb *); // Update a ship in FORMATION MODE

void war_update_attacked_friend(
    ship_stack *,
    ship_sdb *); // Update a friendly ship that has been fired upon
...
void ship_settoflightpoint(
    ship_stack *,
    ship_sdb *); // Set a ship's gotoxyz from it's flightpath point
void war_update_multipleattackrequests(
    ship_stack *,
    ship_sdb
...
void
war_distancecheckwithplayer(ship_stack *,
