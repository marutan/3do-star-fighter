...
void
palette_set(long p_red, long p_green, long p_blue)
...

{
  ...
  for (bank = 0; bank < screen->sc_NumScreens; bank++) {
    for (clut_reg = 0; clut_reg < 32; clut_reg++) {
      SetScreenColor(screen->sc_ScreenItems[bank],
                     MakeCLUTColorEntry(clut_reg,
                                        ((clut_reg << 3) * p_red) >> 8,
