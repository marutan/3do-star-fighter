...

{
  ...
  long hoz_y; //= ( sine_table [ camera_x_rotation ] * cosine_table [
              //camera_y_rotation ] )>>12 ;
  long hoz_z; //= ( sine_table [ camera_x_rotation ] * sine_table [
              //camera_y_rotation ] )>>12 ;

  long vert_x; //= cosine_table [ camera_x_rotation+256 ] ;
  long vert_y; //= ( sine_table [ camera_x_rotation+256 ] * cosine_table [
               //camera_y_rotation ] )>>12 ;
  long vert_z; //= ( sine_table [ camera_x_rotation+256 ] * sine_table [
               //camera_y_rotation ] )>>12 ;
  ...
  // rot_data->offset_x = -((((( ( camera_x_position >> 12 ) & 4095 )
  // ROT_OFFSET) * cosine_table [ camera_x_rotation ] )>>12) +
  //					((( ( ( camera_y_position >> 12 ) & 4095
  //) ROT_OFFSET) * cosine_table [ camera_x_rotation+256 ] )>>12))
  //					-(((hoz_x<<4) + (hoz_x>>1) + (vert_x<<4)
  //+ (vert_x>>1))) ;

  // rot_data->offset_y = (((camera_z_position>>12)*sine_y)>>12) -
  //					((((( ( ( camera_x_position >> 12 ) &
  //4095 ) ROT_OFFSET) * sine_table [ camera_x_rotation ] )>>12) * cosine_y
  //)>>12) -
  //					((((( ( ( camera_y_position >> 12 ) &
  //4095 ) ROT_OFFSET) * sine_table [ camera_x_rotation+256 ] )>>12) * cosine_y
  //) >> 12)
  //					-(((hoz_y<<4) + (hoz_y>>1) + (vert_y<<4)
  //+ (vert_y>>1))) ;

  // rot_data->offset_z = (((camera_z_position>>12)*cosine_y)>>12) +
  //					((((( ( ( camera_x_position >> 12 ) &
  //4095 ) ROT_OFFSET) * sine_table [ camera_x_rotation ] )>>12) * sine_y )>>12)
  //+
  //					((((( ( ( camera_y_position >> 12 ) &
  //4095 ) ROT_OFFSET) * sine_table [ camera_x_rotation+256 ] )>>12) * sine_y
  //)>>12)
  //					-(((hoz_z<<4) + (hoz_z>>1) + (vert_z<<4)
  //+ (vert_z>>1))) ;
  ...
  */
  ...
  for (loop = 0; loop <= 6; loop += 1) {
    ...
    // rot_data->offset_x = -((((( ( camera_x_position>>shift) &16383) -8192)
    // *cosine_table[camera_x_rotation])>>12) +
    //					  ((( ( ( camera_y_position>>shift)
    //&16383) -8192) *cosine_table[camera_x_rotation+256])>>12))
    //					  -(((hoz_x<<5) + (hoz_x<<1) +
    //(vert_x<<5) + (vert_x<<1))) ;

    // rot_data->offset_y = ((( camera_z_position>>shift)*sine_y)>>12) -
    //				((((( ( ( camera_x_position>>shift) &16383)
    //-8192) *sine_table[camera_x_rotation]) >>12)*cosine_y)>>12) -
    //				((((( ( ( camera_y_position>>shift) &16383)
    //-8192) *sine_table[camera_x_rotation+256]) >>12)*cosine_y)>>12)
    //			  -(((hoz_y<<5) + (hoz_y<<1) + (vert_y<<5) +
    //(vert_y<<1))) ;

    // rot_data->offset_z = ((( camera_z_position>>shift)*cosine_y)>>12) +
    //				((((( ( ( camera_x_position>>shift) &16383)
    //-8192)*sine_table[camera_x_rotation])>>12)*sine_y)>>12) +
    //				((((( ( ( camera_y_position>>shift) &16383)
    //-8192)*sine_table[camera_x_rotation+256])>>12)*sine_y)>>12)
    //				-(((hoz_z<<5) + (hoz_z<<1) + (vert_z<<5) +
    //(vert_z<<1))) ;
  ...
  if (camera_z_position > (32 * (1 << 24))) {
    res = 1;
  }
  if (camera_z_position > (48 * (1 << 24))) {
    res = 2;
  }
  if (camera_z_position > (64 * (1 << 24))) {
    res = 3;
  }
