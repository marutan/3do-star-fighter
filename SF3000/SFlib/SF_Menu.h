...
#define ITEM_TM_C                                                              \
  2 // Item can be ticked along with others, but is not currently
...
typedef struct menu_info {
  long current_menu, command, selected, hidden;
  CCB *cel_textccb;
} menu_info;
...
void
menu_setmusictracks(void);    // Setup music playlist from menu track selections
...
void menu_savegamefile(
    long); // Gets filename & saves (If no room, goes into replace menu)
...
void
menu_setlanguage(long);     // Change language of game [CURRENTLY EUROPEAN ONLY]
