#ifndef __3do_nodes_h__
#define __3do_nodes_h__

#include <stdint.h>

#include "types.h"

#define MKNODEID(a,b)   (int32_t)( ((a)<<8) | (b))

/* Standard Node structure */
typedef struct Node
{
  struct Node *n_Next;  /* pointer to next node in list */
  struct Node *n_Prev;  /* pointer to previous node in list */
  uint8_t n_SubsysType;   /* what folio manages this node */
  uint8_t n_Type;         /* what type of node for the folio */
  uint8_t n_Priority;     /* queueing priority */
  uint8_t n_Flags;                /* misc flags, see below */
  int32_t  n_Size;                /* total size of node including hdr */
  /* Optional part starts here */
  char *n_Name;         /* ptr to null terminated string or NULL */
} Node, *NodeP;

typedef struct ItemNode
{
  struct ItemNode *n_Next; /* pointer to next itemnode in list */
  struct ItemNode *n_Prev; /* pointer to previous itemnode in list */
  uint8_t n_SubsysType;   /* what folio manages this node */
  uint8_t n_Type;         /* what type of node for the folio */
  uint8_t n_Priority;     /* queueing priority */
  uint8_t n_Flags;                /* misc flags, see below */
  int32_t n_Size;         /* total size of node including hdr */
  char *n_Name;         /* ptr to null terminated string or NULL */
  uint8_t n_Version;      /* version of of this itemnode */
  uint8_t n_Revision;     /* revision of this itemnode */
  uint8_t n_FolioFlags;   /* flags for this item's folio */
  uint8_t n_ItemFlags;    /* additional system item flags */
  Item  n_Item;         /* ItemNumber for this data structure */
  Item  n_Owner;                /* creator, present owner, disposer */
  void *n_ReservedP;    /* Reserved pointer */
} ItemNode, *ItemNodeP;

/* Node structure used for linking only */
typedef struct MinNode
{
  struct MinNode *n_Next;
  struct MinNode *n_Prev;
} MinNode;

#endif /* __3do_nodes_h__ */
